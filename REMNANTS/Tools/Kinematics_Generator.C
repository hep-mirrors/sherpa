...
FillBlobs(Blob_List *blobs,const Poincare * labboost) {
  ...
  return TransverseKinematics(labboost);
}
...
bool Kinematics_Generator::TransverseKinematics(const Poincare * labboost) {
  switch (m_kintype) {
  case kin_type::DIS1:
    if (!TransverseKinematicsDIS(0)) return false;
    break;
  case kin_type::DIS2:
    if (!TransverseKinematicsDIS(1)) return false;
    break;
  case kin_type::hh:
    if (!TransverseKinematicsHH())   return false;
    break;
  default:
    THROW(fatal_error,
