...
std::set<ATOOLS::Particle*> REMNANTS::Remnant_Base::s_last[2];
...
{
  msg_Out()<<METHOD<<" for "<<parton->Number()<<" ("<<parton->Flav()<<"): ";
  if (TestExtract(parton)) {
    m_initialized=true;
    m_extracted.push_back(parton); 
    m_erem-=parton->Momentum()[0]+m_lastemin;
    msg_Out()<<"success, now "<<m_extracted.size()<<".\n";
    return true;
  }
  msg_Out()<<"fail, now "<<m_extracted.size()<<".\n";
  return false;
}
			       ...
			       const ATOOLS::Vec4D &mom) 
{
  ...
  if (E<0.0 || (E>Eb && !ATOOLS::IsEqual(E,Eb,1.0e-3))) {
    ...
    msg_Error()<<"Remnant_Base::TestExtract("<<flav<<","<<E<<"): "
	       <<"Constituent energy out of range E_b = "<<Eb<<".\n";
    m_laste=E;
    return false;
  }
  ...
  if (erem<0.0) {
    msg_Tracking()<<METHOD<<": No remaining energy for "<<flav<<", "
		  <<"E = "<<E<<" -> E_min = "<<(E+m_lastemin)<<".\n";
    return false;
  }
  if (E<=m_emin) {
    msg_Tracking()<<METHOD<<": Energy exceeds minimum for "
