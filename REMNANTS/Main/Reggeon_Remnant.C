...
Reggeon_Remnant::Reggeon_Remnant(PDF::PDF_Base* pdf, const size_t& beam,
                                 const size_t& tag)
    : Remnant_Base(pdf->Bunch(), beam, tag), p_pdf(pdf),
...
bool Reggeon_Remnant::FillBlob(Colour_Generator* colours, ParticleMomMap* ktmap,
                               const bool& copy)
{
  ...
  // In the Reggeon, there needs to be at least one quark-antiquark pair,
  // this is tracked with the m_valence flag.
  MakeRemnants(colours);
  msg_Debugging() << METHOD
                  << ": Filling blob with remnants, extracted = " << m_extracted
...
void Reggeon_Remnant::Reset(const bool& resc, const bool& DIS)
{
  Remnant_Base::Reset();
  while (!m_spectators.empty()) {
    Particle* part = m_spectators.front();
    if (part->ProductionBlob()) part->ProductionBlob()->RemoveOutParticle(part);
    if (part->DecayBlob()) part->DecayBlob()->RemoveInParticle(part);
    delete part;
...
void Reggeon_Remnant::Output() const
{
  msg_Out() << METHOD << "(" << m_beam << ", " << m_beamflav << ").\n"
            << "   Partons are { ";
  for (const auto& p_parton : *p_partons) { msg_Out() << " " << p_parton; }
  msg_Out() << "}.\n";
}
...
bool Reggeon_Remnant::TestExtract(const Flavour& flav, const Vec4D& mom)
{
  ...
  // the energy of the extracted parton and potentially the mass of its
                                              ...
                                              const bool&     copy)
{
   ...
   * Each parton should have an energy greater than its (hadron) mass,
   * additionally energy must be conserved. This requires book-keeping of two
   * variables:
   * 1. The masses that still need too be generated
   * 2. The energy that has been used so far.
   * For the fit we use the same parameters as for the photon remnants
   */
                                ...
                                double remnant_masses) const
{
  ...
  // Taken from Hadron_Remnant, exponents taken from photon PDFs
  if (zmax < zmin) {
    msg_Debugging() << METHOD << ": Error, zmin, zmax = " << zmin << ", "
                    << zmax << "\n";
    return 0;
  }
  double z;
  ...
  if (beta != -1) {
    double rand = ran->Get();
    z = pow(rand * pow(zmax, beta + 1.) + (1. - rand) * pow(zmin, beta + 1.),
            invb);
  } else
...
void Reggeon_Remnant::MakeSpectator(Particle* parton, Colour_Generator* colours)
{
...
void Reggeon_Remnant::MakeRemnants(Colour_Generator* colours)
{
  if (m_valence) return;
  Particle* part;
  Flavour   quark;
...
double Reggeon_Remnant::EstimateRequiredEnergy(bool needs_valence_quarks) const
{
  double masses = 0.;
  for (Particle const* pit : m_spectators) {
...
void Reggeon_Remnant::FindRecoiler()
{
  for (auto part : m_spectators) {
    if (part->Flav().IsQuark()) p_recoiler = part;
  }
}
