...
class runcard():
    ...
    # def get_documented_commands(self):
    #     for l in open('commands.index'):
    #         cname = l.rstrip('\n')
    #         self.doc_commands['command_name'].append(cname)
    #         reg_expr = re.compile(cname,re.X)
    #         self.doc_commands['matching_expr'].append(reg_expr)
    ...
    def get_selectors(self):
        for selline in re.findall('selector(.*?)selector', self.uniformed, re.S):
            ...
            # selline = selline.replace('\n\n','\n')
            # for l in iter(selline.splitlines()):
            #     # remove empty entries
            #     if(not (l.startswith(')') or l.startswith('}'))):
            #         sel = l.rstrip(';')
            #         # CHECK this
            #         # self.selectors['SELECTORS'][sel.split(' ')[0]]= sel.split(' ')[1:]
...
# rc = runcard('/Users/Nappo/work/hep/code/ew_sudakov/Examples/V_plus_Bs/LHC_Wbb/Run.dat')
# rc = runcard('/Users/Nappo/work/hep/code/ew_sudakov/Examples/V_plus_Jets/LHC_ZJets/Run.dat')
