...
namespace SHERPA {
  ...
  public:
    ...
    {
      Scoped_Settings s{ Settings::GetMainSettings()["HDF5"] };

      m_basename=args.m_outpath+"/"+args.m_outfile;
      m_setcol= s["SET_COLORS"].SetDefault(1).Get<int>();
      m_ncache= s["CACHE_SIZE"].SetDefault(10000).Get<int>();
      m_unweight=s["UNWEIGHT"].SetDefault(0).Get<int>();
      m_clabel=s["START_COLOR_LABEL"].SetDefault(500).Get<int>();
      ...
      params = s["MPIIO_PARAMS"].SetDefault<std::string>({}).GetVector<std::string>();
      MPI_Info info;
	...
	MPI_Info_set(info,params[i].c_str(),params[i+1].c_str());
      ...
      // these are good enough optimziations that I don't think we need to make them optional.
      // first, requests MPI-IO for file operations
      fapl.add(MPIOFileAccess{MPI_COMM_WORLD, info});
      // second, when opening the file, perform any HDF5 internal operations collectively
    ...
    bool SetSumSqrColors(Cluster_Amplitude *const ampl)
    {
      ...
      std::shared_ptr<Color_Integrator> colint(proc->Integrator()->ColorIntegrator());
	...
	if (!valid) continue;
	csum+=psum[i]=dabs(proc->Differential(*ampl,Variations_Mode::nominal_only, 1|2|4).Nominal());
    ...
    void SetColors(Blob_List* blobs)
    {
      ...
      std::shared_ptr<Color_Integrator> ci(proc->Integrator()->ColorIntegrator());
    ...
    /* collective: everyone calls Header */
    void Header()
    {
	...
	eprops[0]=1;
	eprops[1]=m_nmax;
	eprops[2]=(m_ecache.size()+m_offset)*m_nmax;
