  ...
  Output_Base("HepMC3")
{
  ...
  m_pluginlibrary = Settings::GetMainSettings()["HEPMC3_OUTPUT_PLUGIN"].SetDefault("").Get<std::string>();
  m_pluginfunction = Settings::GetMainSettings()["HEPMC3_OUTPUT_FUNCTION"].SetDefault("").Get<std::string>();  
#ifdef USING__GZIP
  m_ext += ".gz";
#endif
#ifdef USING__MPI
