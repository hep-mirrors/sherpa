...
DECLARE_GETTER(MODEL::Test_Standard_Model,"UFOVariationsSMTest",MODEL::Model_Base,MODEL::Model_Arguments);

Model_Base *ATOOLS::Getter<MODEL::Model_Base,MODEL::Model_Arguments,MODEL::Test_Standard_Model>::
operator()(const Model_Arguments &args) const
{
  return new Test_Standard_Model();
}

void ATOOLS::Getter<MODEL::Model_Base,MODEL::Model_Arguments,MODEL::Test_Standard_Model>::
...
bool Test_Standard_Model::ModelInit()
{
  FixEWParameters();
  FixCKM();
...
void Test_Standard_Model::FixCKM()
{
...
void Test_Standard_Model::InitVertices()
{
  InitQEDVertices();
  InitQCDVertices();
  InitEWVertices();
...
void Test_Standard_Model::InitQCDVertices()
{
  Settings& s = Settings::GetMainSettings();
  if (!Flavour(kf_gluon).IsOn()) return;
  m_dec_g4 = s["DECOMPOSE_4G_VERTEX"].Get<int>();
  Kabbala g3 (sqrt(4.*M_PI*Kabbala("alpha_S", ScalarConstant("alpha_S"))));
  Kabbala cpl0=g3*Complex(0.,1.);
  ...
  Kabbala cpl2=g3*g3*Complex(0.,1.);
  m_v.push_back(Single_Vertex());
