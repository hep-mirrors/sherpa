...
namespace MCFM {
  ...
  class MCFM_qqb_z_v: public PHASIC::Virtual_ME2_Base {
  private:
    double *p_p, *p_msqv;
  public:
    MCFM_qqb_z_v(const PHASIC::Process_Info& pi,
...
MCFM_qqb_z_v::MCFM_qqb_z_v(const Process_Info& pi,
...
MCFM_qqb_z_v::~MCFM_qqb_z_v()
{
  delete [] p_p;
  delete [] p_msqv;
}

void MCFM_qqb_z_v::Calc(const Vec4D_Vector &p)
{
  const double sf(4.0*9.0);
  for (int n(0);n<2;++n) GetMom(p_p,n,-p[n]);
  for (int n(2);n<4;++n) GetMom(p_p,n,p[n]);
...
double MCFM_qqb_z_v::Eps_Scheme_Factor(const ATOOLS::Vec4D_Vector& mom)
{
  return 4.*M_PI;
}
...
DECLARE_VIRTUALME2_GETTER(MCFM_qqb_z_v_Getter,"MCFM_qqb_z_v")
Virtual_ME2_Base *MCFM_qqb_z_v_Getter::operator()(const Process_Info &pi) const
{
  ...
  if (pi.m_fi.m_nloqcdtype&nlo_type::loop) {
    ...
    if (fl[0].IsQuark() && fl[1]==fl[0].Bar() && fl[3]==fl[2].Bar() &&
