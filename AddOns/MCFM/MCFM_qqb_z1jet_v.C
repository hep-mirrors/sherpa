...
namespace MCFM {
  ...
  class MCFM_qqb_z1jet_v: public PHASIC::Virtual_ME2_Base {
  private:
    bool    m_neutrinos;
    double *p_p, *p_msqv;
  public:
    MCFM_qqb_z1jet_v(const PHASIC::Process_Info& pi,
...
extern "C" { void qqb_z1jet_v_(double *p,double *msqv); }
...
MCFM_qqb_z1jet_v::MCFM_qqb_z1jet_v(const Process_Info& pi,
...
MCFM_qqb_z1jet_v::~MCFM_qqb_z1jet_v()
{
  delete [] p_p;
  delete [] p_msqv;
}

void MCFM_qqb_z1jet_v::Calc(const Vec4D_Vector &p)
{
  ...
  //         <<"   Results["<<i<<", "<<j<<"]: "<<res2<<" "<<res1<<" "<<res
  //	   <<" for "<<qcdcouple_.ason2pi<<"("<<qcdcouple_.as<<") = "<<ason2pi
  //	     <<"."<<std::endl;
}

double MCFM_qqb_z1jet_v::Eps_Scheme_Factor(const ATOOLS::Vec4D_Vector& mom)
{
  return 4.*M_PI;
}
...
DECLARE_VIRTUALME2_GETTER(MCFM_qqb_z1jet_v_Getter,"MCFM_qqb_z1jet_v")
Virtual_ME2_Base *MCFM_qqb_z1jet_v_Getter::
operator()(const Process_Info &pi) const
{
  ...
  if (pi.m_fi.m_nloqcdtype&nlo_type::loop) {
	...
	chooser_();
      ...
      return new MCFM_qqb_z1jet_v(pi,fl,pID==42);
    }
  }
  return NULL;
}
