...
extern "C" { 
  ...
  void qqb_hww1jet_v_(double *p,double *msqv); 
}
...
void MCFM_qqb_ww1jet_v::Calc(const Vec4D_Vector &p)
{
  ...
  if (!m_ishiggs) qqb_ww1jet_v_(p_p,p_msqv);
             else qqb_hww1jet_v_(p_p,p_msqv);
...
Virtual_ME2_Base *MCFM_qqb_ww1jet_v_Getter::operator()(const Process_Info &pi) const
{
  ...
  if (pi.m_fi.m_nloqcdtype&nlo_type::loop) {
	...
	pID = 66;
      ...
      msg_Out()<<"   found FS in agreement with W pair "
	       <<"("<<pi.m_fi.m_ps.size()<<")."<<std::endl;
	...
	if (fl1==ATOOLS::Flavour(kf_h0) && Flavour(kf_h0).IsOn() &&
	  ...
	  msg_Out()<<"One decay with "<<fl1<<"."<<std::endl;
