...
namespace MCFM {
  ...
  class MCFM_qqb_w2jet_v: public PHASIC::Virtual_ME2_Base {
  private:
    double *p_p, *p_msqv;
  public:
    MCFM_qqb_w2jet_v(const PHASIC::Process_Info& pi,
...
MCFM_qqb_w2jet_v::MCFM_qqb_w2jet_v(const Process_Info& pi,
...
MCFM_qqb_w2jet_v::~MCFM_qqb_w2jet_v()
{
  delete [] p_p;
  delete [] p_msqv;
}

void MCFM_qqb_w2jet_v::Calc(const Vec4D_Vector &p)
{
...
double MCFM_qqb_w2jet_v::Eps_Scheme_Factor(const ATOOLS::Vec4D_Vector& mom)
{
  return 4.*M_PI;
}
...
DECLARE_VIRTUALME2_GETTER(MCFM_qqb_w2jet_v_Getter,"MCFM_qqb_w2jet_v")
Virtual_ME2_Base *MCFM_qqb_w2jet_v_Getter::
operator()(const Process_Info &pi) const
{
  ...
  if (pi.m_fi.m_nloqcdtype&nlo_type::loop) {
    ...
    int pID(0);
    if (fl[0].Strong() && fl[1].Strong() && fl[4].Strong() && fl[5].Strong()) {
	...
	chooser_();
      ...
      return new MCFM_qqb_w2jet_v(pi,fl);
    }
  }
  return NULL;
}
