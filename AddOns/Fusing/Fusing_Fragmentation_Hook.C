...
//       ATOOLS::Data_Reader *reader =  p_sherpa->GetInitHandler()->DataReader();
       ...
       m_nlo = 1; //reader->GetValue<int>("FUSING_FRAGMENTATION_NLO_Mode", 1);
       m_store = 0; // reader->GetValue<int>("FUSING_FRAGMENTATION_STORE_AS_WEIGHT", 0);
       m_check_ho = 1; // reader->GetValue<int>("FUSING_FRAGMENTATION_CHECK_HO", 1);
  ...
  ATOOLS::Return_Value::code Run(ATOOLS::Blob_List* blobs, double &weight) {
    p_bl = blobs;
       ...
       m_pstypespec = p_bl->FindFirst(ATOOLS::btp::Shower)->TypeSpec();
       ...
       while (true){
           ...
           pos = CheckBPair(ampl);
           if (pos==0) {
               if(m_pstypespec=="DIRE") ampl=ampl->Prev();
               else  ampl = ampl->Next();
             }
           else break;
         }
