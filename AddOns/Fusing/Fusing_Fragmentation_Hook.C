...
public:
    ...
    Userhook_Base("Fusing_Fragmentation"), p_as(MODEL::as), p_sherpa(args.p_sherpa)
  {
    ...
    m_nlo = s["FUSING_FRAGMENTATION_NLO_Mode"].SetDefault(1).Get<int>();
    m_store = s["FUSING_FRAGMENTATION_STORE_AS_WEIGHT"].SetDefault(0).Get<int>();
    m_check_ho = s["FUSING_FRAGMENTATION_CHECK_HO"].SetDefault(1).Get<int>();
    ...
    CSSHOWER::CS_Shower* css = dynamic_cast<CSSHOWER::CS_Shower*>
      (p_sherpa->GetInitHandler()->GetShowerHandlers().at(PDF::isr::hard_process)->GetShower());
    if (css) css->EnableFusing(true);
    else THROW(fatal_error, "Fusing only available with CS Shower.");
  }
