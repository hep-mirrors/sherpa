#include "Recola_Virtual_SChannel_WpWp.H"
...
namespace Recola {
  ...
  Recola_Virtual_SChannel_WpWp::Recola_Virtual_SChannel_WpWp(const Process_Info& pi,
  ...
  void Recola_Virtual_SChannel_WpWp::Calc(const Vec4D_Vector& momenta) {
    m_mode=m_modebackup;
  ...
  bool Recola_Virtual_SChannel_WpWp::IsMappableTo(const PHASIC::Process_Info& pi){
    return false;
  }
...
DECLARE_VIRTUALME2_GETTER(Recola_Virtual_SChannel_WpWp,"Recola_Virtual_SChannel_WpWp")
Virtual_ME2_Base *ATOOLS::Getter<PHASIC::Virtual_ME2_Base,PHASIC::Process_Info,Recola_Virtual_SChannel_WpWp>::
operator()(const PHASIC::Process_Info &pi) const
{
  DEBUG_FUNC(pi);
  if (pi.m_loopgenerator!="Recola_SChannel_W+W+") return NULL;
  ...
  if (procIndex>0) {
    ...
    return new Recola_Virtual_SChannel_WpWp(cpi, flavs, procIndex);
  }
  else {
    return NULL;
  }
