...
namespace Recola {
  ...
  Recola_Real::Recola_Real(const Process_Info& pi,
  ...
  void Recola_Real::Calc(const Vec4D_Vector& momenta) {
    m_mode=m_modebackup;
    ...
    Recola_Interface::EvaluateReal(m_recola_id, momenta, m_res, 1);
  ...
  bool Recola_Real::IsMappableTo(const PHASIC::Process_Info& pi){
    return false;
  }
...
operator()(const Process_Info &pi) const
{
  ...
  if (!pi.Has(nlo_type::real)) return NULL;

  int procIndex=Recola_Interface::RegisterProcess(pi, -1);
  
  if (procIndex>0) {
    Flavour_Vector flavs = pi.ExtractFlavours();
    return new Recola_Real(pi, flavs, procIndex);
  }
  else {
    return NULL;
  }
