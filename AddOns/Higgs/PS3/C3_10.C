...
namespace PHASIC {
  class C3_10 : public Single_Channel {
    ...
    Vegas* p_vegas;
  public:
    C3_10(int,int,Flavour*,Integration_Info * const);
    ~C3_10();
...
extern "C" Single_Channel * Getter_C3_10(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C3_10(nin,nout,fl,info);
}
...
void C3_10::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p234=p[0]+p[1];
  double s234_max = p234.Abs2();
...
void C3_10::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 0;
  width = 0.;
...
{
  return std::string("CGND$I_3_4$MTH_34$TC_0__1__2_34$ZS_0$");
}
