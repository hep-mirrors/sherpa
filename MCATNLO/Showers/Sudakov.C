...
void Sudakov::InitSplittingFunctions(MODEL::Model_Base *md,const int kfmode)
{
  ...
  for (Vertex_Table::const_iterator
	...
	Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::FF,kfmode,m_ewmode,1,m_pdfmin)));
	Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::FF,kfmode,m_ewmode,-1,m_pdfmin)));
	Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::FI,kfmode,m_ewmode,1,m_pdfmin)));
	Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::FI,kfmode,m_ewmode,-1,m_pdfmin)));
	if (v->in[0].Bar().Mass()<100.0) {
  	  Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::IF,kfmode,m_ewmode,1,m_pdfmin)));
  	  Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::IF,kfmode,m_ewmode,-1,m_pdfmin)));
 	  Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::II,kfmode,m_ewmode,1,m_pdfmin)));
 	  Add(new Splitting_Function_Base(SF_Key(p_rms,v,dmode,cstp::II,kfmode,m_ewmode,-1,m_pdfmin)));
	}
	if (v->in[1]!=v->in[2]) {
	  AddToMaps(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::FF,kfmode,m_ewmode,1,m_pdfmin)));
	  AddToMaps(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::FF,kfmode,m_ewmode,-1,m_pdfmin)));
	  AddToMaps(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::FI,kfmode,m_ewmode,1,m_pdfmin)));
	  AddToMaps(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::FI,kfmode,m_ewmode,-1,m_pdfmin)));
	  if (v->in[0].Bar().Mass()<100.0) {
  	    Add(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::IF,kfmode,m_ewmode,1,m_pdfmin)));
  	    Add(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::IF,kfmode,m_ewmode,-1,m_pdfmin)));
 	    Add(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::II,kfmode,m_ewmode,1,m_pdfmin)));
 	    Add(new Splitting_Function_Base(SF_Key(p_rms,v,1-dmode,cstp::II,kfmode,m_ewmode,-1,m_pdfmin)));
	  }
	}
      }
...
double Sudakov::GetVi() const {
  ...
  const double mw2   = sqr(Flavour(24).Mass());
  if(p_shower->KinFF()->m_evolscheme == 2) return m_kperp2/(m_z/2.*(Q2-mw2));
...
double Sudakov::GetViab(Parton *split) const {
  ...
  if(p_shower->KinFF()->m_evolscheme == 1){
    ...
    if(IsEqual(m_kperp2, kt2, 1e-3)) return pa*pb / (pi*(pa+pb));
  }
  else if(p_shower->KinFF()->m_evolscheme == 2){
    const double virtuality = pa*pi;
    if(IsEqual(m_kperp2, virtuality, 1e-3)) return pa*pb / (pi*(pa+pb));
  }
  return -1.;
}
...
bool Sudakov::Generate(Parton * split) 
{
  ...
  default:
    ...
    const bool veto(Veto(Q2, m_x, split));
...
bool Sudakov::DefineFFBoundaries(double x)
{
  ...
  if (OverIntegrated(m_zmin,m_zmax,m_scale,Q2,-1)<0.) {
...
bool Sudakov::Veto(double Q2,double x, Parton * split) {
  if (!Splitting(Q2,x,split))          return false;
  return true;
}

bool Sudakov::Splitting(double Q2,double x, Parton * split) {
...
double Sudakov::GetKt2(Parton * split) const {
  Vec4D pai_tilde = split->Momentum();
  Vec4D pw_tilde  = split->GetKinSpect()->Momentum();
  Vec4D pb        = split->GetSpect()->Momentum();
  const double mw2      = sqr(Flavour(24).Mass());
