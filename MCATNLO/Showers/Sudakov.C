...
bool Sudakov::Generate(Parton * split) 
{
  ...
  while (m_kperp2>=m_k0sqf) {
	...
	m_kperp2<k0sq*m_k0sq_gsplit_fac) return false;
    double Q2 = 0.;
    ...
    case (cstp::IF) : {
      ...
      m_y = p_shower->KinIF()->GetY(-Q2,m_kperp2,m_z,ma2,mi2,mk2,
				    ...
				    (*m_splitter)->GetFlavourC());
      m_x   = split->Xbj();
      if (m_kperp2<m_k0sqi || m_y<0.0 || m_y>1.0 || m_z<m_x) continue;
    }
      break;
    case (cstp::II) : {
      double ma2 = sqr(p_rms->Mass(((*m_splitter)->GetFlavourA())));
      ...
      m_y = p_shower->KinII()->GetY(Q2,m_kperp2,m_z,ma2,mi2,mb2,
				    ...
				    (*m_splitter)->GetFlavourC());
      m_x   = split->Xbj();
      if (m_kperp2<m_k0sqi || m_y<0.0 || m_y>1.0-m_z || m_z<m_x) continue;
    }
      break;
  default:
