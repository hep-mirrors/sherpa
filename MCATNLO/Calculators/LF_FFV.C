...
namespace MCATNLO {
  ...
  public:
    ...
    double Mu2(const double &z,const double &y,const double &Q2) const
    {
      return (Q2+p_ms->Mass2(m_flavs[0])+p_ms->Mass(m_flavs[2])+p_ms->Mass2(m_flspec))*y/z;
    }
   ...
   const double scale,const double Q2,Cluster_Amplitude *const sub)
{
  ...
  if (muij2==0. && mui2==0. && muk2==0.) {
    ...
    double value = 2.0 * p_cf->Coupling(mu2,0,sub) * massless + p_cf->Coupling(mu2,1,sub) * longpol;
    return value * JFF(y);
  }
  else {
    //the massive case
    ...
    double longpol = 0.5 * ( 1. - z );
    double value = 2.0 * p_cf->Coupling(mu2,0,sub) * massive + p_cf->Coupling(mu2,1,sub) * longpol;
    return value * JFF(y);
  } 
}
...
{
  double mu2 = Mu2(z,y,Q2);
  double value = 2.0 * p_cf->Coupling(mu2,0,sub) * ( 2./(1.-z+y) - (1.+z) + CDIS(z,y) )
    + p_cf->Coupling(mu2,1,sub) * 0.5 * ( 1. - z );
...
{
  double mu2 = Mu2(z,y,Q2);
  double value = 2.0 * p_cf->Coupling(mu2,0,sub) * ( 2./(1.-z) - (1.+z) )
    + p_cf->Coupling(mu2,1,sub) * 0.5 * ( 1. - z );
...
{
  double mu2 = Mu2(z,y,Q2);
  double value = 2.0 * p_cf->Coupling(mu2,0,sub) * ( 2./z - 2. +z )
    + p_cf->Coupling(mu2,1,sub) * 0.5 * z;
  return value * JII(z,y,eta,scale,sub);
}
...
{
  double mu2 = Mu2(z,y,Q2);
  double value = 2.0 * p_cf->Coupling(mu2,0,sub) * ( (1.-2.*z*(1.-z))*(1.0-0.5/z*CDIS(y,z)) + CDIS(z,y) )
    + p_cf->Coupling(mu2,1,sub) * 0.5;
  return value * JIF(z,y,eta,scale,sub);
}
...
{
  double mu2 = Mu2(z,y,Q2);
  double value = 2.0 * p_cf->Coupling(mu2,0,sub) * (1.-2.*z*(1.-z))
    + p_cf->Coupling(mu2,1,sub) * 0.5;
  return value * JII(z,y,eta,scale,sub);
}
