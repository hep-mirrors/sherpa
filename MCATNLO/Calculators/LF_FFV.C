...
(const double zmin,const double zmax,const double scale,const double xbj)
{
  m_zmin = zmin; m_zmax = zmax; 
  m_Jmax = m_flavs[0].Kfcode()<3?5.:1.;
  return (4.0*p_cf->MaxCoupling(0) + 0.5*p_cf->MaxCoupling(1)) * log((1.-zmin*zmax)/(1.-zmax)) * m_Jmax;
}

double LF_FFV_II::OverEstimated(const double z,const double y)
{
  return (4.0*p_cf->MaxCoupling(0) + 0.5*p_cf->MaxCoupling(1))/(1./m_zmax-z-y) * m_Jmax;
}
...
double LF_FVF_II::operator()
  (const double z,const double y,const double eta,
   const double scale,const double Q2,Cluster_Amplitude *const sub)
{
  double value = 2.0 * p_cf->Coupling(scale,0,sub) * ( 2./z - 2. +z )
