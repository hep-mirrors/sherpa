      ...
      if ((k->Id()&3)==0) {
        ...
        switch(p_shower->KinFF()->m_dipole_case){
          case EXTAMP::IDa:
          {
            ...
            Cluster_Leg *w;
            if(j->Flav().IsbQuark()) { w=ampl->Leg(2); }
            else if(j->Flav().IsbbarQuark()) { w=ampl->Leg(3); }
            else THROW(fatal_error, "Invalid clustering");
            const double mw2      = sqr(Flavour(24).Mass());
            const Vec4D pw        = w->Mom();
            const Vec4D pa        = j->Mom();
            const Vec4D pb        = k->Mom();
            const ATOOLS::Vec4D n = pw+pi;
            Q2                    = (pw+pi).Abs2();
            ff  = ClusterFFDipole(mw2,0.,mw2,0,pw,pi,pa,1|(kin?4:0));
            if (ff.m_stat!=1) return cs;
            const double zain    = pa*n/((pa+pi)*n);
//            const double zain2    = 1.-ff.m_y; // checked: agrees with previous line
            const double vi      = pa*pi/(pa*n);
//            const double vi2      = 1.-ff.m_z; // checked: agrees with previous line
            const double viab     = pa*pb / ((pa+pb)*pi);
            // checked: zain, vi, viab agree with parametes generated by Sudakov::Generate()
            // in following call, only last three arguments are relevant
            kt2=p_shower->KinFF()->GetKT2(Q2,ff.m_y,ff.m_z,mi2,mj2,mk2,mo,j->Flav(),pa,pb,pi);
            // kt2 agrees with m_kperp2 from Sudakov::Generate()

            // give zain and viab as arguments here, as they are retrieved
            // in CS_Gamma::SingleWeight() for calculation of splitting kernel
            // Q2 and phi have no influence
