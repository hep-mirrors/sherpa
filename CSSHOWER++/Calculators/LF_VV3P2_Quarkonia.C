...
double LF_VV3P2_Quarkonia_FF::operator()(
    ...
    double value = 2 * ( sqr(sij - sqr(M)) * (sqr(sij) + 6 * sqr(sqr(M))) - 2*z*((1-z)*sij - sqr(M))*sij*(sqr(sij) -6*sij*sqr(M) + 6*sqr(sqr(M))) );
    value /= sqr(sqr(( sij - sqr(M))));
    // msg_Out() << METHOD << " t=" << scale << ", M=" << M << ", mj= " << mj << ", z=" << z << ", value=" << value << std::endl;
    value *= sqr(m)/sij;
