...
class LF_FFS_Quarkonia_FF : public SF_Lorentz {
public:
  inline LF_FFS_Quarkonia_FF(const SF_Key &key) : SF_Lorentz(key) {}
...
class LF_FFS_Quarkonia_FI : public SF_Lorentz {

protected:
  double m_Jmax;

public:
  inline LF_FFS_Quarkonia_FI(const SF_Key &key) : SF_Lorentz(key) {}
...
class LF_FFS_Quarkonia_IF : public SF_Lorentz {

protected:
  double m_Jmax;

public:
  inline LF_FFS_Quarkonia_IF(const SF_Key &key) : SF_Lorentz(key) {}
...
class LF_FFS_Quarkonia_II : public SF_Lorentz {

protected:
  double m_Jmax;

public:
  inline LF_FFS_Quarkonia_II(const SF_Key &key) : SF_Lorentz(key) {}
...
double LF_FFS_Quarkonia_FF::operator()(const double z, const double y,
                                       const double eta, const double scale,
                                       const double Q2) {
  // assume we are doing q[0] -> qprim [1] Scalar[2]
  ...
  // same mass case (eta c eta b)
  if ( mi2 == mj2) {
    value = (sqr(mj2) * (z*z*z - 6*sqr(z) -23*z - 32 ) - 8*mj2*sij*(z*z*z + 2*sqr(z) + 9*z - 8) + 16*sqr(sij)*sqr(2-z)*z);
  }
...
double LF_FFS_Quarkonia_FF::OverIntegrated(const double zmin, const double zmax,
                                           const double scale,
                                           const double xbj) {
  const double mij = p_ms->Mass(m_flavs[0]); // mass incoming heavy quark
  const double mi  = p_ms->Mass(m_flavs[1]); // mass of escaping heavy quark
  double value(0.);
...
double LF_FFS_Quarkonia_FF::OverEstimated(const double z, const double y) {
  ...
  if ( m_flavs[0].Kfcode() == m_flavs[1].Kfcode()){
      ...
      value =  preF *   (32+321*z-438*sqr(z)+137*sqr(z)*z-16*sqr(sqr(z)))/9;
  }
  else {
    value = 0.0;
  }
...
double LF_FFS_Quarkonia_FF::Z() {
  return 1. - (1. - m_zmin) * pow((1. - m_zmax) / (1. - m_zmin), ATOOLS::ran->Get());
}

double LF_FFS_Quarkonia_FI::operator()(const double zz, const double y,
                                       const double eta, const double scale,
                                       const double Q2) {
  const double z = 1 - zz;
  double mi2 =
...
double LF_FFS_Quarkonia_FI::OverIntegrated(const double zmin, const double zmax,
                                           const double scale,
                                           const double xbj) {
  m_zmin = zmin;
  m_zmax = zmax;
...
double LF_FFS_Quarkonia_FI::OverEstimated(const double z, const double y) {
  return 16. / 27 * pow(0.82, 3) / Flavour(kf_c).Mass() *
         sqr(p_cf->MaxCoupling(0)) * (0.5) / (1. - z) * m_Jmax;
}

double LF_FFS_Quarkonia_FI::Z() {
  return 1. -
         (1. - m_zmin) * pow((1. - m_zmax) / (1. - m_zmin), ATOOLS::ran->Get());
}

double LF_FFS_Quarkonia_IF::operator()(const double zz, const double y,
                                       const double eta, const double scale,
                                       const double Q2) {
  // c --> c J/psi
  // (aj) - (k) --> a - j - k
...
double LF_FFS_Quarkonia_IF::OverIntegrated(const double zmin, const double zmax,
                                           const double scale,
                                           const double xbj) {
  m_zmin = zmin;
  m_zmax = zmax;
...
double LF_FFS_Quarkonia_IF::OverEstimated(const double z, const double y) {
  return 16. / 27 * pow(0.82, 3) / Flavour(kf_c).Mass() *
         (2.0 * p_cf->MaxCoupling(0) * 2. + 0.5 * p_cf->MaxCoupling(1)) /
         (1. - z) * m_Jmax;
}

double LF_FFS_Quarkonia_IF::Z() {
  return 1. -
         (1. - m_zmin) * pow((1. - m_zmax) / (1. - m_zmin), ATOOLS::ran->Get());
}

double LF_FFS_Quarkonia_II::operator()(const double zz, const double y,
                                       const double eta, const double scale,
                                       const double Q2) {
...
double LF_FFS_Quarkonia_II::OverIntegrated(const double zmin, const double zmax,
                                           const double scale,
                                           const double xbj) {
  m_zmin = zmin;
  m_zmax = zmax;
...
double LF_FFS_Quarkonia_II::OverEstimated(const double z, const double y) {
...
DECLARE_GETTER(LF_FFS_Quarkonia_FF, "FFS_Quarkonia", SF_Lorentz, SF_Key);

SF_Lorentz *ATOOLS::Getter<SF_Lorentz, SF_Key, LF_FFS_Quarkonia_FF>::operator()(
  ...
  if ((args.m_mode == 0 && args.p_v->in[0].IntSpin() == 1 &&
    ...
    switch (args.m_type) {
    case cstp::FF:
      return new LF_FFS_Quarkonia_FF(args);
    case cstp::FI:
      return new LF_FFS_Quarkonia_FI(args);
    case cstp::IF:
      return new LF_FFS_Quarkonia_IF(args);
    case cstp::II:
      return new LF_FFS_Quarkonia_II(args);
    case cstp::none:
      break;
    }
  }
...
void ATOOLS::Getter<SF_Lorentz, SF_Key, LF_FFS_Quarkonia_FF>::PrintInfo(
    std::ostream &str, const size_t width) const {
  str << "FFS lorentz functions";
}
