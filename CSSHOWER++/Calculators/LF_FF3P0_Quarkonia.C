...
double LF_FF3P0_Quarkonia_FF::operator()(const double z, const double y,
  ...
  value += sqr(cube(M))/cube(sij - sqr(rij*M)) * ( 8*ri*rij*cube(1-rij*(1-z)) * ( 1 - 18*ri + 14*sqr(ri) -2*rij*(1-z)*(1-2*ri+7*sqr(ri)) + sqr(rij*(1-z))*(1+2*ri) ) );
  value += sqr(sqr(M))/sqr(sij - sqr(rij*M)) * ( -sqr(1-rij*(1-z)) * (
...
double LF_FF3P0_Quarkonia_FI::operator()(const double zz, const double y,
                                       const double eta, const double scale,
                                       const double Q2) {
  const double z = 1 - zz;
  double mi2 =
...
double LF_FF3P0_Quarkonia_IF::operator()(const double zz, const double y,
                                       const double eta, const double scale,
                                       const double Q2) {
  // c --> c J/psi
  // (aj) - (k) --> a - j - k
...
double LF_FF3P0_Quarkonia_II::operator()(const double zz, const double y,
                                       const double eta, const double scale,
                                       const double Q2) {
...
SF_Lorentz *ATOOLS::Getter<SF_Lorentz, SF_Key, LF_FF3P0_Quarkonia_FF>::operator()(
  ...
  if ((args.m_mode == 0 && args.p_v->in[0].IntSpin() == 1 &&
    ...
    switch (args.m_type) {
    case cstp::FF:
      return new LF_FF3P0_Quarkonia_FF(args);
    // case cstp::FI:
    //   return new LF_FF3P0_Quarkonia_FI(args);
    // case cstp::IF:
    //   return new LF_FF3P0_Quarkonia_IF(args);
    // case cstp::II: 
    //   return new LF_FF3P0_Quarkonia_II(args);
    // case cstp::none:
    //   break;
    }
  }
