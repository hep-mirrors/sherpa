   ...
   const double scale,const double Q2)
{
  ...
  if (muij2==0. && mui2==0. && muk2==0.) {
    ...
    //the massive case
    double vtijk = Lambda(1.,muij2,muk2);
...
(const double zmin,const double zmax,const double scale,const double xbj)
{
  ...
  return (4.0*p_cf->MaxCoupling(0) +
	  0.5*p_cf->MaxCoupling(1)) *log((1.-zmin)/(1.-zmax));
}
...
(const double zmin,const double zmax,const double scale,const double xbj)
{
  ...
  return (2.0*p_cf->MaxCoupling(0)*(2.+CDISMax()) +
	  0.5*p_cf->MaxCoupling(1))*log((1.-zmin)/(1.-zmax)) * m_Jmax;
}

double LF_FFV_FI::OverEstimated(const double z,const double y)
{
  return (2.0*p_cf->MaxCoupling(0)*(2.+CDISMax()) +
	  0.5*p_cf->MaxCoupling(1))/(1.-z) * m_Jmax;
}
...
double LF_FFV_IF::operator()
  (const double z,const double y,const double eta,
   const double scale,const double Q2)
{
  double value = 2.0 * p_cf->Coupling(scale,0) *
    ( 2./(1.-z+y) - (1.+z) + CDIS(z,y) )
   ...
   const double scale,const double Q2)
{
  ...
  if (muij2==0. && muj2==0. && muk2==0.) {
    ...
    //the massive case
    double vtijk = Lambda(1.,muij2,muk2);
    double vijk  = sqr(2.*muk2+(1.-muj2-muk2)*(1.-y))-4.*muk2;
    if (vtijk<0.0 || vijk<0.0) return 0.0;
    vtijk = sqrt(vtijk)/(1.-muij2-muk2);
...
(const double zmin,const double zmax,const double scale,const double xbj)
{
  ...
  m_Jmax=m_flspec.Kfcode()<3?5.:1.;
  return (2.0*p_cf->MaxCoupling(0)*(2.+CDISMax()) +
	  0.5*p_cf->MaxCoupling(1)) * log(zmax/zmin) * m_Jmax;
}

double LF_FVF_FI::OverEstimated(const double z,const double y)
{
  return (2.0*p_cf->MaxCoupling(0)*(2.0+CDISMax()) +
	  0.5*p_cf->MaxCoupling(1))/z * m_Jmax;
}
...
(const double zmin,const double zmax,const double scale,const double xbj)
{
  ...
  if (fresh<0.0 || old<0.0 ||
      IsZero(old,s_pdfcut) || IsZero(fresh,s_pdfcut)) return 0.;
  m_Jmax = 5.*fresh/old;
  return (2.0*p_cf->MaxCoupling(0)*(2.+CDISMax()) +
	  0.5*p_cf->MaxCoupling(1)) * log(zmax/zmin) * m_Jmax;
}
   ...
   const double scale,const double Q2)
{
  ...
  if (mui2==0. && muj2==0. && muk2==0.) {
    ...
    double viji = sqr(fac*y)-4.*mui2*muj2;
    double vijk = sqr(2.*muk2+fac*(1.-y))-4.*muk2;
...
{
  m_zmin = zmin; m_zmax = zmax;
  return (2.0*p_cf->MaxCoupling(0) +
	  0.5*p_cf->MaxCoupling(1)) * (m_zmax-m_zmin);
}
 ...
 const double _scale,const double Q2) const
{
  ...
  double scale = ((Q2+p_ms->Mass2(m_flspec))*y/(1.0-y)+
		  2.0*p_ms->Mass2(m_flavs[1])/(1.0-y));
  return scale;
}
...
(const double zmin,const double zmax,const double scale,const double xbj)
{
  m_zmin = zmin; m_zmax = zmax;
  m_Jmax=m_flspec.Kfcode()<3?5.:1.;
  return (2.0*p_cf->MaxCoupling(0)*(1.+CDISMax()) +
	  0.5*p_cf->MaxCoupling(1))* (m_zmax-m_zmin) * m_Jmax;
}

double LF_VFF_FI::OverEstimated(const double z,const double y)
{
  return (2.0*p_cf->MaxCoupling(0)*(1.+CDISMax()) +
	  0.5*p_cf->MaxCoupling(1))* m_Jmax;
}
...
{
  double value = 2.0 * p_cf->Coupling(scale,0) *
    ( (1.-2.*z*(1.-z))*(1.0-0.5/z*CDIS(y,z)) + CDIS(z,y) )
