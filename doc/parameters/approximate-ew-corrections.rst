...
`EWSud.EWNLL` and `EWSud.ExpEWNLL`.
...
* :option:`THRESHOLDS` . Instead of the above :option:`THRESHOLD`, the option
  :option:`THRESHOLDS` can be used to define multiple thresholds, allowing
  for a study of the systematics associated with the definition of the
  threshold.
  Attempting to use both options simultaneously will abort the run.
  :option:`THRESHOLDS` takes a list of thresholds and will, when used
  as on-the-fly variation, generate `EWSud.EWNLL_Thr<val>` and
  `EWSud.ExpEWNLL_Thr<val>` as additional event weights.
  If the EWSud corrections are used as a KFactor, only
  the lowest threshold will be used for the correction that is directly
  applied to the nominal event weight.
  An example is
...
* :option:`WRITE_CONTRIBS` enables writing out the contribution of the
  various sources of logarithmic corrections to the event record. Default
  is `false`.
