...
namespace ALARIC {
  ...
  class Alaric: public PDF::Shower_Base {
  private:
  ...
  public:
    ...
    Alaric(const PDF::Shower_Key &key);
...
Alaric::Alaric(const Shower_Key &key):
  Shower_Base("Alaric"), p_ms(NULL),
  m_maxweight(1.0)
{
...
int Alaric::PerformShowers()
{
  DEBUG_FUNC(this);
  m_weightsmap.Clear();
  unsigned int nem=0;
  for (size_t i(0);i<m_ampls.size();++i) {
    if (m_ampls[i]->ClusterAmplitude()->NIn()==1 /*&&
						   i<m_ampls.size()-1*/) continue;
  ...
  if (m_wcheck && dabs(weight) > m_maxweight) {
    ...
    std::string rname="alaric.random."+rpa->gen.Variable("RNG_SEED")+".dat";
...
int Alaric::PerformDecayShowers()
{
  DEBUG_FUNC(this);
  return PerformShowers();
}

void Alaric::ExtractParton(Blob *const bl,Parton *const p)
{
  Particle *sp=p->Beam()?
...
Cluster_Definitions_Base *Alaric::GetClusterDefinitions()
{
  return p_clus;
}

Amplitude *Alaric::Convert
 ...
 std::map<Cluster_Leg*,Parton*> &lmap)
{
  ...
  for (size_t i(0);i<campl->Legs().size();++i) {
    ...
    p->SetRecMode(p_shower->Recoil()->Mode(campl,cl->Id()));
    ampl->push_back(p);
    p->SetId(p->Counter());
...
Shower_Base *Getter<Shower_Base,Shower_Key,Alaric>::
operator()(const Shower_Key &key) const
{
  return new Alaric(key);
}

void Getter<Shower_Base,Shower_Key,Alaric>::
