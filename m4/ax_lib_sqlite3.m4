...
[
    AC_ARG_WITH([sqlite3],
        AS_HELP_STRING(
            [--with-sqlite3=@<:@ARG@:>@],
            [use SQLite 3 library from @<:@ARG@:>@]
        ),
        [
        WANT_SQLITE3="yes";
        if test "$withval" = "install"; then
	    ac_sqlite3_path=$ac_default_prefix;
    	    test "x$prefix" != xNONE && ac_sqlite3_path=$prefix;
	    if ! test -f ${ac_sqlite3_path}/include/sqlite3.h; then
	       sqlite_version=sqlite-autoconf-3080200;
    	       wget http://www.sqlite.org/2013/${sqlite_version}.tar.gz;
    	       tar xzf ${sqlite_version}.tar.gz;
    	       cd ${sqlite_version};
    	       ./configure --prefix=${ac_sqlite3_path} --disable-static || exit;
    	       make || exit; make install || exit;
    	       cd ..;
    	       rm -rf ${sqlite_version}.tar.gz ${sqlite_version}/;
    	       echo "Successfully installed sqlite3 into ${ac_sqlite3_path}."
	    fi
        else
