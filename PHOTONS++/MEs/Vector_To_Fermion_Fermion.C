...
double Vector_To_Fermion_Fermion::SmodFull(unsigned int kk) {
...
double Vector_To_Fermion_Fermion::Smod(unsigned int a, unsigned int b, unsigned int c) {
  return 0.;
}
...
Complex Vector_To_Fermion_Fermion::InfraredSubtractedME_0_1(const int& ewmode) {
  //return 0.;
  m_moms = m_moms0;
  Vec4C epsV = Polarization_Vector(m_moms[0])[m_spins[0]];
  XYZFunc XYZ(3,m_moms,m_flavs,false);
  ...
  term1 *=((p1p2+0.5*m2)*(C_11(m2,m2,s,0.,m2,m2,mu2)+C_12(m2,m2,s,0.,m2,m2,mu2))
           +(D-2.)/4.*m2*(C_21(m2,m2,s,0.,m2,m2,mu2)+C_22(m2,m2,s,0.,m2,m2,mu2))
           +(D-2.)/2.*p1p2*C_23(m2,m2,s,0.,m2,m2,mu2)
...
Complex Vector_To_Fermion_Fermion::InfraredSubtractedME_1_05(unsigned int i, const double& xi) {
...
Complex Vector_To_Fermion_Fermion::InfraredSubtractedME_1_15(unsigned int i, const double& xi) {
  return 0.;
}
...
double Vector_To_Fermion_Fermion::GetBeta_0_1(const int& ewmode) {
  // limit mV >> mf
...
void Vector_To_Fermion_Fermion::Print_Info() {
  msg_Out() << " --------------------------- \n"
