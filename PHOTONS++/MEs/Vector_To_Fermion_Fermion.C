...
Vector_To_Fermion_Fermion::Vector_To_Fermion_Fermion(Particle_Vector_Vector pvv_zero, Particle_Vector_Vector pvv_one) : Dipole_FF(pvv_zero) {
...
Complex Vector_To_Fermion_Fermion::InfraredSubtractedME_0_0() {
  m_moms = m_moms0;
  Vec4C epsV = Polarization_Vector(m_moms[0]).at(m_spins[0]);
...
Complex Vector_To_Fermion_Fermion::InfraredSubtractedME_1_05(unsigned int i) {
  ...
  for (unsigned int s=0; s<=1; s++) { // spin of pseudo-particle in propagator representation
    r1 = r1 + XYZ.X(1,m_spins[1],epsP,4,s,1.,1.)*XYZ.X(4,s,epsV,2,m_spins[2],m_cL,m_cR);
    r2 = r2 + XYZ.X(1,m_spins[1],epsP,5,s,1.,1.)*XYZ.X(5,s,epsV,2,m_spins[2],m_cL,m_cR);
  }
  // emission off anti-fermion (b)
  for (unsigned int s=0; s<=1; s++) { // spin of pseudo-particle in propagator representation
    r3 = r3 + XYZ.X(1,m_spins[1],epsV,6,s,m_cL,m_cR)*XYZ.X(6,s,epsP,2,m_spins[2],1.,1.);
    r4 = r4 + XYZ.X(1,m_spins[1],epsV,7,s,m_cL,m_cR)*XYZ.X(7,s,epsP,2,m_spins[2],1.,1.);
  }
  // add prefactors
