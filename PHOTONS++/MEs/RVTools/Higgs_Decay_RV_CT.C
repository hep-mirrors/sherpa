...
Higgs_Decay_RV_CT::Higgs_Decay_RV_CT
...
void Higgs_Decay_RV_CT::Init_Coefficients()
{
  return;
}

DivArrC Higgs_Decay_RV_CT::H_Vertex_CT_L()
{  
  // Counterterm for H-decay vertex - note: no need to include ZA transition in QED, also contains
  ...
  return COne*(p_EW->dm(m_m2,0.,-1.,-0.5)/m_m
	       +(p_EW->dZfermR(m_m2,0.,-1.,-0.5)
		 +p_EW->dZfermL(m_m2,0.,-1.,-0.5)));
}

DivArrC Higgs_Decay_RV_CT::H_Vertex_CT_R()
{  
  // Counterterm for H-decay vertex - note: no need to include ZA transition in QED, also contains
  // fermion wavefunction counterterm for external fermions, no CT necessary for vertex besides
...
DivArrC Higgs_Decay_RV_CT::RV_Fermion_CT(const int& ME, const int& LR, const Vec4C& epsP)
{
  // Corrections due to internal CT insertions
  ...
  if (ME == 1) {
    if (LR == 0) {
      return COne*(-p_EW->dZfermL(m_m2,0.,-1.,-0.5)*(2.*(epsP*m_p1)/(m_s1k-m_m2)
						     -2.*(epsP*m_p2)/(m_s2k-m_m2))
		   +2.*m_m*p_EW->dm(m_m2,0.,-1.,-0.5)*(2.*(epsP*m_p1)/pow((m_s1k-m_m2),2.)
						       -2.*(epsP*m_p2)/pow((m_s2k-m_m2),2.)));
    }
    else if (LR == 1) {
      return COne*(-p_EW->dZfermL(m_m2,0.,-1.,-0.5)*(2.*(epsP*m_p1)/(m_s1k-m_m2)
						     -2.*(epsP*m_p2)/(m_s2k-m_m2))
  ...
  // ubar1 \slashed{m_epsP} P_i v2
    ...
    if (LR == 0) {
      return COne*p_EW->dm(m_m2,0.,-1.,-0.5)*(1./(m_s1k-m_m2)
					      +1./(m_s2k-m_m2));
    }
    else if (LR == 1) {
      return COne*p_EW->dm(m_m2,0.,-1.,-0.5)*(1./(m_s1k-m_m2)
  ...
  // ubar1 \slashed{m_epsP} \slashed{k} P_i v2
    ...
    if (LR == 0) {
      return COne*(-p_EW->dZfermL(m_m2,0.,-1.,-0.5)*(1./(m_s1k-m_m2)
						     +1./(m_s2k-m_m2))
		   +2.*m_m*p_EW->dm(m_m2,0.,-1.,-0.5)*(1./pow((m_s1k-m_m2),2.)
						       +1./pow((m_s2k-m_m2),2.)));
    }
    else if (LR == 1) {
      return COne*(-p_EW->dZfermL(m_m2,0.,-1.,-0.5)*(1./(m_s1k-m_m2)
...
DivArrC Higgs_Decay_RV_CT::RV_Vertex_CT(const int& ME, const int& LR, const Vec4C& epsP)
{
  // Vertex CT corrections
  // ME takes values 1..8, denoting m_standard MEs
  // LR takes values 0 for left-handed projection of ME, 1 for right-handed projection,  
  // ubar1 P_i v2
  if (ME == 1) {
    if (LR == 0) {
      return H_Vertex_CT_L()*(2.*(epsP*m_p1)/(m_s1k-m_m2)
			      -2.*(epsP*m_p2)/(m_s2k-m_m2));
    }
    else if (LR == 1) {
      return H_Vertex_CT_R()*(2.*(epsP*m_p1)/(m_s1k-m_m2)
			      -2.*(epsP*m_p2)/(m_s2k-m_m2));
    }
  }
  // ubar1 \slashed{k} P_i v2
  ...
  // ubar1 \slashed{m_epsP} \slashed{k} P_i v2
    ...
    if (LR == 0) {
      return H_Vertex_CT_L()*(1./(m_s1k-m_m2)
			      +1./(m_s2k-m_m2));
    }
    else if (LR == 1) {
      return H_Vertex_CT_R()*(1./(m_s1k-m_m2)
			      +1./(m_s2k-m_m2));
    }
  }
  else {
...
DivArrC Higgs_Decay_RV_CT::RV_B(const int& ME, const int& LR, const Vec4C& epsP)
{
  // Corrections due to IR form factor B 
  // ME takes values 1..8, denoting m_standard MEs
  // LR takes values 0 for left-handed projection of ME, 1 for right-handed projection,  
  // ubar1 P_i v2
  if (ME == 1) {
    if (LR == 0) {
      return B*(2.*(epsP*m_p1)/(m_s1k-m_m2)
		-2.*(epsP*m_p2)/(m_s2k-m_m2));
    }
    else if (LR == 1) {
      return B*(2.*(epsP*m_p1)/(m_s1k-m_m2)
