...
#ifdef PHOTONS_DEBUG
  msg_Tracking()<<m_chargedinparticles.size()<<" "<<m_neutralinparticles.size()
                <<" "<<m_chargedoutparticles.size()<<" "<<m_neutraloutparticles.size()<<endl;
#endif

  if ((m_chargedinparticles.size() == 0) && (m_chargedoutparticles.size() >= 2))
    m_dtype = Dipole_Type::ff;
  else if ((m_chargedinparticles.size() == 1) && (((m_chargedoutparticles.size() >= 1) && (m_neutraloutparticles.size() >= 1)) || (m_chargedoutparticles.size() >= 2)))
    m_dtype = Dipole_Type::fi;
  else if ((m_chargedinparticles.size() >= 2) && (m_chargedoutparticles.size() == 0))
...
#ifdef PHOTONS_DEBUG
  msg_Tracking()<<"momentum conservation in lab: "<<p_blob->CheckMomentumConservation()<<endl;
#endif
}
