...
double AMEGIC::Single_Process::Differential2() 
{ 
...
double AMEGIC::Single_Process::operator()(const ATOOLS::Vec4D* mom)
{
  ...
  return M2 * sqr(m_pol.Massless_Norm(m_nin+m_nout,&m_flavs.front(),p_BS));
}
...
void AMEGIC::Single_Process::FillAmplitudes(HELICITIES::Amplitude_Tensor* atensor,double sfactor)
{
