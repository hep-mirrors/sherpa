...
Process_Info::Process_Info(ATOOLS::Flavour *fl,Pol_Info *pl)
{
...
void Process_Info::AddSubList(int n,ATOOLS::Flavour* fl,Pol_Info* pl)
{
  if (m_sublist[0].size()>0) m_sublist[0].clear();
  for (int i=0;i<n;i++) m_sublist[0].push_back(new Process_Info(&fl[i],&pl[i]));
}
...
int Process_Info::GetDPOffset(int n)
{
  int m=0;
  return GetDPOffset(n,m);
}

int Process_Info::GetDPOffset(int &n,int &m)
{
  if (n<0) return 0;
...
Process_Info* Process_Info::GetDecay(int &n)
{
...
void Process_Info::GetFlavList(ATOOLS::Flavour* fl, int n)
{
...
Process_Info* Process_Info::GetSubProcess(int n)
{
  int dn=1;
...
int Process_Info::GetTotalFlavList(ATOOLS::Flavour* fl, int n)
{
...
Process_Info* Process_Info::FindDM(char c)
{
  if (p_pl) {
    if (p_pl->pol_type=='d' && p_pl->type[0]==c) return this;
  }
  for (size_t i=0;i<m_sublist[0].size();i++) {
    Process_Info* pi=m_sublist[0][i]->FindDM(c);
    if (pi) return pi;
  }
  return NULL;
}

bool Process_Info::CheckCompleteness()
{
  if (m_sublist[0].size()==0) {
...
void Process_Info::MergePointList(Point** plist,Point* np, int nin) 
{
  int nd=0;
  int ep=nin;
...
int Order_FVST::operator()(const Process_Info * a, const Process_Info * b) {
...
int Order_SVFT::operator()(const Process_Info * a, const Process_Info * b) {
  //    if "a < b" return 1  else 0;
  if (*a->p_fl==*b->p_fl && (a->m_sublist[0].size()>0 || b->m_sublist[0].size()>0)) {
    if (a->m_sublist[0].size()>b->m_sublist[0].size()) return 1;
    if (a->m_sublist[0].size()<b->m_sublist[0].size()) return 0;
...
int Order_Mass::operator()(const Process_Info * a, const Process_Info * b) {
  //    if "a > b" return 1  else 0;
  if (*a->p_fl==*b->p_fl && (a->m_sublist[0].size()>0 || b->m_sublist[0].size()>0)) {
    if (a->m_sublist[0].size()>b->m_sublist[0].size()) return 1;
    if (a->m_sublist[0].size()<b->m_sublist[0].size()) return 0;
...
int Order_InvMass::operator()(const Process_Info * a, const Process_Info * b) {
  //    if "a < b" return 1  else 0;
  if (*a->p_fl==*b->p_fl && (a->m_sublist[0].size()>0 || b->m_sublist[0].size()>0)) {
    if (a->m_sublist[0].size()>b->m_sublist[0].size()) return 1;
    if (a->m_sublist[0].size()<b->m_sublist[0].size()) return 0;
...
int Order_Kfc::operator()(const Process_Info * a, const Process_Info * b) {
  //    if "a < b" return 1  else 0;
  if (*a->p_fl==*b->p_fl && (a->m_sublist[0].size()>0 || b->m_sublist[0].size()>0)) {
    if (a->m_sublist[0].size()>b->m_sublist[0].size()) return 1;
    if (a->m_sublist[0].size()<b->m_sublist[0].size()) return 0;
...
int Order_Anti::operator()(const Process_Info * a, const Process_Info * b) {
  //    if "a < b" return 1  else 0;
  if (*a->p_fl==*b->p_fl && (a->m_sublist[0].size()>0 || b->m_sublist[0].size()>0)) {
    if (a->m_sublist[0].size()>b->m_sublist[0].size()) return 1;
    if (a->m_sublist[0].size()<b->m_sublist[0].size()) return 0;
...
int Order_Coupling::operator()(const Process_Info * a, const Process_Info * b) {
  if (*a->p_fl==*b->p_fl && (a->m_sublist[0].size()>0 || b->m_sublist[0].size()>0)) {
    if (a->m_sublist[0].size()>b->m_sublist[0].size()) return 1;
    if (a->m_sublist[0].size()<b->m_sublist[0].size()) return 0;
    for (size_t i=0;i<a->m_sublist[0].size();++i) {
...
string Process_Info::GenerateName()
{
  string name;
