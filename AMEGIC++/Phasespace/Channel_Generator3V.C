...
Channel_Generator3V::Channel_Generator3V(int _nin,int _nout,
...
Point* Channel_Generator3V::CopyPoints(Point* p)
{
  Point* ph = NULL; 
  if (p!=NULL) { 
    ph = new Point(*p);
...
Point* Channel_Generator3V::TransformTS(Point* p)
{
...
void Channel_Generator3V::MRPScan()
{}

int Channel_Generator3V::MakeChannel(int& echflag,int n,string& path,string& pID)
{  
...
//   cout<<name<<endl<<endl;
  ...
  chf	<<name<<"::"<<name<<"(int nin,int nout,Flavour* fl,Integration_Info * const info)"<<endl
	...
	<<"  m_thexp = reader.Get(\"AMEGIC_THRESHOLD_EPSILON\", 1.5);"<<endl; //TODO: initialise reader
  if (tcount>0) {
    chf	<<"  m_amct  = 1.0+reader.Get(\"AMEGIC_CHANNEL_EPSILON\", 0.0);"<<endl
        <<"  m_alpha = reader.Get(\"AMEGIC_TCHANNEL_ALPHA\", 0.9);"<<endl
        <<"  m_ctmax = 1.;"<<endl
	<<"  m_ctmin = -1.;"<<endl;
  }
  for (size_t i=0; i<m_idc.size();++i) {
...
void Channel_Generator3V::Step0(int flag,Point* p,int& rannum,ofstream& sf) 
{
  if (nin != 2) return;
  Point* ph = p->left;
  if (ph->left==0) {
...
void Channel_Generator3V::GenerateDecayChain(int flag,Point* p,int& rannum,ofstream& sf,
...
bool Channel_Generator3V::QCDAntenna(int flag,Point* p,int& rannum,ofstream& sf,int n)
{ 
...
void Channel_Generator3V::GenerateMassChain(int flag,Point* p,Point* clmp,int& rannum,ofstream& sf)
{
  if (p->left==0) {
...
void Channel_Generator3V::SetProps(Point* p,Point** props,Point** propt, int& count)
{
  if (p->left==0) return;
...
void Channel_Generator3V::CalcSmin(int flag,const char* min,string lm,ofstream& sf,Point* p)
{
...
string Channel_Generator3V::GetFlMass(Point* p)
{
...
string Channel_Generator3V::LinkedMasses(Point* p)
{
  if (p->left==0) {
    char help[4];
    sprintf(help,"%i",0);
...
void Channel_Generator3V::IdentifyProps(Point* _plist)
{
  InitT(_plist);
  Point* endp;
  tcount = 0;
...
void Channel_Generator3V::BackLinks(Point* p,Point* &endp)
{
...
string Channel_Generator3V::IString(int i)
{
  MyStrStream sstr;
  sstr<<i;
  string istr;
...
string Channel_Generator3V::Order(string s)
{
  int beg = s.find("_");
  if (beg!=-1) {
...
void  Channel_Generator3V::AddToVariables(int flag,const string& lhs,const string& rhs,const int& type,
					ofstream& sf)
{
  if (flag<0) return;
...
bool  Channel_Generator3V::CheckVariables(int flag,const string& lhs,const int& type)
{
...
int Channel_Generator3V::AntennaS(Point* p)
{
...
std::string Channel_Generator3V::CreateChannelID(int echflag)
{
