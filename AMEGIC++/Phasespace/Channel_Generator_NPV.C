...
Channel_Generator_NPV::Channel_Generator_NPV(int _nin,int _nout,
...
Point* Channel_Generator_NPV::CopyPoints(Point* p)
{
  Point* ph = NULL; 
  if (p!=NULL) { 
    ph = new Point(*p);
...
Point* Channel_Generator_NPV::TransformTS(Point* p)
{
...
Point* Channel_Generator_NPV::GetMirrorTopo(Point* p)
{
  Point* ph = NULL; 
  if (p!=NULL) { 
    ph = new Point(*p);
...
int Channel_Generator_NPV::MarkNP(Point* p)
{
...
void Channel_Generator_NPV::MRPScan()
{
...
int Channel_Generator_NPV::MakeChannel(int& echflag,int n,string& path,string& pID)
{  
  ...
  chf<<"// Channel_Generator_NPV"<<endl;
...
void Channel_Generator_NPV::Step0(int flag,Point* p,int& rannum,ofstream& sf) 
{
  if (nin != 2) return;
  Point* ph = p->left;
  if (ph->left==0) {
...
void Channel_Generator_NPV::GenerateDecayChain(int flag,Point* p,int& rannum,ofstream& sf,
...
bool Channel_Generator_NPV::QCDAntenna(int flag,Point* p,int& rannum,ofstream& sf,int n)
{ 
...
void Channel_Generator_NPV::GenerateMassChain(int flag,Point* p,Point* clmp,int& rannum,ofstream& sf)
{
  if (p->left==0) {
...
//   }
  ...
  if (p->m>0 && !IsZero(res) && Massive(p->fl)) maxpole = 1./res;
  ...
  default:
	...
	<<"s"<<mummy<<"_max,s"<<mummy<<",p_rans["<<rannum<<"]);"<<endl;
    }
    rannum++;
  }
...
void Channel_Generator_NPV::GenerateMassFwd(int flag,Point* p,int& rannum,ofstream& sf)
{
  if (p->left==0) {
    string m = LinkedMasses(p);
    AddToVariables(flag, m, string("p_ms[") + GetMassIndex(m) + string("]"), 0,
...
void Channel_Generator_NPV::SetProps(Point* p,Point** props,Point** propt, int& count)
{
  if (p->left==0) return;
...
string Channel_Generator_NPV::LinkedMasses(Point* p)
{
  if (p->left==0) {
    char help[4];
    snprintf(help,4,"%i",0);
...
void Channel_Generator_NPV::IdentifyProps(Point* _plist)
{
  InitT(_plist);
  Point* endp;
  tcount = 0;
...
void Channel_Generator_NPV::BackLinks(Point* p,Point* &endp)
{
...
void Channel_Generator_NPV::InitT(Point* p)
{
  p->t = 0;
  if (p->left==0) return;
  InitT(p->left);
...
string Channel_Generator_NPV::IString(int i)
{
  MyStrStream sstr;
  sstr<<i;
  string istr;
...
string Channel_Generator_NPV::Order(string s)
{
  int beg = s.find("_");
  if (beg!=-1) {
...
void  Channel_Generator_NPV::AddToVariables(int flag,const string& lhs,const string& rhs,const int& type,
					ofstream& sf)
{
  if (flag<0) return;
  ...
  if (cit==declarations.end()) {
    ...
    if (rhs != declarations[name]) {
      msg_Error()<<" ERROR in Channel_Generator_NPV::AddToVariables ()"<<endl;
      Abort();
    }
  }
}

bool  Channel_Generator_NPV::CheckVariables(int flag,const string& lhs,const int& type)
{
...
int Channel_Generator_NPV::AntennaS(Point* p)
{
...
std::string Channel_Generator_NPV::CreateChannelID(int echflag)
{
  m_idc.clear();
  ...
  if (echflag==0) idstr = string("CG0PR$")+idstr;
  if (echflag==1) idstr = string("CG0PL$")+idstr;
  m_idstr = idstr;
  return idstr;
}

double Channel_Generator_NPV::PMassSum(Point* p,vector<int>* kfs)
{
  if (!p->left) return 1.;
  double m = 0.;
