...
C     FIRST TRY EXCLUSIVE DECAY TO SINGLE PION
      IF (MW1.GT.(MZ1+AMPI).AND.MW1.LT.(MZ1+1.)) THEN
...
C
          IF (MWIW.GT.(MZJZ+AMUP+AMDN+PSGAP)) THEN
            IF (IW.EQ.1.AND.JZ.EQ.1.AND.(MW1.LT.(MZ1+1.))) THEN
...
C          wi --> nu_e + e + zj
          IF (MWIW.GT.(MZJZ+AME+PSGAP)) THEN
            IF (MWIW.LT.(AMW+MZJZ)) THEN
            TMP(1)=XIPM**2+YIPM**2
            TMP(2)=XIPM**2-YIPM**2
            TMP(3)=MWIW
...
C         Enter information for decay matrix element
          ...
          END IF
C          wi --> nu_mu + mu + zj
          IF (MWIW.GT.(MZJZ+AMMU+PSGAP)) THEN
            IF (MWIW.LT.(AMW+MZJZ)) THEN
            TMP(1)=XIPM**2+YIPM**2
            TMP(2)=XIPM**2-YIPM**2
            TMP(3)=MWIW
...
C          See above for other cases.
          ...
          END IF
          IF (MWIW.GT.(MZJZ+AMTAU+PSGAP)) THEN
            IF (MWIW.LT.(AMW+MZJZ)) THEN
            TMP(1)=XIPM**2+YIPM**2
            TMP(2)=XIPM**2-YIPM**2
            TMP(3)=MWIW
