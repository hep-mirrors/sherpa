...
#include "MODEL/DarkQCD/Calculators/CF_DarkQCD.H"
...
CF_DarkQCD::CF_DarkQCD(const SF_Key &key):
  SF_Coupling(key),
  m_maincplinfo(DarkQCD_Coupling_Info()), m_altcplinfo(DarkQCD_Coupling_Info()),
  m_altcplmax(),
  m_q(0.), m_k0sq(0.0), m_scvmode(0)
{
  auto s = Settings::GetMainSettings()["DARK_SECTOR"];
  SetNC(s["NC"].SetDefault(3).Get<double>());
  if (key.p_v->in[0].StrongCharge()==8 &&
      key.p_v->in[1].StrongCharge()==8 &&
      key.p_v->in[2].StrongCharge()==8) m_q=m_CA;
  else m_q=(key.p_v->in[0].StrongCharge()==8)?m_TR:m_CF;
...
bool CF_DarkQCD::SetCoupling(MODEL::Model_Base *md,
			 const double &k0sqi,const double &k0sqf,
			 const double &isfac,const double &fsfac)
{
  // obtain global variables
  auto pss = Settings::GetMainSettings()["SHOWER"];
  /// TODO: replace by actual dark alpha...
  MODEL::Running_AlphaS * cpl = (MODEL::Running_AlphaS *)(md->GetScalarFunction("alpha_Dark"));
  ...
  m_maincplinfo = DarkQCD_Coupling_Info(cpl, rsf);
  ...
  m_altcplinfo = DarkQCD_Coupling_Info();
  m_altcplmax.clear();

  return true;
}

void CF_DarkQCD::SetAlternativeUnderlyingCoupling(void *cpl, double sf)
{
  ...
  if (cpl == NULL) {
    ...
    m_altcplinfo = DarkQCD_Coupling_Info(as, sf);
...
double CF_DarkQCD::CplMax(MODEL::Running_AlphaS * as, double rsf) const
{
...
double CF_DarkQCD::Coupling(const double &scale,const int pol)
{
#ifdef DEBUG__AlphaS
  DEBUG_FUNC("pol="<<pol);
#endif
  if (pol!=0) return 0.0;
  DarkQCD_Coupling_Info cplinfo = CurrentCouplingInfo();
...
bool CF_DarkQCD::AllowSpec(const ATOOLS::Flavour &fl,const int mode)
{
...
double CF_DarkQCD::CplFac(const double &scale) const
{
...
DECLARE_CPL_GETTER(CF_DarkQCD_Getter);

SF_Coupling *CF_DarkQCD_Getter::operator()
  (const Parameter_Type &args) const
{
  return new CF_DarkQCD(args);
}

void CF_DarkQCD_Getter::PrintInfo
...
DECLARE_GETTER(CSSHOWER::CF_DarkQCD_Getter,"SF_DarkQCD_Fill",
	       void,CSSHOWER::SFC_Filler_Key);

void *ATOOLS::Getter<void,CSSHOWER::SFC_Filler_Key,CSSHOWER::CF_DarkQCD_Getter>::
...
void ATOOLS::Getter<void,SFC_Filler_Key,CF_DarkQCD_Getter>::
