...
def _fix_py2_syntax(ufo_dir):
    """Convert Python 2 syntax to Python 3 in a UFO model directory.

    Handles implicit relative imports and raise statements.
    Fallback for when 2to3 is not available (removed in Python 3.13+).
    """
    ufo_dir = Path(ufo_dir)
    local_modules = {f.stem for f in ufo_dir.glob('*.py') if f.name != '__init__.py'}

    for py_file in ufo_dir.glob('*.py'):
        ...
        for line in lines:
            ...
            if m and m.group(1) in local_modules:
                alias = m.group(2) or ''
                new_lines.append(f'{indent}from . import {m.group(1)}{alias}\n')
                continue

            # from X import ...
            m = re.match(r'^from\s+(\w+)\s+(import\s+.+)$', stripped)
            if m and m.group(1) in local_modules:
                new_lines.append(f'{indent}from .{m.group(1)} {m.group(2)}\n')
                continue

            # raise X, Y -> raise X(Y)
            m = re.match(r'^raise\s+(\w+)\s*,\s*(.+)$', stripped)
            if m:
                new_lines.append(f'{indent}raise {m.group(1)}({m.group(2)})\n')
                continue
...
def try_import(ufo_path, auto_convert=False):
    pkg_name = ufo_path.parts[-2]
    parent_dir = str(ufo_path.parent.parent)
    if parent_dir not in sys.path:
        sys.path.insert(0, parent_dir)

    try:
        fix_param_writer(ufo_path.parent)
        ufo_mod = importlib.import_module(pkg_name)
        ...
        if response.lower()[0] == 'y':
            model_name = pkg_name + '_py3'
            new_path = ufo_path.parent.parent / model_name
            print(f'Writing python3 model to {new_path}')
            # Try 2to3 first, fall back to manual import fix
            converted = False
            try:
                result = subprocess.run(
                    ['2to3', '-W', ufo_path.parent.as_posix(),
            ...
            if not converted:
                import shutil
                warning('2to3 not available, falling back to manual import conversion')
                if new_path.exists():
                    shutil.rmtree(new_path)
                shutil.copytree(ufo_path.parent, new_path)
                _fix_py2_syntax(new_path)
