...
double Width_Calculator::Width(Decay_Channel * dec) {
  ...
  if (dec->GetDecaying().IsScalar()) {
    ...
    case 2:
      if (dec->GetDecayProduct(0).IsFermion() &&
	  dec->GetDecayProduct(1).IsVector()) return FFV(dec);

      if (dec->GetDecayProduct(1).IsFermion() &&
	  dec->GetDecayProduct(0).IsVector()) return FVF(dec);
      
      if (dec->GetDecayProduct(0).IsFermion() &&
	  dec->GetDecayProduct(1).IsScalar())return FFS(dec);
      
      if (dec->GetDecayProduct(1).IsFermion() &&
	  dec->GetDecayProduct(0).IsScalar()) return FSF(dec); 
    case 3:
    default:
      break;
    }
...
double Width_Calculator::FSF(Decay_Channel * dec) {
  Single_Vertex * vertex(FindVertex(dec->GetDecaying(),
				    dec->GetDecayProduct(0),dec->GetDecayProduct(1)));
  if (vertex==NULL) return 0.;
...
double Width_Calculator::FVF(Decay_Channel * dec) {
  Single_Vertex * vertex(FindVertex(dec->GetDecaying(),
				    dec->GetDecayProduct(0),dec->GetDecayProduct(1)));
  if (vertex==NULL) return 0.;
  ...
  double ME2   = (M2+m02-m12 +(M2 -m02-m12)*(M2-m02+m12)/m12)*
                 (std::norm(vertex->cpl[0].Value())+std::norm(vertex->cpl[1].Value()))+
                 6.*m_M*m_m[1]*(2.*std::abs(vertex->cpl[0].Value()*vertex->cpl[1].Value())) ;
