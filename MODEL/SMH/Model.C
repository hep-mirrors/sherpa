...
DECLARE_GETTER(SMH,"SMH",Model_Base,Model_Arguments);

Model_Base *Getter<Model_Base,Model_Arguments,SMH>::
operator()(const Model_Arguments &args) const
{
  return new SMH();
}

void Getter<Model_Base,Model_Arguments,SMH>::
...
void SMH::RegisterDefaults() const
{
...
bool SMH::ModelInit(const PDF::ISR_Handler_Map& isr)
{
  Settings& s = Settings::GetMainSettings();
  FixEWParameters();  
  FixCKM();
...
void SMH::InitEFTVertices()
{
  ...
  m_v.back().Lorentz.push_back("HGG");
  ...
  for (size_t i(1);i<6;++i) {
    Flavour flav(i);
    if (flav.IsMassive()) continue;
    m_v.push_back(Single_Vertex());
    m_v.back().AddParticle(Flavour(kf_h0));
    m_v.back().AddParticle(flav.Bar());
    m_v.back().AddParticle(flav);
    m_v.back().AddParticle(Flavour(kf_gluon));
    m_v.back().Color.push_back(Color_Function(cf::T,4,3,2));
    m_v.back().Lorentz.push_back("HQQG");
    m_v.back().cpl.push_back(ghgg*g3);
    m_v.back().order[0]=3;
    m_v.back().order.push_back(2);
  }
...
size_t SMH::IndexOfOrderKey(const std::string& key) const
{
  if (key == "SMH")
    return 2;
  else
