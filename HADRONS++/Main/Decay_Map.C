...
void Decay_Map::ReadInConstants()
{
...
void Decay_Map::SetHadronProperties()
{
...
void Decay_Map::Read()
{
  ...
  for (size_t i=0;i<Decayers.size();++i) {
    ...
    if( line[0] == string("CREATE_BOOKLET") ) {
      ...
      FlDtVMap::iterator it = find(decayerflav);
      if (it==end()) {
        insert(make_pair(decayerflav, vector<Hadron_Decay_Table*>(1,dt)));
      }
      else {
...
void Decay_Map::Initialise()
{
  for (FlDtVMap::iterator pos = this->begin(); pos != this->end(); ++pos) {
    for(size_t i=0; i<pos->second.size(); i++) {
      Hadron_Decay_Table * dt(pos->second[i]);
...
void Decay_Map::CreateBooklet()
{
  ...
  for ( FlDtVMap::iterator pos = this->begin(); pos != this->end(); ++pos) {
    Hadron_Decay_Table * dt  (pos->second[0]);
    if(dt==NULL) continue;
    ...
    for( int i=0; i<dt->NumberOfDecayChannels(); ++i ) {
      dc = (Hadron_Decay_Channel*) dt->GetDecayChannel(i);
...
std::vector<BRPairFlavourSet> Decay_Map::GetInclusives(
  ...
  for( int I=0; I<dt->NumberOfDecayChannels(); ++I ) {
    ...
    for (FlSetConstIter fl=outs.begin();fl!=outs.end();++fl) {
      if (this->find((*fl))==this->end() ||
        ...
        for( size_t i=0; i<new_flset.size(); ++i ) {
          dauchans.push_back( GetInclusives( (*this)[(*fl)][0], brmap, new_flset[i].first, new_flset[i].second, 0 ) );
        }
        new_flset.clear();
