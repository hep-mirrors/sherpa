...
SimpleResonanceFlavour::SimpleResonanceFlavour( std::string name, double _mass,
  ...
  m_gamma(0.), m_delta(0.), m_phi1(0.), m_phi2(0.), m_massfactor(1.)
{
}
...
{
  if( m_running )  return Complex(m_mass2, 0.)/Complex( m_mass2-s, -1.*OffShellMassWidth(s) );
  return Complex(m_mass2,0.)/Complex( m_mass2-s, -m_mass * m_width);
}
 
Complex ResonanceFlavour::BreitWignerPhases( double s )
{
  double sreal = s*(m_gamma*cos(m_phi1) + m_delta*cos(m_phi2));
  double simag = s*(m_gamma*sin(m_phi1) + m_delta*sin(m_phi2));
  if( m_running ) {
    double MG = OffShellMassWidth(s);
