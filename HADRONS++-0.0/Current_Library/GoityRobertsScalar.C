#include "GoityRobertsScalar.H"

using namespace HADRONS;
using namespace ATOOLS;


void GoityRobertsScalar::SetModelParameters( struct GeneralModel _md )
{
  m_Vcb = _md("Vcb",0.04);
}


void GoityRobertsScalar::Calc()
{
  Vec4D pB = p_moms[0];
  Vec4D pD = p_moms[1];
  Vec4D pPi= p_moms[2];
...
// Sum
  ...
  p_results[0] = m_Vcb * sqrt(mB*mD) * (Complex(0.0,1.0)*h*mB*mD*cross(vB,vD,pPi)+
					a1*pPi+a2*mB*vB+a3*mD*vD);
}


DECLARE_GETTER(GoityRobertsScalar_Getter, "GoityRobertsScalar",
               Current_Base,Flavour_Info);

Current_Base* GoityRobertsScalar_Getter::operator()(const Flavour_Info &parameters) const
{
  return new GoityRobertsScalar(parameters.flavs, parameters.nout, parameters.indices, "GoityRobertsScalar");
}

void GoityRobertsScalar_Getter::
