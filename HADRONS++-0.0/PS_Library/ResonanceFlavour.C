...
SimpleResonanceFlavour::SimpleResonanceFlavour( std::string name, double _mass, double _width )
{
  m_name = name;
  m_mass = _mass;
  m_mass2 = sqr(_mass);
...
ResonanceFlavour::ResonanceFlavour( kf::code _kfc, double _mass, double _width, int _run, string path )
...
void ResonanceFlavour::InitialiseThreeBodyResonance( ResonanceFlavour &res1, ResonanceFlavour &res2, double beta )
{
  m_body=3;
  if( m_running ) {
    p_hist = CreateGHistogram( res1, res2, beta, kf::pi_plus );
    m_G_at_m2 = GetValueOfG(m_mass2);
  }
}


Histogram * ResonanceFlavour::CreateGHistogram( ResonanceFlavour res1, ResonanceFlavour res2, double beta, kf::code out )
{
  // create file name
  char fn[512];
  ...
  if( !f ) {                            // if file does not exist
    ...
    Histogram myHist = Histogram( 0, low, up+step, nbins+1 );
    double q2  (low), phi (0.);
    while( q2<=up+step ) {
    ...
    myHist.Output(string(fn));         // write into file
  }
  ifstream g( fn );
  if (g) {                          // if file exists
...
double ResonanceFlavour::IntegralG( double Q2, ResonanceFlavour res1, ResonanceFlavour res2, double beta, kf::code out )
{
...
double ResonanceFlavour::OffShellMassWidth( double s )
{
  ...
  switch( m_kfc ) {
    ...
    case kf::rho_1700:
    case kf::f_0_980:
      return TwoBodyResonanceMassWidth(s,Flavour(kf::pi_plus).PSMass());
    case kf::rho_770_plus:
    case kf::rho_1450_plus:
    case kf::rho_1700_plus:
      return TwoBodyResonanceMassWidth(s,Flavour(kf::pi).PSMass(), Flavour(kf::pi_plus).PSMass());
    case kf::K_star_892:
    case kf::K_star_1410:
    case kf::K_star_1680:
    case kf::K_0_star_1430:
      return TwoBodyResonanceMassWidth(s,Flavour(kf::pi_plus).PSMass(), Flavour(kf::K_plus).PSMass());
    case kf::K_star_892_plus:
    case kf::K_star_1410_plus:
    case kf::K_star_1680_plus:
    case kf::K_0_star_1430_plus:
      return TwoBodyResonanceMassWidth(s,Flavour(kf::pi).PSMass(), Flavour(kf::K_plus).PSMass());
  }
  msg.Error()<<"WARNING in ResonanceFlavour::OffShellMassWidth(double s) : "<<endl
...
double ResonanceFlavour::ThreeBodyResonanceMassWidth( double s )
{
  if( p_hist ) {
    ...
    msg.Error()<<"ERROR in ResonanceFlavour::ThreeBodyResonanceMassWidth() : "<<endl
