...
namespace ANALYSIS {
  ...
  public:
    ...
    (const std::string &inlist,const std::string &reflist,
     ...
     const Int_Matrix &items,const Variable_Vector &vars,
  ...
  (const Argument_Matrix &parameters) const
{
  ...
  for (size_t i=0;i<parameters.size();++i) {
	...
	items.push_back(cit);
      ...
      for (size_t j(1);j<cur.size();++j) 
	vars.push_back(ATOOLS::Variable_Getter::GetObject(cur[j],""));
    }
	...
	maxs.push_back(ToType<double>(cur[j]));
      ...
      for (size_t j(1);j<cur.size();++j) 
	histos[0].push_back(ToType<double>(cur[j]));
    }
  ...
  for (size_t i(vars.size());i<max;++i) vars.push_back(vars.back());
  ...
  return new One_Variable_Selector
    (inlist,reflist,outlist,flavs,items,vars,mins,maxs,histos,parameters());
}
...
(const std::string &inlist,const std::string &reflist,
 ...
 const Double_Matrix &histos,Primitive_Analysis *const ana):
  ...
  m_flavs(flavs), m_items(items), m_vars(vars), 
...
{
  msg_Debugging()<<METHOD<<"(): {\n";
  std::string bname(pname+"/"+m_reflist);
...
{
  return new One_Variable_Selector
    (m_inlist,m_reflist,m_outlist,m_flavs,m_items,m_vars,
     m_mins,m_maxs,m_histos,p_ana);
}
