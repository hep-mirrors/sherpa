...
namespace ANALYSIS {
  ...
  class Four_Particle_Calculator_Base: public Primitive_Observable_Base {  
  protected:
...
(const ATOOLS::Particle_List &plist,double weight,int ncount)
{
  ...
  Calculate(inlist[pos[0]],inlist[pos[1]],inlist[pos[2]],inlist[pos[3]]);
}
...
Primitive_Observable_Base *Intermediate_Mass_Fitter::Copy() const
{
...
Primitive_Observable_Base *Delta_Phi_Fitter::Copy() const
{
