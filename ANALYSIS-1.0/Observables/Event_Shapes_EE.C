...
Event_Shapes_EE_Getter::operator()(const String_Matrix &parameters,
...
void Event_Shapes_EE::Evaluate(const Blob_List & blobs,double value,int ncount)
{
  Particle_List * pl=p_ana->GetParticleList(m_inlistname);
  Select(*pl,value,ncount);
}

void Event_Shapes_EE::Evaluate(const Particle_List & pl,double value,int ncount)
{
  Select(pl,value,ncount);
}

void Event_Shapes_EE::Select(const Particle_List & pl_in,double value,int ncount)
{
  Vec3D mom;
