...
namespace ANALYSIS {
  ...
  class Booster : public Primitive_Observable_Base {
  private:
  ...
  class Rotator : public Primitive_Observable_Base {
  private:
    std::string  m_reflist, m_outlist;
    std::vector<Flavour> m_flavs;
    std::vector<int>     m_items;
...
Booster::Booster(const std::string &inlist,
		 ...
		 const std::vector<Flavour> &flavs,
		 const std::vector<int> &items):
  m_reflist(reflist), m_outlist(outlist), m_flavs(flavs), m_items(items)
{
  m_splitt_flag = false;
  m_listname = inlist;
}
...
Primitive_Observable_Base * Booster::Copy() const
{
  return new Booster(m_listname,m_reflist,m_outlist,m_flavs,m_items);
}
...
Primitive_Observable_Base * Rotator::Copy() const
{
  return new Rotator(m_listname,m_reflist,m_outlist,m_flavs,m_items);
}
