  ...
  p_generator(NULL)
{
  ...
  case run_mode::xsecs_only:
    msg_Events()<<"Run Sinic to generate cross sections only."<<std::endl;
    GenerateXsecs();
    exit(1);
...
void Sinic::InitialiseFormFactors() {
...
void Sinic::InitialiseSingleChannelEikonals(const double & Ecms) 
{
  MBpars.Set(std::string("originalY"),
	     log(Ecms/ATOOLS::Flavour(kf_p_plus).HadMass()));
...
void Sinic::InitialiseCrossSections(const double & energy) {
  m_cross = Cross_Sections(&m_eikonals,energy,m_test);
  m_cross.CalculateTotalCrossSections();
}

void Sinic::InitialiseBeamRemnants(BEAM::Beam_Spectra_Handler * const beam,
				   PDF::ISR_Handler *const isr) { 
...
ATOOLS::Return_Value::code Sinic::FillBeamBlobs(ATOOLS::Blob_List * blobs) {
  msg_Tracking()
...
void Sinic::GenerateXsecs() {
  ...
  for (std::list<double>::iterator energy=Energies.begin();
    ...
    if (Elastics.find((*energy))!=Elastics.end()) {
      ...
      m_cross.GetSigmaElastic()->PrintDifferentialelasticXsec(false,dirname);
