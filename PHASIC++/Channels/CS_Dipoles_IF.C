...
IF_Dipole::IF_Dipole(const ATOOLS::Cluster_Leg &lij,
                     const ATOOLS::Cluster_Leg &lk,
                     const ATOOLS::Cluster_Leg &li,
                     const ATOOLS::Cluster_Leg &lj) :
  CS_Dipole(lij,lk,li,lj), m_xmode(1), m_umode(1), m_exp(2)
{
  // read in y,z mode
  Data_Reader read(" ",";","!","=");
  read.SetInputFile("Integration.dat");
  std::string xmode,umode;
  if (!read.ReadFromFile(xmode,"EEG_IF_X_DICING")) xmode="logarithmic";
//   else msg_Info()<<METHOD<<"(): Set x mode in IF dipole '"<<m_id
//                  <<"' to "<<xmode<<".\n";
  if (!read.ReadFromFile(umode,"EEG_IF_U_DICING")) umode="linear";
//   else msg_Info()<<METHOD<<"(): Set u mode in IF dipole '"<<m_id
//                  <<"' to "<<umode<<".\n";
  if      (xmode=="linear")      m_xmode=0;
  else if (xmode=="logarithmic") m_xmode=1;
  else if (xmode=="quadratic")   m_xmode=2;
  else THROW(fatal_error,"unknown EEG_IF_X_DICING mode");
  if      (umode=="linear")      m_umode=0;
  else if (umode=="power")       m_umode=1;
  else if (umode=="exponential") m_umode=2;
  else THROW(fatal_error,"unknown EEG_IF_U_DICING mode");

  if (m_umode==2) THROW(not_implemented,"not implemented yet");
  THROW(not_implemented,"IF dipole splitting for the EEG not implemented yet");
}

IF_Dipole::~IF_Dipole() {}

CS_Dipole *IF_Dipole::Copy() const
{
  return new IF_Dipole(*this);
}

bool IF_Dipole::GeneratePoint
...
//       m_rn[1]=(1.-pow(-2.*rn[1]+1.,1./(2.*m_exp+1.)))/2.;
//   // exponential
//   else if (m_umode==2) m_rn[1]=rn[1];
//   else THROW(fatal_error,"m_umode changed to undefined value");
// 
// 
//   // phi
//   // linear f(x)~1
...
double IF_Dipole::GenerateWeight(const Cluster_Amplitude *ampl)
{
