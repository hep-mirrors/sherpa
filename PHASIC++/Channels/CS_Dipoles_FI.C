...
FI_Dipole::FI_Dipole(const ATOOLS::Cluster_Leg &lij,
                     const ATOOLS::Cluster_Leg &lk,
                     const ATOOLS::Cluster_Leg &li,
                     const ATOOLS::Cluster_Leg &lj) :
  CS_Dipole(lij,lk,li,lj), m_xmode(1), m_zmode(1), m_exp(2)
{
  // read in y,z mode
  Data_Reader read(" ",";","!","=");
  read.SetInputFile("Integration.dat");
  std::string xmode,zmode;
  if (!read.ReadFromFile(xmode,"EEG_FI_X_DICING")) xmode="logarithmic";
//   else msg_Info()<<METHOD<<"(): Set x mode in FI dipole '"<<m_id
//                  <<"' to "<<xmode<<".\n";
  if (!read.ReadFromFile(zmode,"EEG_FI_Z_DICING")) zmode="linear";
//   else msg_Info()<<METHOD<<"(): Set z mode in FI dipole '"<<m_id
//                  <<"' to "<<zmode<<".\n";
  if      (xmode=="linear")      m_xmode=0;
  else if (xmode=="logarithmic") m_xmode=1;
  else if (xmode=="quadratic")   m_xmode=2;
  else THROW(fatal_error,"unknown EEG_FI_X_DICING mode");
  if      (zmode=="linear")      m_zmode=0;
  else if (zmode=="power")       m_zmode=1;
  else if (zmode=="exponential") m_zmode=2;
  else THROW(fatal_error,"unknown EEG_FI_Z_DICING mode");

  if (m_zmode==2) THROW(not_implemented,"not implemented yet");
  THROW(not_implemented,"FI dipole splitting for the EEG not implemented yet");
}

FI_Dipole::~FI_Dipole() {}

CS_Dipole *FI_Dipole::Copy() const
{
  return new FI_Dipole(*this);
}

bool FI_Dipole::GeneratePoint
...
double FI_Dipole::GenerateWeight(const Cluster_Amplitude *ampl)
{
