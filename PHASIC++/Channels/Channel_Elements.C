					    ...
					    double ctmin,double ctmax)
{
  ...
  double wt = 1./(M_PI*Channel_Basics::SqLam(s,s1,s2)/4.*pow(a+ct,ctexp)
                  			       ...
                  			       const Vec4D& q,const Vec4D& p1)
{
  ...
  return 1./(-2.*M_PI*pow(amct,ctexp)*Channel_Basics::Hj1(ctexp,ctkin-ctmin,
                                                                ctkin-ctmax));
}
                                            ...
                                            double smax,double s)
{
  ...
  double wt = 1./(pow(s,sexp)*Channel_Basics::PeakedWeight(0.,sexp,
                                                           smin,smax,1));
  if (!(wt>0) && !(wt<0) && wt!=0) { 
				       ...
				       const double a,double &ran)
{
  ...
  double wt = 1./(a*(1.-a)*Channel_Basics::BoundaryPeakedWeight(amin,amax,
                                                                a,ran));
  if (!(wt>0) && !(wt<0) && wt!=0) { 
                                         ...
                                         double s)
{
  ...
  double wt = s/((sqr(s)+pow(mass,4.)) * 
                 Channel_Basics::PeakedWeight(pow(mass,4.),1.,
                                              sqr(smin),sqr(smax),1)/2.);
...
double Channel_Elements::ThresholdMomenta(double mass,double smin,double smax,
                                          double ran)
{
  double s = sqrt(Channel_Basics::PeakedDist(pow(mass,4.),1.,
                                             sqr(smin),sqr(smax),1,ran));
  if (!(s>0) && !(s<0) && s!=0) msg_Error()<<"ThresholdMomenta produced a nan !"
                                           <<endl;
  if ((s<smin) || (s>smax))     msg_Error()<<"ThresholdMomenta out of bounds !"
                                           <<endl;
  return s;
}

double Channel_Elements::ThresholdWeight(double sexp,double mass,
...
//                  Channel_Basics::PeakedWeight(pow(mass,4.),sexp,
//                                               sqr(smin),sqr(smax),1)/2.);
  double wt = s/(pow(sqr(s)+pow(mass,4.),0.5*(sexp+1.)) * 
...
double Channel_Elements::ThresholdMomenta(double sexp,double mass,
                                          double smin,double smax,double ran)
{
  if (smin>smax) return smax;
  double sgmin=sqrt(sqr(smin)+sqr(sqr(mass)));
  double sgmax=sqrt(sqr(smax)+sqr(sqr(mass)));
  double s = sqrt(sqr(Channel_Basics::PeakedDist(0.,sexp,sgmin,sgmax,1,ran))
                  -sqr(sqr(mass)));
  if (!(s>0) && !(s<0) && s!=0) {
    msg_Error()<<"ThresholdMomenta produced a nan !"<<endl;
    msg_Error()<<"Channel_Elements::ThresholdMomenta "<<sexp<<" "<<mass<<" "
...
{
  if ((s<=smin) && (s>=smax)) return 0;
  double wt = 1./(pow(pole-s,sexp)*Channel_Basics::PeakedWeight(pole,sexp,
                                                                smin,smax,-1));
					...
					double &ran1,double &ran2)
{
  ...
  if (ct<ctmin||ct>ctmax) {
    ran1=ran2=-1.;
    msg_Error()<<"TChannelWeight: bad momenta!!!! "
               <<ctmin<<" - "<<ctmax<<" ("<<ct<<")"<<endl;
  ...
  double wt = 2.*sabs/(-pow(aminct,ctexp)
                       *Channel_Basics::Hj1(ctexp,a-ctmin,a-ctmax)
                       *p1outmass*M_PI);
...
int Channel_Elements::TChannelMomenta(Vec4D p1in,Vec4D p2in,
				      Vec4D &p1out,Vec4D &p2out,
					  ...
					  const double ran,const int mode) const
{
  /*!
    ...
    \begin{align}
      \frac{1}{2}\log\frac{x_{1, min}^2}{\tau}
        \le y \le \frac{1}{2}\frac{x_{1, max}^2}{\tau}
      \frac{1}{2}\log\frac{\tau}{x_{2, max}^2}
        \le y \le \frac{1}{2}\frac{\tau}{x_{2, min}^2}
    \end{align}
    where $x_{1/2, max}$ stem from the corresponding Base or
    the hard process respectively and
  ...
  */
  ...
  if (y<ymin || y>ymax) {
    msg_Error()<<"Channel_Elements::GenerateYUniform("<<tau<<","<<xinfo<<","
               <<yinfo<<"): "<<" Y out of bounds ! "<<endl
               <<"   ymin, ymax vs. y : "<<ymin<<" "<<ymax<<" vs. "<<y<<endl;
...
double Channel_Elements::GenerateYForward(const double yexponent,
					  const double tau,
					  const Double_Container &xinfo,
					  const Double_Container &yinfo,
					  const double ran,const int mode) const
					...
					double& ran,const int mode) const
{
  ...
  double wt = Channel_Basics::PeakedWeight(ypeak,yexponent,
                                           ymin,ymax,yinfo[2],-1,ran)
              *pow(ypeak-yinfo[2],yexponent);
  if (!(wt>0) && !(wt<0) && wt!=0) {
    msg_Error()<<"WeightYForward produces a nan!"<<endl
               <<ymax<<" "<<ymin<<" "<<yexponent<<" "
               <<yinfo[2]<<" "<<xinfo[3]<<endl;
  }
  return wt;
}

double Channel_Elements::GenerateYBackward(const double yexponent,
					   const double tau,
					   const Double_Container &xinfo,
					   const Double_Container &yinfo,
					   const double ran,
					   const int mode) const
{
  ...
  ymin=ATOOLS::Max(yinfo[0],ymin);
  ymax=ATOOLS::Min(yinfo[1],ymax);
  double y=-Channel_Basics::PeakedDist(-ymin-xinfo[1],yexponent,
                                       -ymax,-ymin,-1,ran);
  if (ATOOLS::IsZero(y)) y=0.;
  if (y<ymin || y>ymax){ 
					 ...
					 double& ran,const int mode) const
{
  ...
  ymax=ATOOLS::Min(yinfo[1],ymax);
  if (yinfo[2]<ymin || yinfo[2]>ymax) return 0.0;
  double wt = Channel_Basics::PeakedWeight(-ymin-xinfo[1],yexponent,
                                           -ymax,-ymin,-yinfo[2],-1,ran)
