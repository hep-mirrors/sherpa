...
FF_Dipole::FF_Dipole(const ATOOLS::Cluster_Leg &lij,
                     const ATOOLS::Cluster_Leg &lk,
		     const ATOOLS::Cluster_Leg &li,
		     const ATOOLS::Cluster_Leg &lj) :
  CS_Dipole(lij,lk,li,lj), m_ymode(1), m_zmode(1), m_exp(2)
{
  // read in y,z mode
  Data_Reader read(" ",";","!","=");
  read.SetInputFile("Integration.dat");
  std::string ymode,zmode;
  if (!read.ReadFromFile(ymode,"EEG_FF_Y_DICING")) ymode="logarithmic";
//   else msg_Info()<<METHOD<<"(): Set y mode in FF dipole '"<<m_id
//                  <<"' to "<<ymode<<".\n";
  if (!read.ReadFromFile(zmode,"EEG_FF_Z_DICING")) zmode="linear";
//   else msg_Info()<<METHOD<<"(): Set z mode in FF dipole '"<<m_id
//                  <<"' to "<<zmode<<".\n";
  if      (ymode=="linear")      m_ymode=0;
  else if (ymode=="logarithmic") m_ymode=1;
  else if (ymode=="quadratic")   m_ymode=2;
  else THROW(fatal_error,"unknown EEG_FF_Y_DICING mode");
  if      (zmode=="linear")      m_zmode=0;
  else if (zmode=="power")       m_zmode=1;
  else if (zmode=="exponential") m_zmode=2;
  else THROW(fatal_error,"unknown EEG_FF_Z_DICING mode");

  if (m_zmode==2) THROW(not_implemented,"not implemented yet");
}

FF_Dipole::~FF_Dipole() {}

CS_Dipole *FF_Dipole::Copy() const
{
  return new FF_Dipole(*this);
}

bool FF_Dipole::GeneratePoint
(Cluster_Amplitude *const ampl,const double *rns)
{
  ...
  // y
  // linear f(x)~1
  if      (m_ymode==0) m_rn[0]=m_amin+rn[0]*(m_amax-m_amin);
  // logarithmic f(x)~1/x
  else if (m_ymode==1) m_rn[0]=m_amin*pow(m_amax/m_amin,rn[0]);
  // quadratic f(x)~(x-a/2)
  else if (m_ymode==2) m_rn[0]=m_amin/2.0+sqrt(sqr(m_amin/2.0)+rn[0]*m_amax*(m_amax-m_amin));
  else THROW(fatal_error,"m_ymode changed to undefined value");

  // z
  // linear f(x)~1
  if      (m_zmode==0) m_rn[1]=rn[1];
  // power f(x)~(x-1/2)^(2n)
  else if (m_zmode==1)
    if (rn[1]>0.5)
      ...
      m_rn[1]=(1.-pow(-2.*rn[1]+1.,1./(2.*m_exp+1.)))/2.;
  // exponential
  else if (m_zmode==2) m_rn[1]=rn[1];
  else THROW(fatal_error,"m_zmode changed to undefined value");
...
double FF_Dipole::GenerateWeight(const Cluster_Amplitude *ampl)
{
...
(const Cluster_Leg *li,const Cluster_Leg *lj,
 const Cluster_Leg *lk,double &y,double &z,double &phi,
...
(Cluster_Leg *const li,Cluster_Leg *const lj,Cluster_Leg *const lk,
 Cluster_Leg *const lij,const double &y,const double &z,const double &phi)
{
