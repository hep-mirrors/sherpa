...
void Multi_Channel::GenerateWeight(Vec4D * p,Cut_Data * cuts,bool compute)
{
  if (channels.empty()) return;
  Vec4D_Vector pp(p,&p[nin+nout]);
  if (channels.size()==1) {
    channels[0]->GenerateWeight(&pp.front(),cuts,compute);
