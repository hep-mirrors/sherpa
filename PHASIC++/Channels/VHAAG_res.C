...
VHAAG_res::VHAAG_res(int _nin,int _nout,int pn,VHAAG_res* ovl)
{
  ...
  if (!ovl) {
    ...
    m_rkf = s["RES_KF"].SetDefault(24).Get<int>();
    n_d1 = s["RES_D1"].SetDefault(2).Get<int>();
    n_d2 = s["RES_D2"].SetDefault(3).Get<int>();
...
void VHAAG_res::GenerateWeight(ATOOLS::Vec4D *p,Cut_Data *cuts,bool)
{
  CalculateS0(cuts);
  double wt=1.;
