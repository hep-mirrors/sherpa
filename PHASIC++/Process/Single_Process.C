  ...
  if (GetSubevtList() == nullptr) {
      ...
      if (m_dads) {
        ...
        if (dps.p_dip != nullptr) {
          ...
          for (Process_Base* proc : dps.m_procs) {
            ...
            wgtmap["ME"] *= dps.m_weight * m_dsweight;
            m_dadswgtmap += wgtmap;
    ...
    for (int i {0}; i < GetSubevtList()->size(); ++i) {
      ...
      if (sub->m_me == 0.0) {
        sub->m_result = 0.0;
        if (varmode == Variations_Mode::all) {
          sub->m_results["ME"] = MakeWeights(Variations_Type::qcd);
          sub->m_results["QCUT"] = MakeWeights(Variations_Type::qcut);
        } else {
          sub->m_results["ME"] = Weights {};
        }
        sub->m_results["ME"] = 0.0;
      } else {
        // calculate ISR weight
	...
	if (sub->p_ampl) {
	  if (sub->p_real->p_ampl) ampls.push_back(sub->p_real->p_ampl);
        ...
        sub->m_results["ME"] *= sub->m_result;
        // NOTE: set all variation weights to zero, if the subevent is rejected
        // due to its scale being above Qcut; this is required for a correct
        // addition of the subevent weights to form the total weight in
        // ReweightRS
        if (sub->m_results["QCUT"].Nominal() == 0.0) {
          Reweight(sub->m_results["ME"], [](
                double varweight,
...
void Single_Process::ResetResultsForDifferential(Variations_Mode varmode)
{
  ...
  if (varmode != Variations_Mode::nominal_only) {
    m_last["ME"] = MakeWeights(Variations_Type::qcd);
    m_lastb["ME"] = MakeWeights(Variations_Type::qcd);
    m_dadswgtmap["ME"] = MakeWeights(Variations_Type::qcd);
  }
  m_last["ME"] = 1.0;
  m_dadswgtmap["ME"] = 0.0;
  m_lastb["ME"] = 0.0;
...
void Single_Process::ReweightBVI(ClusterAmplitude_Vector& ampls)
{
  ...
  Reweight(m_last["ME"], [this, &ampls, &info](
    ...
    if (m_mewgtinfo.m_type == mewgttype::none ||
      ...
      m_lastb["ME"].var(varindex) = (m_lastbxs != 0.0) ? res : 0.0;
      return K * res;
      ...
      m_lastb["ME"].var(varindex) = (m_lastbxs != 0.0) ? resb : 0.0;
      return res;
    }
  });
}
  ...
  s_variations->ForEach(
      [this, &info, &last_subevt_idx](size_t varindex,
        ...
        for (int i {0}; i <= last_subevt_idx; ++i) {
          auto sub = (*GetSubevtList())[i];
          if (sub->m_results["ME"].var(varindex) != 0.0) {
            ...
            sub->m_results["ME"].var(varindex) = contrib;
          }
        }
      });
