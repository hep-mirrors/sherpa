...
KP_Terms::KP_Terms(Process_Base *const proc, const sbt::subtype st,
    ...
    : m_stype(st), m_itype(cs_itype::K | cs_itype::P),
      m_kcontrib(cs_kcontrib::Kb | cs_kcontrib::KFS | cs_kcontrib::t |
                 cs_kcontrib::Kt),
  ...
  if (nfgs < nf)
    THROW(fatal_error, "Number of flavours in g->qq splitting (" +
                           ToString(nfgs) +
  ...
  m_kcontrib =
      ToType<cs_kcontrib::type>(kpsettings["KCONTRIB"].Get<std::string>());
  ...
  if (m_NC != 3.)
    msg_Out() << "Set N_color=" << m_NC << "." << std::endl;
  p_kernel->SetNC(m_NC);
  SetColourFactors();
  ...
  msg_Tracking() << "Switch collinear VFF splittings "
                 << (collVFF ? "on" : "off") << "." << std::endl;
  p_kernel->SetCollinearVFFSplitting(collVFF);

  SetMassive();
  m_sa =
      ((m_stype == sbt::qcd && m_flavs[0].Strong()) ||
       (m_stype == sbt::qed && (m_flavs[0].Charge() || m_flavs[0].IsPhoton())));
  m_sb =
      ((m_stype == sbt::qcd && m_flavs[1].Strong()) ||
...
void KP_Terms::SetColourFactors() {
  ...
  for (size_t i(0); i < m_flavs.size(); ++i) {
    if (m_stype == sbt::qcd) {
      if (abs(m_flavs[i].StrongCharge()) == 0)
        cpls[i] = 0.;
      else if (abs(m_flavs[i].StrongCharge()) == 3)
        cpls[i] = p_kernel->CF();
      else if (abs(m_flavs[i].StrongCharge()) == 8)
        cpls[i] = p_kernel->CA();
      else
        THROW(fatal_error, "Unknown colour charge.");
    } else if (m_stype == sbt::qed) {
      if (m_flavs[i].Charge())
        cpls[i] = sqr(m_flavs[i].Charge());
  ...
  for (size_t i = p_proc->NIn(); i < m_flavs.size(); i++) {
    if (m_flavs[i].IsMassive() &&
        ((m_stype == sbt::qcd && m_flavs[i].Strong()) ||
         (m_stype == sbt::qed && m_flavs[i].Charge())))
      m_massive = true;
    if ((m_stype == sbt::qcd && m_flavs[i].IsGluon()) ||
        (m_stype == sbt::qed && m_flavs[i].IsPhoton()))
      fsgluons++;
  }
  if (p_kernel->Nmf() > 0)
    m_massive = true;
...
void KP_Terms::SetCoupling(const MODEL::Coupling_Map *cpls) {
  ...
  if (m_stype == sbt::qcd)
    ...
    THROW(fatal_error,
          "Cannot set coupling for subtraction type" + ToString(m_stype));

  if (cpls->find(cplname) != cpls->end())
    p_cpl = cpls->find(cplname)->second;
  else
...
void KP_Terms::SetAlpha(const double &aff, const double &afi, const double &aif,
                        const double &aii) {
  p_kernel->SetAlpha(aff, afi, aif, aii);
}

void KP_Terms::SetKappa(const double &kappa) { p_kernel->SetKappa(kappa); }
 ...
 const double &wgt)
{
  ...
  if (m_sa) {
    ...
    if (m_itype & cs_itype::K) {
      if (m_kcontrib & cs_kcontrib::Kb) {
        ...
        m_kpca[0] = -w * p_kernel->Kb1(m_typea, x0) + p_kernel->Kb2(m_typea) -
                    p_kernel->Kb4(m_typea, eta0);
        m_kpca[1] =
            w * (p_kernel->Kb1(m_typea, x0) + p_kernel->Kb3(m_typea, x0));
      ...
      if (m_kcontrib & cs_kcontrib::KFS) {
        ...
        if (m_facscheme > 0) {
          ...
          m_kpca[0] += -w * p_kernel->KFS1(m_typea, x0) +
                       p_kernel->KFS2(m_typea) - p_kernel->KFS4(m_typea, eta0);
          m_kpca[1] +=
              w * (p_kernel->KFS1(m_typea, x0) + p_kernel->KFS3(m_typea, x0));
      ...
      for (int i = 0; i < 4; i++)
        m_kpca[i] *= p_kernel->Cpl(0) * dsij[0][0];
      msg_Debugging() << "    kpca[0]=" << m_kpca[0]
                      << " ,  kpca[1]=" << m_kpca[1]
                      << " ,  kpca[2]=" << m_kpca[2]
                      << " ,  kpca[3]=" << m_kpca[3] << std::endl;
      ...
      if (m_kcontrib & cs_kcontrib::t) {
        ...
        for (size_t i = pls; i < m_plist.size(); i++) {
          ...
          double muq2x = sajx;
          if (spin != 2)
            muq2 = sqr(m_flavs[m_plist[i]].Mass()) / saj;
          if (spin != 2)
            muq2x = sqr(m_flavs[m_plist[i]].Mass()) / sajx;
          m_kpca[0] +=
              dsij[0][i] * (-w * p_kernel->t1(m_typea, spin, muq2, x0) +
                            p_kernel->t2(m_typea, spin, muq2) -
                            p_kernel->t4(m_typea, spin, muq2, eta0));
          m_kpca[1] +=
              dsij[0][i] * (w * (p_kernel->t1(m_typea, spin, muq2x, x0) +
                                 p_kernel->t3(m_typea, spin, muq2x, x0)));
          m_kpca[2] +=
              dsij[0][i] * (-w * p_kernel->t1(m_typea + 2, spin, muq2, x0) +
                            p_kernel->t2(m_typea + 2, spin, muq2) -
                            p_kernel->t4(m_typea + 2, spin, muq2, eta0));
          m_kpca[3] +=
              dsij[0][i] * (w * (p_kernel->t1(m_typea + 2, spin, muq2x, x0) +
                                 p_kernel->t3(m_typea + 2, spin, muq2x, x0)));
          m_kpca[m_typea * 2 - 2] +=
              dsij[0][i] * p_kernel->t2c(m_typea, spin,
                                         sqr(m_flavs[m_plist[i]].Mass()) / saj,
                                         saj);
          if (spin != 2) {
            m_kpca[1] -= dsij[0][i] * w * p_kernel->Kbc3(m_typea, muq2x, x0);
            m_kpca[3] -=
                dsij[0][i] * w * p_kernel->Kbc3(m_typea + 2, muq2x, x0);
          }
          if (spin == 2) {
            for (size_t j = 0; j < p_kernel->Nmf(); j++) {
              ...
              if (m_xpa[xpcnt].xp > eta0) {
                m_kpca[0] +=
                    dsij[0][i] * p_kernel->t6(m_typea, m_xpa[xpcnt].xp);
                m_kpca[1] +=
                    dsij[0][i] * w * p_kernel->t5(m_typea, x0, m_xpa[xpcnt].xp);
                m_kpca[2] +=
                    dsij[0][i] * p_kernel->t6(m_typea + 2, m_xpa[xpcnt].xp);
                m_kpca[3] += dsij[0][i] * w *
                             p_kernel->t5(m_typea + 2, x0, m_xpa[xpcnt].xp);

                m_xpa[xpcnt].kpc =
                    dsij[0][i] *
                    (-w * p_kernel->t5(m_typea, x0, m_xpa[xpcnt].xp) -
      ...
      if (m_kcontrib & cs_kcontrib::Kt) {
        ...
        if (m_sb) {
          ...
          m_kpca[0] -= dsij[0][1] *
                       (-w * p_kernel->Kt1(m_typea, x0) +
                        p_kernel->Kt2(m_typea) - p_kernel->Kt4(m_typea, eta0));
          m_kpca[1] -=
              dsij[0][1] * w *
              (p_kernel->Kt1(m_typea, x0) + p_kernel->Kt3(m_typea, x0));
          m_kpca[2] -= dsij[0][1] * (-w * p_kernel->Kt1(m_typea + 2, x0) +
                                     p_kernel->Kt2(m_typea + 2) -
                                     p_kernel->Kt4(m_typea + 2, eta0));
          m_kpca[3] -=
              dsij[0][1] * w *
              (p_kernel->Kt1(m_typea + 2, x0) + p_kernel->Kt3(m_typea + 2, x0));
          m_kpca[m_typeb * 2 - 2] +=
              dsij[0][1] * p_kernel->t2c(m_typea, m_typeb, 0., 0.);
        }
        msg_Debugging() << "    kpca[0]=" << m_kpca[0]
                        << " ,  kpca[1]=" << m_kpca[1]
                        << " ,  kpca[2]=" << m_kpca[2]
    ...
    if (m_itype & cs_itype::P) {
      ...
      for (size_t i = 1; i < m_plist.size(); i++) {
        fsum += dsij[0][i];
        asum += dsij[0][i] *
                log(muf2 / dabs(2. * mom[m_plist[0]] * mom[m_plist[i]]));
      }
      double p4(-w * p_kernel->P1(m_typea, x0) + p_kernel->P2(m_typea) -
                p_kernel->P4(m_typea, eta0));
      double p5(w * (p_kernel->P1(m_typea, x0) + p_kernel->P3(m_typea, x0)));
      double p6(-w * p_kernel->P1(m_typea + 2, x0) + p_kernel->P2(m_typea + 2) -
                p_kernel->P4(m_typea + 2, eta0));
      double p7(
          w * (p_kernel->P1(m_typea + 2, x0) + p_kernel->P3(m_typea + 2, x0)));
  ...
  if (m_sb) {
    ...
    if (m_itype & cs_itype::K) {
      if (m_kcontrib & cs_kcontrib::Kb) {
        ...
        m_kpcb[0] = -w * p_kernel->Kb1(m_typeb, x1) + p_kernel->Kb2(m_typeb) -
                    p_kernel->Kb4(m_typeb, eta1);
        m_kpcb[1] =
            w * (p_kernel->Kb1(m_typeb, x1) + p_kernel->Kb3(m_typeb, x1));
      ...
      if (m_kcontrib & cs_kcontrib::KFS) {
        ...
        if (m_facscheme > 0) {
          ...
          m_kpcb[0] += -w * p_kernel->KFS1(m_typeb, x1) +
                       p_kernel->KFS2(m_typeb) - p_kernel->KFS4(m_typeb, eta1);
          m_kpcb[1] +=
              w * (p_kernel->KFS1(m_typeb, x1) + p_kernel->KFS3(m_typeb, x1));
      ...
      for (int i = 0; i < 4; i++)
        m_kpcb[i] *= p_kernel->Cpl(1) * dsij[0][0];
      msg_Debugging() << "    kpcb[0]=" << m_kpcb[0]
                      << " ,  kpcb[1]=" << m_kpcb[1]
                      << " ,  kpcb[2]=" << m_kpcb[2]
                      << " ,  kpcb[3]=" << m_kpcb[3] << std::endl;
      ...
      if (m_kcontrib & cs_kcontrib::t) {
        ...
        for (size_t i = pls; i < m_plist.size(); i++) {
          ...
          double muq2x = sajx;
          if (spin != 2)
            muq2 = sqr(m_flavs[m_plist[i]].Mass()) / saj; // mu-tilde
          if (spin != 2)
            muq2x = sqr(m_flavs[m_plist[i]].Mass()) / sajx; // mu
          m_kpcb[0] +=
              dsij[pls - 1][i] * (-w * p_kernel->t1(m_typeb, spin, muq2, x1) +
                                  p_kernel->t2(m_typeb, spin, muq2) -
                                  p_kernel->t4(m_typeb, spin, muq2, eta1));
          m_kpcb[1] +=
              dsij[pls - 1][i] * (w * (p_kernel->t1(m_typeb, spin, muq2x, x1) +
                                       p_kernel->t3(m_typeb, spin, muq2x, x1)));
          m_kpcb[2] += dsij[pls - 1][i] *
                       (-w * p_kernel->t1(m_typeb + 2, spin, muq2, x1) +
                        p_kernel->t2(m_typeb + 2, spin, muq2) -
                        p_kernel->t4(m_typeb + 2, spin, muq2, eta1));
          m_kpcb[3] += dsij[pls - 1][i] *
                       (w * (p_kernel->t1(m_typeb + 2, spin, muq2x, x1) +
                             p_kernel->t3(m_typeb + 2, spin, muq2x, x1)));
          m_kpcb[m_typeb * 2 - 2] +=
              dsij[pls - 1][i] *
              p_kernel->t2c(m_typeb, spin,
                            sqr(m_flavs[m_plist[i]].Mass()) / saj, saj);
          if (spin != 2) {
            m_kpcb[1] -=
                dsij[pls - 1][i] * w * p_kernel->Kbc3(m_typeb, muq2x, x1);
            m_kpcb[3] -=
                dsij[pls - 1][i] * w * p_kernel->Kbc3(m_typeb + 2, muq2x, x1);
          }
          if (spin == 2) {
            for (size_t j = 0; j < p_kernel->Nmf(); j++) {
              ...
              if (m_xpb[xpcnt].xp > eta1) {
                m_kpcb[0] +=
                    dsij[pls - 1][i] * p_kernel->t6(m_typeb, m_xpb[xpcnt].xp);
                m_kpcb[1] += dsij[pls - 1][i] * w *
                             p_kernel->t5(m_typeb, x1, m_xpb[xpcnt].xp);
                m_kpcb[2] += dsij[pls - 1][i] *
                             p_kernel->t6(m_typeb + 2, m_xpb[xpcnt].xp);
                m_kpcb[3] += dsij[pls - 1][i] * w *
                             p_kernel->t5(m_typeb + 2, x1, m_xpb[xpcnt].xp);

                m_xpb[xpcnt].kpc =
                    dsij[pls - 1][i] *
                    (-w * p_kernel->t5(m_typeb, x1, m_xpb[xpcnt].xp) -
      ...
      if (m_kcontrib & cs_kcontrib::Kt) {
        ...
        if (m_sa) {
          ...
          m_kpcb[0] -= dsij[0][1] *
                       (-w * p_kernel->Kt1(m_typeb, x1) +
                        p_kernel->Kt2(m_typeb) - p_kernel->Kt4(m_typeb, eta1));
          m_kpcb[1] -=
              dsij[0][1] * w *
              (p_kernel->Kt1(m_typeb, x1) + p_kernel->Kt3(m_typeb, x1));
          m_kpcb[2] -= dsij[0][1] * (-w * p_kernel->Kt1(m_typeb + 2, x1) +
                                     p_kernel->Kt2(m_typeb + 2) -
                                     p_kernel->Kt4(m_typeb + 2, eta1));
          m_kpcb[3] -=
              dsij[0][1] * w *
              (p_kernel->Kt1(m_typeb + 2, x1) + p_kernel->Kt3(m_typeb + 2, x1));
        }
        msg_Debugging() << "    kpcb[0]=" << m_kpcb[0]
                        << " ,  kpcb[1]=" << m_kpcb[1]
                        << " ,  kpcb[2]=" << m_kpcb[2]
    ...
    if (m_itype & cs_itype::P) {
      ...
      for (size_t i = 0; i < m_plist.size(); i++)
        if (i != pls - 1) {
          ...
          asum +=
              dsij[pls - 1][i] *
              log(muf2 / dabs(2. * mom[m_plist[pls - 1]] * mom[m_plist[i]]));
        }
      double p4(-w * p_kernel->P1(m_typeb, x1) + p_kernel->P2(m_typeb) -
                p_kernel->P4(m_typeb, eta1));
      double p5(w * (p_kernel->P1(m_typeb, x1) + p_kernel->P3(m_typeb, x1)));
      double p6(-w * p_kernel->P1(m_typeb + 2, x1) + p_kernel->P2(m_typeb + 2) -
                p_kernel->P4(m_typeb + 2, eta1));
      double p7(
          w * (p_kernel->P1(m_typeb + 2, x1) + p_kernel->P3(m_typeb + 2, x1)));
      // TODO: fix muf2 dependence
      m_kpcb[0] += asum * p4;
      m_kpcb[1] += asum * p5;
      m_kpcb[2] += asum * p6;
  ...
  msg_Debugging() << "    kpca[0]=" << m_kpca[0] << " ,  kpca[1]=" << m_kpca[1]
                  << " ,  kpca[2]=" << m_kpca[2] << " ,  kpca[3]=" << m_kpca[3]
                  << std::endl;
  msg_Debugging() << "    kpcb[0]=" << m_kpcb[0] << " ,  kpcb[1]=" << m_kpcb[1]
                  << " ,  kpcb[2]=" << m_kpcb[2] << " ,  kpcb[3]=" << m_kpcb[3]
                  << std::endl;
}

double KP_Terms::Get(PDF::PDF_Base *pdfa, PDF::PDF_Base *pdfb, const double &x0,
                     const double &x1, const double &eta0, const double &eta1,
  ...
  DEBUG_FUNC(m_stype << ": " << "fl0=" << fl0 << ", x0=" << x0
                     << ", eta0=" << eta0 << ", muf02=" << muf02
                     << ", muf02fac=" << muf02fac << "\n                   "
                     << "fl1=" << fl1 << ", x1=" << x1 << ", eta1=" << eta1
                     << ", muf12=" << muf12 << ", muf12fac=" << muf12fac);
  if (m_sa && (pdfa == NULL || !pdfa->Contains(fl0)))
    return 0.0;
  if (m_sb && (pdfb == NULL || !pdfb->Contains(fl1)))
    return 0.0;
  if (!m_sa && !m_sb)
    return 0.;
  if ((m_sa && x0 < eta0) || (m_sb && x1 < eta1))
    return 0.;
  size_t pls = 1;
  if (m_sa && m_sb)
    pls++;
  ...
  if (m_sa) {
    ...
    // a' = gluon/photon
    if (pdfa->Contains(gluon))
      fagx = pdfa->GetXPDF(gluon) / eta0;
    // a' = quark (if a = quark)
    if (fl0.IsQuark() && pdfa->Contains(fl0))
      faqx = pdfa->GetXPDF(fl0) / eta0;
    ...
    // in QED -Q_i^2 needs to be multiplied in
      ...
      for (size_t i = 0; i < quark.Size(); i++)
        if (pdfa->Contains(quark[i])) {
          if (m_stype == sbt::qcd)
            faqx += pdfa->GetXPDF(quark[i]);
          else
            faqx += sqr(quark[i].Charge()) * pdfa->GetXPDF(quark[i]);
        }
      faqx /= eta0;
    }
    // f_a(eta), f_a'(eta)
    pdfa->Calculate(eta0, muf02 * muf02fac);
    if (pdfa->Contains(fl0))
      fa = pdfa->GetXPDF(fl0) / eta0;
    ...
    if (!m_negativepdf && !(fa > 0.)) {
      msg_Tracking() << METHOD << "(): fa is not pos. definite, fa = " << fa
                     << std::endl;
      return 0.;
    }
    if (pdfa->Contains(gluon))
    ...
    // in QED -Q_i^2 needs to be multiplied in
      ...
      for (size_t i = 0; i < quark.Size(); i++)
        if (pdfa->Contains(quark[i])) {
          if (m_stype == sbt::qcd)
            faq += pdfa->GetXPDF(quark[i]);
          else
            faq += sqr(quark[i].Charge()) * pdfa->GetXPDF(quark[i]);
        }
      faq /= eta0;
    }
  ...
  if (m_sb) {
    ...
    // b' = gluon/photon
    if (pdfb->Contains(gluon))
      fbgx = pdfb->GetXPDF(gluon) / eta1;
    // b' = quark (if b = quark)
    if (fl1.IsQuark() && pdfb->Contains(fl1))
      fbqx = pdfb->GetXPDF(fl1) / eta1;
    // b' = quark (if b = gluon)
    // in QCD CF/CA already in kpca/b[1]
    // in QED -Q_i^2 needs to be multiplied in
    else {
      for (size_t i = 0; i < quark.Size(); i++)
        if (pdfb->Contains(quark[i])) {
          if (m_stype == sbt::qcd)
            fbqx += pdfb->GetXPDF(quark[i]);
          else
            fbqx += sqr(quark[i].Charge()) * pdfb->GetXPDF(quark[i]);
        }
      fbqx /= eta1;
    }
    // f_a(eta), f_a'(eta)
    pdfb->Calculate(eta1, muf12 * muf12fac);
    if (pdfb->Contains(fl1))
      fb = pdfb->GetXPDF(fl1) / eta1;
    // TODO: check whether this check is sensible (fa>0.)
    ...
    if (!m_negativepdf && !(fb > 0.)) {
      msg_Tracking() << METHOD << "(): fb is not pos. definite, fb = " << fb
                     << std::endl;
      return 0.;
    }
    if (pdfb->Contains(gluon))
    ...
    // in QED -Q_i^2 needs to be multiplied in
      ...
      for (size_t i = 0; i < quark.Size(); i++)
        if (pdfb->Contains(quark[i])) {
          if (m_stype == sbt::qcd)
            fbq += pdfb->GetXPDF(quark[i]);
          else
            fbq += sqr(quark[i].Charge()) * pdfb->GetXPDF(quark[i]);
        }
      fbq /= eta1;
    }

    for (size_t i = 0; i < m_xpb.size(); i++)
      if (m_xpb[i].xp > eta1) {
  ...
  if (m_sa && fa) {
    double a(m_kpca[0] * faq + m_kpca[1] * faqx + m_kpca[2] * fag +
             m_kpca[3] * fagx);
    if (muf02fac != 1.) {
      const double lF(log(muf02fac));
      a += (m_kpca[4] * faq + m_kpca[5] * faqx + m_kpca[6] * fag +
            m_kpca[7] * fagx) *
           lF;
    }
    a /= fa;
  ...
  if (m_sb && fb) {
    double b(m_kpcb[0] * fbq + m_kpcb[1] * fbqx + m_kpcb[2] * fbg +
             m_kpcb[3] * fbgx);
    if (muf12fac != 1.) {
      const double lF(log(muf12fac));
      b += (m_kpcb[4] * fbq + m_kpcb[5] * fbqx + m_kpcb[6] * fbg +
            m_kpcb[7] * fbgx) *
           lF;
    }
    b /= fb;
  ...
  if (msg_LevelIsDebugging()) {
    ...
    msg_Out() << "  " << m_itype << "-Terms Beam A:\n"
              << "    xa=" << x0 << " ,   etaa=" << eta0 << "    fa=" << fa
              << " ,  faq=" << faq << " ,  faqx=" << faqx << " ,  fag=" << fag
              << " ,  fagx=" << fagx << "\n"
              << "    kpca[0]=" << m_kpca[0] << " ,  kpca[1]=" << m_kpca[1]
              << " ,  kpca[2]=" << m_kpca[2] << " ,  kpca[3]=" << m_kpca[3]
              << std::endl;
    msg_Out() << "  => "
              << (m_kpca[0] * faq + m_kpca[1] * faqx + m_kpca[2] * fag +
                  m_kpca[3] * fagx) *
                     fb
              << std::endl;
    msg_Out() << "  " << m_itype << "-Terms Beam B:\n"
              << "    xb=" << x1 << " ,   etab=" << eta1 << "    fb=" << fb
              << " ,  fbq=" << fbq << " ,  fbqx=" << fbqx << " ,  fbg=" << fbg
              << " ,  fbgx=" << fbgx << "\n"
              << "    kpcb[0]=" << m_kpcb[0] << " ,  kpcb[1]=" << m_kpcb[1]
              << " ,  kpcb[2]=" << m_kpcb[2] << " ,  kpcb[3]=" << m_kpcb[3]
              << std::endl;
    msg_Out() << "  => "
              << (m_kpcb[0] * fbq + m_kpcb[1] * fbqx + m_kpcb[2] * fbg +
                  m_kpcb[3] * fbgx) *
                     fa
              << std::endl;
    msg_Out() << "res=" << res << std::endl
              << "  => " << res * fa * fb << std::endl;
...
void KP_Terms::FillMEwgts(ATOOLS::ME_Weight_Info &wgtinfo) {
  if (wgtinfo.m_type & mewgttype::KP) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      wgtinfo.m_wfac[i + offset + 8] =
          wgtinfo.m_swap ? m_kpcb[i + 4] : m_kpca[i + 4];
      wgtinfo.m_wfac[i + offset + 12] =
          wgtinfo.m_swap ? m_kpca[i + 4] : m_kpcb[i + 4];
    }
  }
}
