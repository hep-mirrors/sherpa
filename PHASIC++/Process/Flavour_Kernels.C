...
void Flavour_Kernels::SetSubType(const int subtype)
{
  m_subtype=subtype;
}
...
double Flavour_Kernels::Kb3(int type,double x)
{
  ...
  case 2:
    return m_CF*((1.+sqr(1.-x))/x*log(m_alpha*(1.-x)/x)+x+2.*c);
  case 3:
    return m_TR*((x*x+sqr(1.-x))*log(m_alpha*(1.-x)/x)+2.*x*(1.-x));
  case 4:
    return 2.*m_CA*(((1.-x)/x-1.+x*(1.-x))*log(m_alpha*(1.-x)/x)+at+c);
  }
  return 0.;
}
...
double Flavour_Kernels::t2c(int type)
{
  double c(0.0);
  if (m_subtype==1)
    switch(type) {
    case 1:
      return -1./4.;
    case 2:
      return 1./36.-1./18.*m_TR*m_nf/m_CA;
    }
  return 0.;
}
