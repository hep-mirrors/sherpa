...
{
  DEBUG_FUNC(m_name);
  // Detect instanton final state early: for 2->1 processes with sole FS
  // particle kf_instanton the instanton absorbs net quark number
  // non-perturbatively, so charge, strong-charge, and quark-count
  // conservation checks in the perturbative sense do not apply.
  bool instanton_fs = (fi.m_ps.size()==1 &&
                       fi.m_ps[0].m_fl==Flavour(kf_instanton));
  if (instanton_fs) {
    msg_Debugging()<<METHOD<<"(): instanton final state — bypassing "
               <<"charge/strong conservation for '"
               <<GenerateName(ii,fi)<<"'\n";
    return true;
  }
