...
Phase_Space_Point::~Phase_Space_Point() {
  ...
  if (p_fixedISboost) delete p_fixedISboost;
...
void Phase_Space_Point::InitFixedIncomings() {
  if (m_nin==1) {
    ...
    if (p_beamhandler->On() || p_isrhandler->On()!=0) return;
    double Ebeam[2];
    ...
    for (int i=0;i<m_nin;++i) p_fixedISboost->Boost(m_ISmoms[i]);
...
bool Phase_Space_Point::operator()(Process_Integrator *const process,
  ...
  //msg_Out()<<"\n\n\n"
  //	   <<"=====================================================\n"
  //	   <<METHOD<<"(beam = "<<p_beamchannels<<", isr = "<<p_isrchannels<<")\n"
  //	   <<"=====================================================\n";
  ...
  if (m_nin==2) {
    ...
    if (p_beamchannels) {
      ...
      for (size_t i=0;i<2;i++) p_beamhandler->BoostInLab(p_moms[i]);
...
bool Phase_Space_Point::MakeIncoming() 
{
  if (m_nin==1 || (!p_beamhandler->On() && p_isrhandler->On()==0)) {
    for (size_t i=0;i<2;i++) p_moms[i] = m_ISmoms[i];
    return true;
  }
  else if (m_nin == 2) {
    double Eprime(m_Ecms);
    if (p_isrhandler->On()>0) {
      if (m_isrspkey[3]==0.) m_isrspkey[3] = sqr(m_Ecms);
      m_sprime = m_isrspkey[3];
      m_Eprime = sqrt(m_sprime);
    }
    else if (p_beamhandler->On()) {
      if (m_beamspkey[3]==0.) m_beamspkey[3] = sqr(m_Ecms);
      m_sprime = m_beamspkey[3];
      m_Eprime = sqrt(m_sprime);
    }
    if (m_Eprime<m_masses[0]+m_masses[1]) return false;
  ...
  msg_Error()<<"Error in"<<METHOD<<":\n"
	     ...
	     <<(m_nin==1?m_masses[0]:m_masses[0]+m_masses[1])<<") or\n"
	     <<"   bad number of incoming particles ("<<m_nin<<").\n";
  return false;
} 

void Phase_Space_Point::BoostSystem() {
  if (p_isrchannels)  p_isrhandler->BoostInLab(p_moms,m_nvec);
  if (p_beamchannels) p_beamhandler->BoostInLab(p_moms,m_nvec);
  // This is for the case of fixed beams and captures asymmetric initial states
  if (p_fixedISboost) {
    for (int i=0;i<m_nvec;++i) p_fixedISboost->BoostBack(p_moms[i]);
  }
}
...
bool Phase_Space_Point::Check4Momentum() {
  Vec4D pin,pout;
  for (int i=0;i<m_nin;i++) pin += p_moms[i];
  for (int i=m_nin;i<m_nin+m_nout;i++) pout += p_moms[i];
