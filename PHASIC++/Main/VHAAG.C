		     ...
		     ATOOLS::Vec4D& p1,ATOOLS::Vec4D& p2,int n2,double *ran)
{
  ...
  double s2 = CE.MasslessPropMomenta(s_i,smin,smax,ran[0]);
				...
				ATOOLS::Vec4D p1,ATOOLS::Vec4D p2,int n2,double *ran)
{
  ...
  double smin = double(n2*(n2-1)/2)*m_s0;
  double smax = Min(s-n2*m_s0,s*(1.-a1min));
  wt*= CE.MasslessPropWeight(s_i,smin,smax,s2,ran[0]);
...
void VHAAG::GenerateWeight(ATOOLS::Vec4D *p,ATOOLS::Cut_Data *cuts)
{
  CalculateS0(cuts);
  double wt=1.;
...
void VHAAG::GeneratePoint(ATOOLS::Vec4D *p,ATOOLS::Cut_Data *cuts,double *ran)
{
  CalculateS0(cuts);
