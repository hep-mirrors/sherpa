...
double Process_Integrator::SelectionWeight(const int mode) const
{
  if (!p_proc->IsGroup()) {
	...
	return m_external_selectionweight;
      ...
      if (m_timing_statistics_min_unc) 
	return dabs(TotalResult()*m_meanenhfunc*m_enhancefac/m_effi/pow(m_effevperev,0.5));
...
std::vector<double> Process_Integrator::TotalEffiAndEffEvPerEv(bool unweighted) const
{
  ...
  if (p_proc->IsGroup()) {
    ...
    for (size_t i(0);i<p_proc->Size();++i) {
      ...
      if (m_timing_statistics_min_unc) 
	proci_selw = dabs(proci_xsec)/pow(proci_effevperev,0.5)/proci_effi;
      sum_xsec += proci_xsec;
...
double Process_Integrator::GetMaxEps(double epsilon)
{
  ...
  *p_whisto += *p_whisto_neg;
  ...
  if (whisto_abs_sum==0) {
    msg_Error()<<"Warning: The weight histogram for "<< p_proc->ResultsName() << " is empty. Partial unweighting is not possible."<<std::endl;
...
void Process_Integrator::Reset(const int mode)
{
  ...
  m_smax=m_max=m_wmin=m_ssigma2=m_ssumsqr=m_ssum=m_ssumenh=m_ssumenhabs=m_ssumenhsqr=0.0;
  m_sn=0;
  m_sncut=0;
  m_son=1;
  m_vsmax.clear(); 
...
void Process_Integrator::ReadResults()
{
  if (m_resultpath.length()==0) return;
  std::string fname(p_proc->ResultsName());
  //needs to be called before the next two, to set p_pshandler->IntegrationSherpaVersion())
