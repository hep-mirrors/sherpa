  ...
  Scale_Setter_Base(args), m_tagset(this)
{
  ...
  if (s_cmode<0) {
    ...
    s_allowuo=s["ALLOW_SCALE_UNORDERING"].GetScalarWithOtherDefault<int>(1);
...
(Cluster_Amplitude *ampl,ClusterAmplitude_Vector &ampls,const int ord) const
{
  ...
  DEBUG_FUNC("nmin = "<<m_nmin<<", strict = "<<strict<<", ord = "<<ord);
  msg_Debugging()<<*ampl<<"\n";
	...
	for (size_t f(0);f<cf.size();++f) {
	  for (size_t k(0);k<ampl->Legs().size();++k) {
		...
		continue;
	      ...
	      if ((li->Id()|lj->Id())&3) {
	        if (-ci.second.m_pijt.PPlus()>rpa->gen.PBeam(0).PPlus() ||
	            -ci.second.m_pijt.PMinus()>rpa->gen.PBeam(1).PMinus()) {
                  continue;
                }
              }
	      if (lk->Id()&3)
	        if (-ci.second.m_pkt.PPlus()>rpa->gen.PBeam(0).PPlus() ||
	            -ci.second.m_pkt.PMinus()>rpa->gen.PBeam(1).PMinus()) continue;
	      ccs.push_back(ci);
