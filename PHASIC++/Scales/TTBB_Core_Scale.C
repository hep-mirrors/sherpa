...
namespace PHASIC {
  ...
  class TTBB_Core_Scale: public Core_Scale_Setter {
  public:

    TTBB_Core_Scale(const Core_Scale_Arguments &args):
...
PDF::CParam TTBB_Core_Scale::Calculate(Cluster_Amplitude *const ampl)
{
  DEBUG_FUNC("");
  msg_Debugging()<<*ampl<<"\n";
  ...
  if (campl->Legs().size()!=4) {
    ...
    campl->Delete();
    msg_Debugging()<<"more than 2 legs, use sum of MPerp in case of QCD particles only \n";
  ...
  if (fl[0].Strong() && fl[1].Strong()) {// hh collision
    if (fl[2].Strong() && fl[3].Strong()) {
      msg_Debugging()<<"pure QCD like, custom TTBB\n";
     // default
      muq2=muf2=mur2=sqr(172.5) + 0.5*(campl->Leg(2)->Mom().PPerp2() + campl->Leg(3)->Mom().PPerp2() );
...
Cluster_Amplitude *TTBB_Core_Scale::Cluster
...
DECLARE_ND_GETTER(TTBB_Core_Scale,"TTBB",
		  Core_Scale_Setter,Core_Scale_Arguments,true);

Core_Scale_Setter *ATOOLS::Getter
<Core_Scale_Setter,Core_Scale_Arguments,TTBB_Core_Scale>::
operator()(const Core_Scale_Arguments &args) const
{
  return new TTBB_Core_Scale(args);
}
