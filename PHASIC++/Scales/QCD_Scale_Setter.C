...
namespace PHASIC {
  ...
  public:
    ...
    QCD_Scale_Setter(Process_Base *const proc,
...
{
  return new QCD_Scale_Setter(args.p_proc,args.m_ren,args.m_fac);
}
...
QCD_Scale_Setter::QCD_Scale_Setter
...
(const ATOOLS::Cluster_Amplitude &ampl,int i,int j,int k)
{
  ...
  if (!(li->Flav().IsGluon() || lj->Flav().IsGluon() ||
	li->Flav()==lj->Flav().Bar())) return nd;
...
(Cluster_Amplitude &ampl,int i,int j,int k)
{
  ...
  if (li->Flav().IsQuark()) {
    if (lj->Flav()==li->Flav().Bar()) 
      li->SetFlav(Flavour(kf_gluon));
  }
  else {
    if (lj->Flav().IsQuark())
      li->SetFlav(lj->Flav());
