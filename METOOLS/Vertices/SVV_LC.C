...
namespace METOOLS {
  ...
  class SVV_Calculator: public Lorentz_Calculator {
  public:
  ...
  public:
    ...
    SVV_Calculator(const Vertex_Key &key);

    std::string Label() const;
    
    void Evaluate();

  };// end of class SVV_Calculator
...
template <typename SType>
SVV_Calculator<SType>::SVV_Calculator(const Vertex_Key &key):
...
template <typename SType>
void SVV_Calculator<SType>::Evaluate()
{
  p_v->SetZero();
  if (p_v->JA()->Zero()||p_v->JB()->Zero()) return;
#ifdef DEBUG__BG
  msg_Debugging()<<*p_v->JA()<<"(+)"<<*p_v->JB()<<" SVV("<<m_dir
		 <<"): m_cpl = "<<m_cpl<<"\n";
  msg_Indent();
#endif
  size_t i(0);
  ...
  case 1: {
	...
	if (p_cc->Evaluate(*ait,*bit)) {
	  ...
	  msg_Debugging()<<"    "<<**bit<<"\n";
#endif
	  CVec4Type *j(CVec4Type::New(**ait*(**bit)[0]));
	  *j*=SComplex(m_cpl);
	  j->SetH(p_v->H(i));
	  j->SetS((*ait)->S()|(*bit)->S());
	  p_cc->AddJ(j);
  ...
  case 2: {
	...
	if (p_cc->Evaluate(*ait,*bit)) {
	  ...
	  msg_Debugging()<<"    "<<**ait<<"\n";
#endif
	  CVec4Type *j(CVec4Type::New(**bit*(**ait)[0]));
	  *j*=SComplex(m_cpl);
	  j->SetH(p_v->H(i));
	  j->SetS((*ait)->S()|(*bit)->S());
	  p_cc->AddJ(j);
...
template <typename SType>
std::string SVV_Calculator<SType>::Label() const
{
  return "SVV["+ToString(m_cpl)+"]";
}

namespace METOOLS {

  template class SVV_Calculator<double>;
  template class SVV_Calculator<long double>;

}

DECLARE_TEMPLATE_GETTER(SVV_Getter,"Gab",Lorentz_Calculator,Vertex_Key);

template <typename SType,char STag> Lorentz_Calculator *
SVV_Getter<SType,STag>::operator()(const Vertex_Key &key) const
{
  return new SVV_Calculator<SType>(key);
}

template <typename SType,char STag>
void SVV_Getter<SType,STag>::PrintInfo
(std::ostream &str,const size_t width) const
{
  str<<"SVV vertex";
}

template class SVV_Getter<double,'D'>;
template class SVV_Getter<long double,'Q'>;
