#include "METOOLS/HadronCurrents/Current_ME.H"
...
Current_ME::Current_ME(const Flavour_Vector& flavs,
                       const vector<int>& decayindices,
                       const string& name):
  ME_Base(flavs,decayindices,name), p_c1(NULL), p_c2(NULL)
{}
...
{
  string current_string = s["Type"].SetDefault("Unknown").Get<string>();
  auto indices = s["Indices"].SetDefault({-1}).GetVector<int>();
  ME_Parameters fi(m_flavs, indices);
  Current_Base* current =
    Current_Getter_Function::GetObject(current_string,fi);
  if(current==NULL) {
...
void Current_ME::Calculate(const Vec4D_Vector& p, bool anti)
{
  ...
  vector<int> spins,spins1,spins2;
...
void ATOOLS::Getter<METOOLS::ME_Base,METOOLS::ME_Parameters,METOOLS::Current_ME>::
PrintInfo(ostream &st,const size_t width) const {
  st<<"Current_ME"<<endl;
}
