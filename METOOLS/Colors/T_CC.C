...
DECLARE_GETTER(T_C_Getter,"T",Color_Calculator,Vertex_Key);

Color_Calculator *T_C_Getter::operator()(const Vertex_Key &key) const
{
  return new T_Calculator(key);
}

void T_C_Getter::PrintInfo(std::ostream &str,const size_t width) const
{
  str<<"fundamental";
}
