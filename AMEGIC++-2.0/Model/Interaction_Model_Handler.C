...
{ 
  if (modeltype==std::string("pure_QCD")) return new Interaction_Model_QCD(p_model,cplscheme,yukscheme);
  if (modeltype==std::string("pure_EW"))  return new Interaction_Model_EW(p_model,cplscheme,yukscheme);
