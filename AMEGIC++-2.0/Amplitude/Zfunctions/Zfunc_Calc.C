...
Kabbala Z_Calc::Do() 
{
  if (IsZero(M(0))) return P(pn)*Z(0,1);
  return P(pn)*(Z(0,1)-M(0)*X(0,0)*X(1,0));
}

Kabbala VVS_Calc::Do() 
{
  Kabbala prefactor = P(pn)*sgen->Get_Enumber(coupl[4]);
  return prefactor*(-M(1)*X(0,1)*X(1,1)+X(0,0)*(-M(0)*X(1,0)+M(0)*M(1)*V(0,1)*X(1,1))+Z(1,0));
}

Kabbala SSV_Calc::Do() 
{
  Kabbala prefactor = P(pn)*sgen->Get_Enumber(coupl[6]);
...
{
  Kabbala prefactor = P(pn)*sgen->Get_Enumber(coupl[4])*sgen->Get_Enumber(coupl[5]);
...
Kabbala ZNC_Calc::Do() 
{
  Kabbala prefactor = exp(sgen->Get_Enumber(Complex(0.,1.))*(VNC(0)+VNC(1)))*P(pn);
