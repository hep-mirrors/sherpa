...
void Amplitude_Handler::OptimizeProps(int N,Single_Amplitude* f1)
{
  ...
  for(list<Pfunc*>::iterator pit=pl->begin();pit!=pl->end();++pit){
	...
	for(int j=0;j<(*zit)->pn;j++)if((*zit)->psnew[j].numb==(*pit)->arg[0]){
	  if((*zit)->psnew[j].direction==Direction::Incoming)
	       (*zit)->psnew[j].direction=Direction::Outgoing;
	  else (*zit)->psnew[j].direction=Direction::Incoming;
	}
      }
    }
  }
...
void Amplitude_Handler::PreCluster(Single_Amplitude* firstgraph)
{
  ...
  while (f1) {
    ...
    for(int i=0;i<propselect.size();i++){
      ...
      Zfunc *zh[2];
      for (list<Zfunc*>::iterator zit=zlist->begin();zit!=zlist->end();++zit){
	for(int j=0;j<(*zit)->narg;j++){
	  if((*zit)->arg[j]==propselect[i]){
	    zh[ia]=(*zit);
	    ia++;
	    break;
	  }
	...
	int n0=0;
	for(int j=0;j<zh[0]->narg;j++)if(zh[0]->arg[j]>99)n0++;
	int n1=0;
	for(int j=0;j<zh[1]->narg;j++)if(zh[1]->arg[j]>99)n1++;
	if(n0<=1 || n1<=1 || 
	   ((zh[0]->type==zl::Y || zh[0]->type==zl::Z) 
	    &&(zh[0]->type==zl::Y || zh[0]->type==zl::Z))){
	  ...
	  if(zh[0]->narg>zh[1]->narg){zh0=zh[1];zh1=zh[0];}
	  if(zh[0]->narg==zh[1]->narg){
	    //cout<<zh[0]->narg<<" "<<zh[1]->narg<<endl;
	    for(int j=0;j<zh[0]->narg;j++){
	      if(PropProject(f1,zh[0]->arg[j])<PropProject(f1,zh[1]->arg[j]))break;
	      if(PropProject(f1,zh[0]->arg[j])>PropProject(f1,zh[1]->arg[j])){zh0=zh[1];zh1=zh[0];break;}
	    }
	  }
