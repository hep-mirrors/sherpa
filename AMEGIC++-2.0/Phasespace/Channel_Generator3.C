...
Channel_Generator3::Channel_Generator3(int _nin,int _nout,Flavour * _fl,
...
Channel_Generator3::~Channel_Generator3() 
{ 
  for (int i=0;i<m_pclist.size();i++) delete m_pclist[i];
}

void Channel_Generator3::GenerateTopos(Point* p)
{
  if (nin != 2) return;
  Point* ph = p->left;
  if (ph->left==0) {
...
Point* Channel_Generator3::CopyPoints(Point* p)
{
  Point* ph = NULL; 
  if (p!=NULL) { 
    ph = new Point(*p);
...
Point* Channel_Generator3::TransformTS(Point* p)
{
...
void Channel_Generator3::MRPScan()
{}

int Channel_Generator3::MakeChannel(int& echflag,int n,string& path,string& pID)
{  
...
void Channel_Generator3::Step0(int flag,Point* p,int& rannum,ofstream& sf) 
{
  if (nin != 2) return;
  Point* ph = p->left;
  if (ph->left==0) {
...
void Channel_Generator3::GenerateDecayChain(int flag,Point* p,int& rannum,ofstream& sf,
...
bool Channel_Generator3::QCDAntenna(int flag,Point* p,int& rannum,ofstream& sf,int n)
{ 
...
void Channel_Generator3::GenerateMassChain(int flag,Point* p,Point* clmp,int& rannum,ofstream& sf)
{
  if (p->left==0) {
...
void Channel_Generator3::SetProps(Point* p,Point** props,Point** propt, int& count)
{
  if (p->left==0) return;
...
void Channel_Generator3::CalcTSmin(int flag,vector<string>& p,ofstream& sf)
{
  string help;
...
void Channel_Generator3::CalcSmin(int flag,char* min,string lm,ofstream& sf,Point* p)
{
...
string Channel_Generator3::EstimatePeak(int flag,Point* p,ofstream& sf)
{
  string help,h1;
  if (p->m==0) {
...
string Channel_Generator3::GetFlMass(Point* p)
{
...
string Channel_Generator3::LinkedMasses(Point* p)
{
  if (p->left==0) {
    char help[4];
...
void Channel_Generator3::IdentifyProps(Point* _plist)
{
  InitT(_plist);
  Point* endp;
  tcount = 0;
...
void Channel_Generator3::BackLinks(Point* p,Point* &endp)
{
...
void Channel_Generator3::InitT(Point* p)
{
  p->t = 0;
  if (p->left==0) return;
  InitT(p->left);
...
string Channel_Generator3::IString(int i)
{
  MyStrStream sstr;
  sstr<<i;
  string istr;
...
string Channel_Generator3::Order(string s)
{
  int beg = s.find("_");
  if (beg!=-1) {
...
void  Channel_Generator3::AddToVariables(int flag,const string& lhs,const string& rhs,const int& type,
					ofstream& sf)
{
  if (flag<0) return;
  ...
  if (cit==declarations.end()) {
    ...
    if (rhs != declarations[name]) {
      msg.Error()<<" ERROR in Channel_Generator3::AddToVariables ()"<<endl;
      abort();
    }
  }
}

bool  Channel_Generator3::CheckVariables(int flag,const string& lhs,const int& type)
{
...
int Channel_Generator3::AntennaS(Point* p)
{
...
std::string Channel_Generator3::CreateChannelID(int echflag)
{
...
double Channel_Generator3::PMassSum(Point* p,int *pt)
{
  int tptl,tptr;
  *pt=0;
  if (!p->left) return 0.;
