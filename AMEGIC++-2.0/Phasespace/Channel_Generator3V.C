...
Channel_Generator3V::Channel_Generator3V(int _nin,int _nout,Flavour * _fl,
...
void Channel_Generator3V::CalcSmin(int flag,char* min,string lm,ofstream& sf,Point* p)
{
					...
					ofstream& sf)
{
  ...
  if (cit==declarations.end()) {
    ...
    if (rhs != declarations[name]) {
      msg.Error()<<" ERROR in Channel_Generator3V::AddToVariables ()"<<endl;
      abort();
    }
  }
}
