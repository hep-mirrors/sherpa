...
//   m_scnt=0;
  ...
  if (_seldata) p_selector = new Combined_Selector(m_nin,m_nout,p_flavours,_seldata,m_cuttag);
    ...
    if (m_nout>2)
      msg_Out()<<"WARNING in Single_Process_MHV "<<m_name<<endl
...
//     if (p_partner!=this) links.push_back(this);
  ...
  case -3: return -3;
  default :
    msg_Error()<<"ERROR in Single_Process_MHV::InitAmplitude : "<<std::endl
...
//   }
  ...
  case 1 : 
    ...
    return 0;
  default :
    msg_Error()<<"Error in Single_Process_MHV::InitAmplitude : Failed for "<<m_name<<"."<<endl;
    return -2;
  }
}
...
void Single_Process_MHV::CreateMappingFile() {
  ...
  if (IsFile(outname)) {
    ...
    if (MEname != m_libname || PSname != m_pslibname) {
      msg_Error()<<"ERROR in Single_Process_MHV::CreateMappingFile() :"<<std::endl
...
bool Single_Process_MHV::CalculateTotalXSec(std::string _resdir) { 
  ...
  if (!(ATOOLS::IsZero((m_totalxs-TotalResult())/(m_totalxs+TotalResult())))) {
    msg_Error()<<"ERROR in Single_Process_MHV::CalculateTotalXSec :"<<std::endl
...
bool Single_Process_MHV::PrepareXSecTables() { 
  ...
  if (!(ATOOLS::IsZero((m_totalxs-TotalResult())/(m_totalxs+TotalResult())))) {
    msg_Error()<<"ERROR in Single_Process_MHV::PrepareXSecTables :"<<std::endl
...
const int Single_Process_MHV::SelectedHelicity()
{
  ...
  if (hel>=m_helnumber) {
    msg_Out()<<"WARNING in Single_Process_MHV::SelectedHelicity() after "<<m_throws<<std::endl;
    hel = m_helnumber-1;
  }
  return hel;
}
...
double Single_Process_MHV::operator()(const ATOOLS::Vec4D * mom,const int hel)
{
  if (!p_shand->Is_String()) {
    msg_Error()<<"Error in Single_Process_MHV::operator()(ATOOLS::Vec4D * p,int hel)"<<std::endl
