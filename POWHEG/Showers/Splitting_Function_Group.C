...
ostream& POWHEG::operator<<(std::ostream& str, Splitting_Function_Group &group) {
...
void Splitting_Function_Group::SetRB(RB_Map *const rbmap)
{
  for (size_t i(0);i<m_splittings.size();++i)
    m_splittings[i]->SetRB(rbmap);
}
