...
#define PARAMETER_TYPE POWHEG::SF_Key
#define OBJECT_TYPE POWHEG::SF_Lorentz
#define SORT_CRITERION std::less<std::string>
#include "ATOOLS/Org/Getter_Function.C"

template class Getter_Function
<POWHEG::SF_Coupling,POWHEG::SF_Key,SORT_CRITERION>;

template class Getter_Function
<void,POWHEG::SFC_Filler_Key,SORT_CRITERION>;
  ...
  m_rbkey(0,kf_none,kf_none,kf_none)
{
  ...
  if (m_type==cstp::IF) m_rbkey.m_type=1;
  else if (m_type==cstp::FI) m_rbkey.m_type=2;
  else if (m_type==cstp::II) m_rbkey.m_type=3;
  if (m_rbkey.m_type&1) {
    m_rbkey.m_flij=GetFlavourB();
    m_rbkey.m_fli=GetFlavourA();
  }
  else {
    m_rbkey.m_flij=GetFlavourA();
    m_rbkey.m_fli=GetFlavourB();
  }
  m_rbkey.m_flj=GetFlavourC();
}
...
{
  double lastint = p_lf->OverIntegrated(zmin,zmax,scale,xbj)/m_symf/m_polfac*m_rbmax;
...
void Splitting_Function_Base::SetRB(ATOOLS::RB_Map *const rbmap)
{
  m_rbmax=1.0;
  if (rbmap==NULL) return;
  RB_Map::const_iterator rbit(rbmap->find(m_rbkey));
  if (rbit!=rbmap->end()) m_rbmax=rbit->second->m_max;
  else if ((m_rbkey.m_type&1)==0) {
    rbit=rbmap->find(m_rbkey.SwapIJ());
    if (rbit!=rbmap->end()) m_rbmax=rbit->second->m_max;
  }
}
