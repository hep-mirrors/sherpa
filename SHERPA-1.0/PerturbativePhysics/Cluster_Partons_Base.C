...
Cluster_Partons_Base::Cluster_Partons_Base(Matrix_Element_Handler * me, Jet_Finder * jf,
					   int maxjetnumber, int isrmode,int isrshoweron, int fsrshoweron) :
  p_ct(NULL), p_combi(NULL), p_sud(NULL), p_issud(NULL), 
  p_me(me), p_jf(jf), p_runas(NULL), 
  m_maxjetnumber(maxjetnumber),
  m_sud_mode(0), m_isrmode(isrmode), m_isrshoweron(isrshoweron), 
  m_fsrshoweron(fsrshoweron), m_kfac(0.), p_local_tree(NULL) 
{
  ...
  msg_Tracking()<<"Cluster_Partons_Base runs in mode : \n"
		<<"   SUDAKOVTYPE    = "<<m_bp_mode<<"\n"
		<<"   SUDAKOVASORDER = "<<m_as_order<<"\n"
		<<"   SUDAKOVASFAC   = "<<as_fac<<"\n"
		<<"   CALCJETRATE    = "<<jetratemode<<"\n"
		<<"   kfac           = "<<m_kfac<<std::endl;
  if (false) {//(m_as_order!=-1 || as_fac!=1.) {
    PRINT_INFO("new run as "<<m_as_order<<" "<<as_fac);
  ...
  if (m_isrshoweron!=0 || m_fsrshoweron!=0) {
    ...
    p_issud = new NLL_Sudakov(m_bp_mode|BP::is_mode,p_jf->Smax(),p_jf->Smin(),p_runas,jetratemode);
    m_sud_mode = 2;  
  }
  ...
  m_q2_jet = rpa.gen.RenormalizationScaleFactor() * sqr(Min(m_qmin_i,m_qmin_f));
}
...
int Cluster_Partons_Base::SetColours(Vec4D * p, Flavour * fl)
{
  ...
  m_q2_qcd = m_q2_hard = s;

  if (ncol==4) {
    msg.Out()<<"WARNING in Cluster_Partons_Base::SetColours() : called for 4 coloured objects \n"
  ...
  if (ncol==3 && nquark==2 && ngluon==1) {
    ...
    //    m_q2_qcd = 0.5*(sqr(p[2][1])+sqr(p[2][2])+sqr(p[3][1])+sqr(p[3][2]));
    m_q2_qcd = 0.;
    if (fl[2].Strong()) m_q2_qcd+=sqr(p[2][1])+sqr(p[2][2]);
    if (fl[3].Strong()) m_q2_qcd+=sqr(p[3][1])+sqr(p[3][2]);
    ...
    //    m_q2_qcd = 0.5*(sqr(q[2][1])+sqr(q[2][2])+sqr(q[3][1])+sqr(q[3][2]));

    // --- end ---
    m_q2_hard  = 0.5*(sqr(q[2][1])+sqr(q[2][2])+sqr(q[3][1])+sqr(q[3][2]));
    m_q2_hard += p[2].Abs2()+p[3].Abs2();
    //    m_q2_qcd = Min(dabs(t),dabs(u));
  ...
  if (!test && ncol==2) {
    ...
    //         mass effects ?!
    if (fl[0].Strong()) {
      if (fl[2].Strong())      m_q2_qcd = m_q2_hard = dabs(t);
      else if (fl[3].Strong()) m_q2_qcd = m_q2_hard = dabs(u);
    }
    else if (fl[1].Strong()) {
      if (fl[2].Strong())      m_q2_qcd = m_q2_hard = dabs(u);
      else if (fl[3].Strong()) m_q2_qcd = m_q2_hard = dabs(t);
    }
  }
...
void Cluster_Partons_Base::FillDecayTree(Tree * fin_tree,XS_Base * xs)
{
  if (!fin_tree && m_fsrshoweron) {
    msg.Error()<<"ERROR in Cluster_Partons_Base::FillDecayTrees: no trees!"
	       <<" No shower to be performed! "<<std::endl;
    return;
  } 
  if (p_blob->NInP()!=1 || p_blob->NOutP()!=2) {
    msg.Error()<<"ERROR in Cluster_Partons_Base::FillDecayTrees: "<<std::endl
...
void Cluster_Partons_Base::FixJetvetoPt2(double & jetvetopt2)
{
  double pt2min=p_ct->MinKt2();
  if (pt2min>0.0 && pt2min<std::numeric_limits<double>::max()) jetvetopt2=pt2min;
  else {
    msg.Error()<<"Cluster_Partons_Base::FixJetvetoPt2(..): No minimum scale found."<<std::endl;
  }
}


void Cluster_Partons_Base::FillTrees(Tree ** ini_trees,Tree * fin_tree)
{
  if ((!ini_trees && m_isrshoweron) || (!fin_tree && m_fsrshoweron)) {
    msg.Error()<<"ERROR in Cluster_Partons_Base::FillTrees:"<<std::endl
  ...
  *(mo->part) = Particle(0,Flavour(kf::none),sum);
  ...
  double scale = HardScale();
  if (p_xs) scale = p_xs->Scale(PHASIC::stp::fac);
  ...
  Combine_Table_Base * ct_tmp = p_ct;
  ...
  if (msg.LevelIsDebugging()) {
    msg.Out()<<" in Cluster_Partons_Base::FillTrees("<<m_isrshoweron<<","
...
Knot * Cluster_Partons_Base::Point2Knot(Tree * tree, const Leg & po, 
...
Vec4D Cluster_Partons_Base::Momentum(Knot * mo, int & number) {
...
bool Cluster_Partons_Base::IsColourConnected(Particle * a, Particle * b) 
{
...
double Cluster_Partons_Base::ColourAngle(const std::vector<Knot *> & knots, const int i) 
{
  ...
  for (int j=start;j<(int)knots.size();++j) {
	...
	//	std::cout<<" angle("<<i<<","<<j<<")="<<angle<<std::endl;
...
void Cluster_Partons_Base::DetermineColourAngles(const std::vector<APACIC::Knot *> & knots) 
{
  int n=knots.size();
  // save momenta
  ...
  //  std::cout<<" Cluster_Partons_Base::DetermineColourAngles \n";
...
{ 
  ...
  if (m_njet==m_maxjetnumber) FixJetvetoPt2(qmin2);
  q2i = Max(qmin2,sqr(m_qmin_i));
  q2f = Max(qmin2,sqr(m_qmin_f)); 
}

void    Cluster_Partons_Base::EstablishRelations(APACIC::Knot * mo,APACIC::Knot * d1,APACIC::Knot * d2,
