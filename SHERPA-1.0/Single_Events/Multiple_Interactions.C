...
bool Multiple_Interactions::CheckBlobList(ATOOLS::Blob_List *const bloblist) 
{
  p_bloblist=bloblist;
  m_diced=false;
  double ptmax=m_ptmax=std::numeric_limits<double>::max();
  ...
  for (ATOOLS::Blob_List::const_iterator bit=bloblist->begin();
	...
	ptmax=ATOOLS::Min(ptmax,(*bit)->InParticle(i)->Momentum().PPerp());
      if (!m_diced && VetoHardProcess(*bit)) {
	m_ptmax=0.0;
	break;
      }
    }
  ...
  if (ptmax>=m_ptmax) {
	...
	for (int i=0;i<(*bit)->NOutP();++i) 
	  ptmax=ATOOLS::Min(ptmax,(*bit)->OutParticle(i)->Momentum().PPerp());
	for (int i=0;i<(*bit)->NInP();++i) {
	  p_mihandler->ISRHandler()->
  ...
  return (m_ptmax=ptmax)!=std::numeric_limits<double>::max();
}
...
bool Multiple_Interactions::Treat(ATOOLS::Blob_List *bloblist,double &weight)
{
  ...
  p_mihandler->SetScaleMax(ATOOLS::Min(m_ptmax,m_lastptmax),0);
  ...
  if (success) {
	...
	m_lastptmax=blob->OutParticle(0)->Momentum().PPerp();
	std::cout<<"MI: vetoed "<<m_diced<<" "<<m_lastptmax<<"\n";
	delete blob;
	return true;
      }
    }
