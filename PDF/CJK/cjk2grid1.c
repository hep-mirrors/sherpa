/* cjk2grid1.f -- translated by f2c (version 20200916).
...
#define flavv2_1 flavv2_

Extern struct {
    integer ist;
} istv2_;

#define istv2_1 istv2_

Extern struct {
    integer iread;
} ireadv2_;

#define ireadv2_1 ireadv2_
...
/* ******************************************************************** */
/* ***     The same parametrization as in cjk2grid.f but for       **** */
...
/* ***             photon for the CJK 2 fit based on               **** */
/* ***                                                             **** */
...
/* ***      any observable X(parton densities), obtained in        **** */
/* ***                                                             **** */
/* ***                       P.Jankowski                           **** */
...
/* ***                                                             **** */
/* ***   ISET = 0 : bestfit S0 - "cjk2best.dat"                    **** */
/* ***                                                             **** */
...
/* ***                                                             **** */
/* ***   ISET = 1 : S1+  - "cjk2set1pl.dat"                        **** */
/* ***   ISET = 2 : S1-  - "cjk2set1mn.dat"                        **** */
/* ***   ISET = 3 : S2+  - "cjk2set2pl.dat"                        **** */
/* ***   ISET = 4 : S2-  - "cjk2set2mn.dat"                        **** */
/* ***   ISET = 5 : S3+  - "cjk2set3pl.dat"                        **** */
/* ***   ISET = 6 : S3-  - "cjk2set3mn.dat"                        **** */
/* ***   ISET = 7 : S4+  - "cjk2set4pl.dat"                        **** */
/* ***   ISET = 8 : S4-  - "cjk2set4mn.dat"                        **** */
/* ***                                                             **** */
/* ******************************************************************** */
...
/* ******************************************************************** */
/*<       SUBROUTINE CJK2GRID(ISET,IOPT,X,XC,XB,Q2,XPDF,F2alfa) >*/
/* Subroutine */ int cjk2grid_(integer *iset, integer *iopt, doublereal *x, 
	...
	doublereal *f2alfa)
{
    ...
    /* Local variables */
    static doublereal i__;
    static integer nf;
    static doublereal xx, mb2, mc2, qq2, mu2, xdn, xxb, xxc, xup, xchm, xbot, 
	    xglu, xstr;
    extern /* Subroutine */ int gridv2_(integer *, doublereal *, doublereal *,
...
/*<       INTEGER flav,Nf,step,IOPT,ISET,IST >*/
/*<        >*/
/*<       logical t >*/
...
/*<       if (IOPT.EQ.1) then >*/
    if (*iopt == 1) {
/*<          call GRIDV2(1,XX,XXC,XXB,QQ2,XGLU,XDN,XUP,XSTR,XCHM,XBOT) >*/
	gridv2_(&c__1, &xx, &xxc, &xxb, &qq2, &xglu, &xdn, &xup, &xstr, &xchm,
		 &xbot);
/*<          XPDF(0) = xglu >*/
	xpdf[0] = xglu;
/*<          XPDF(1) = xdn >*/
...
/*<       if (IOPT.EQ.2) then >*/
    if (*iopt == 2) {
/*<          call GRIDV2(2,XX,XXC,XXB,QQ2,XGLU,XDN,XUP,XSTR,XCHM,XBOT) >*/
	gridv2_(&c__2, &xx, &xxc, &xxb, &qq2, &xglu, &xdn, &xup, &xstr, &xchm,
		 &xbot);
/*<          XPDF(0) = xglu >*/
	xpdf[0] = xglu;
/*<          XPDF(1) = xdn >*/
...
/* ******************************************************************** */
/*<        >*/
/* Subroutine */ int gridv2_(integer *iopt, doublereal *xin, doublereal *xcin,
	...
	xbot)
{
    ...
    /* Local variables */
    extern /* Subroutine */ int findxhv2_(doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, integer *, integer *, 
	    integer *, doublereal *, doublereal *), readtabv2_(integer *);
    ...
    extern /* Subroutine */ int fitv2_(integer *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *);
    static integer bordb, bordc;
    static doublereal xmaxb, xmaxc;
    extern /* Subroutine */ int findxv2_(doublereal *, doublereal *, integer *
	    ), findq2v2_(integer *, doublereal *, doublereal *, integer *);
...
/*<       common /IREADV2/ IREAD >*/
/*<       common /ISTV2/ IST >*/
/*<       common /PARTV2/ gl,dn,up,st,ch,bt,chx,btx >*/
/*<        >*/
/*<        >*/
/*<        >*/
/*<        >*/
...
/*<       NQQ2 = NQ2 >*/
    nqq2 = 32;
/*<       call findq2v2(NQQ2,q2data,QQ2,J) >*/
    findq2v2_(&nqq2, q2data, &qq2, &j);
/*<       call findxv2(xdata,XX,I) >*/
    findxv2_(xdata, &xx, &i__);
...
/* L20: */
		glh[m + (n << 2) - 5] = partv2_1.gl[istv2_1.ist + (k + l * 34)
			 * 9];
	    }
	}
/*<          call fitv2(bord,QQ2,XX,q2,x,glh,xglu) >*/
	fitv2_(&bord, &qq2, &xx, q2, x, glh, xglu);
/*<          xglu = alfa*xglu >*/
	*xglu *= .00729735308;
/*<       else >*/
    } else {
...
/*<                glh(M,N) = gl(IST,K,L) >*/
		glh[m + (n << 2) - 5] = partv2_1.gl[istv2_1.ist + (k + l * 34)
			 * 9];
/*<                dnh(M,N) = dn(IST,K,L) >*/
		dnh[m + (n << 2) - 5] = partv2_1.dn[istv2_1.ist + (k + l * 34)
			 * 9];
/*<                uph(M,N) = up(IST,K,L) >*/
		uph[m + (n << 2) - 5] = partv2_1.up[istv2_1.ist + (k + l * 34)
			 * 9];
/*<  21            sth(M,N) = st(IST,K,L) >*/
/* L21: */
		sth[m + (n << 2) - 5] = partv2_1.st[istv2_1.ist + (k + l * 34)
			 * 9];
	    }
	}
/*<          call fitv2(bord,QQ2,XX,q2,x,glh,xglu) >*/
	fitv2_(&bord, &qq2, &xx, q2, x, glh, xglu);
/*<          call fitv2(bord,QQ2,XX,q2,x,dnh,xdn) >*/
	fitv2_(&bord, &qq2, &xx, q2, x, dnh, xdn);
/*<          call fitv2(bord,QQ2,XX,q2,x,uph,xup) >*/
	fitv2_(&bord, &qq2, &xx, q2, x, uph, xup);
/*<          call fitv2(bord,QQ2,XX,q2,x,sth,xstr) >*/
	fitv2_(&bord, &qq2, &xx, q2, x, sth, xstr);
/*<          xglu = alfa*xglu >*/
	*xglu *= .00729735308;
/*<          xdn  = alfa*xdn >*/
	*xdn *= .00729735308;
...
/*<       NQQ2H = NQ2H >*/
    nqq2h = 48;
/*<       call findq2v2(NQQ2H,q2hdata,QQ2,JH) >*/
    findq2v2_(&nqq2h, q2hdata, &qq2, &jh);
...
/*<          call findxhv2(xmxc,xdata,ch,chx,XXC,Imaxc(JH),JH,bordc,xc,chh) >*/
	findxhv2_(&xmxc, xdata, partv2_1.ch, partv2_1.chx, &xxc, &imaxc[jh - 
		1], &jh, &bordc, xc, chh);
/*<          call fitv2(bordc,QQ2,XXC,q2h,xc,chh,xchm) >*/
	fitv2_(&bordc, &qq2, &xxc, q2h, xc, chh, xchm);
...
/*<          call findxhv2(xmxb,xdata,bt,btx,XXB,Imaxb(JH),JH,bordb,xb,bth) >*/
	findxhv2_(&xmxb, xdata, partv2_1.bt, partv2_1.btx, &xxb, &imaxb[jh - 
		1], &jh, &bordb, xb, bth);
/*<          call fitv2(bordb,QQ2,XXB,q2h,xb,bth,xbot) >*/
	fitv2_(&bordb, &qq2, &xxb, q2h, xb, bth, xbot);
...
/* **************************************************************** */
/*<       SUBROUTINE READTABV2(IOPT) >*/
/* Subroutine */ int readtabv2_(integer *iopt)
{
    /* Format strings */
...
/*<       do 2000 IST = 0,8 >*/
    for (ist = 0; ist <= 8; ++ist) {
/*<          if (IST.EQ.0) name = 'cjk2best.dat' >*/
	if (ist == 0) {
	    s_copy(name__, "cjk2best.dat", (ftnlen)15, (ftnlen)12);
	}
/*<          if (IST.EQ.1) name = 'cjk2set1pl.dat' >*/
	if (ist == 1) {
	    s_copy(name__, "cjk2set1pl.dat", (ftnlen)15, (ftnlen)14);
	}
/*<          if (IST.EQ.2) name = 'cjk2set1mn.dat' >*/
	if (ist == 2) {
	    s_copy(name__, "cjk2set1mn.dat", (ftnlen)15, (ftnlen)14);
	}
/*<          if (IST.EQ.3) name = 'cjk2set2pl.dat' >*/
	if (ist == 3) {
	    s_copy(name__, "cjk2set2pl.dat", (ftnlen)15, (ftnlen)14);
	}
/*<          if (IST.EQ.4) name = 'cjk2set2mn.dat' >*/
	if (ist == 4) {
	    s_copy(name__, "cjk2set2mn.dat", (ftnlen)15, (ftnlen)14);
	}
/*<          if (IST.EQ.5) name = 'cjk2set3pl.dat' >*/
	if (ist == 5) {
	    s_copy(name__, "cjk2set3pl.dat", (ftnlen)15, (ftnlen)14);
	}
/*<          if (IST.EQ.6) name = 'cjk2set3mn.dat' >*/
	if (ist == 6) {
	    s_copy(name__, "cjk2set3mn.dat", (ftnlen)15, (ftnlen)14);
	}
/*<          if (IST.EQ.7) name = 'cjk2set4pl.dat' >*/
	if (ist == 7) {
	    s_copy(name__, "cjk2set4pl.dat", (ftnlen)15, (ftnlen)14);
	}
/*<          if (IST.EQ.8) name = 'cjk2set4mn.dat' >*/
	if (ist == 8) {
	    s_copy(name__, "cjk2set4mn.dat", (ftnlen)15, (ftnlen)14);
	}
...
/*<             do 1 J=0,NX+1 >*/
	    for (j = 0; j <= 53; ++j) {
/*<                gl(IST,I,J) = 0.d0 >*/
		partv2_1.gl[ist + (i__ + j * 34) * 9] = 0.;
/*<                dn(IST,I,J) = 0.d0 >*/
		partv2_1.dn[ist + (i__ + j * 34) * 9] = 0.;
/*<                up(IST,I,J) = 0.d0 >*/
		partv2_1.up[ist + (i__ + j * 34) * 9] = 0.;
/*<  1             st(IST,I,J) = 0.d0 >*/
/* L1: */
		partv2_1.st[ist + (i__ + j * 34) * 9] = 0.;
	    }
	}
/*<          do 10 I=1,NQ2 >*/
...
/* L10: */
		s_rsfe(&io___62);
		do_fio(&c__1, (char *)&partv2_1.gl[ist + (i__ + j * 34) * 9], 
			(ftnlen)sizeof(doublereal));
		do_fio(&c__1, (char *)&partv2_1.dn[ist + (i__ + j * 34) * 9], 
			(ftnlen)sizeof(doublereal));
		do_fio(&c__1, (char *)&partv2_1.up[ist + (i__ + j * 34) * 9], 
			(ftnlen)sizeof(doublereal));
		do_fio(&c__1, (char *)&partv2_1.st[ist + (i__ + j * 34) * 9], 
			(ftnlen)sizeof(doublereal));
		e_rsfe();
	    }
	}
...
/*<             do 2 J=0,NX+1 >*/
	    for (j = 0; j <= 53; ++j) {
/*<                ch(IST,I,J) = 0.d0 >*/
		partv2_1.ch[ist + (i__ + j * 50) * 9] = 0.;
/*<  2             bt(IST,I,J) = 0.d0 >*/
/* L2: */
		partv2_1.bt[ist + (i__ + j * 50) * 9] = 0.;
	    }
	}
/*<          do 20 I=1,NQ2H >*/
...
/* L20: */
		s_rsfe(&io___63);
		do_fio(&c__1, (char *)&partv2_1.ch[ist + (i__ + j * 50) * 9], 
			(ftnlen)sizeof(doublereal));
		do_fio(&c__1, (char *)&partv2_1.bt[ist + (i__ + j * 50) * 9], 
			(ftnlen)sizeof(doublereal));
		e_rsfe();
	    }
	}
...
/*<             read(10,300) (chx(IST,I,J),J=1,24) >*/
	    s_rsfe(&io___64);
	    for (j = 1; j <= 24; ++j) {
		do_fio(&c__1, (char *)&partv2_1.chx[ist + (i__ + j * 50) * 9 
...
/*<             read(10,300) (btx(IST,I,J),J=1,24) >*/
	    s_rsfe(&io___65);
	    for (j = 1; j <= 24; ++j) {
		do_fio(&c__1, (char *)&partv2_1.btx[ist + (i__ + j * 50) * 9 
			- 450], (ftnlen)sizeof(doublereal));
	    }
	    e_rsfe();
/*<  40      continue >*/
...
/*<             do 31 J=1,24 >*/
	    for (j = 1; j <= 24; ++j) {
/*<                chx(IST,I,J) = 0.d0 >*/
		partv2_1.chx[ist + (i__ + j * 50) * 9 - 450] = 0.;
/*<  31            btx(IST,I,J) = 0.d0 >*/
/* L31: */
		partv2_1.btx[ist + (i__ + j * 50) * 9 - 450] = 0.;
	    }
	}
/*<  1000    continue >*/
L1000:
...
/* ******************************************************************** */
/*<       SUBROUTINE FINDQ2V2(NQ2,q2data,QQ2,I) >*/
/* Subroutine */ int findq2v2_(integer *nq2, doublereal *q2data, doublereal *
	qq2, integer *i__)
{
...
/* ***            Here I use the bisection method                  **** */
/* ******************************************************************** */
/*<       SUBROUTINE FINDXV2(xdata,XX,I) >*/
/* Subroutine */ int findxv2_(doublereal *xdata, doublereal *xx, integer *i__)
{
    static doublereal x;
    static integer il, iu;
...
/* ***            Here I use the bisection method                  **** */
/* ******************************************************************** */
/*<       SUBROUTINE FINDXHV2(xmax,xdata,hq,hqx,XX,Imax,J,bord,x,hqh) >*/
/* Subroutine */ int findxhv2_(doublereal *xmax, doublereal *xdata, 
...
/* L30: */
		    hqh[m + 2 - *j + (l + 2 - i__ << 2)] = hq[istv2_1.ist + (
			    m + l * 50) * 9];
		}
	    }
/*<          else >*/
...
/*<                hqh(N,4) = hqx(IST,M,6*N-5) >*/
		hqh[n + 16] = hqx[istv2_1.ist + (m + (n * 6 - 5) * 50) * 9];
/*<                do 40 L=I-1,I+1 >*/
		i__1 = i__ + 1;
		for (l = i__ - 1; l <= i__1; ++l) {
/*<  40               hqh(N,L+2-I) = hq(IST,M,L) >*/
/* L40: */
		    hqh[n + (l + 2 - i__ << 2)] = hq[istv2_1.ist + (m + l * 
			    50) * 9];
		}
	    }
/*<          endif >*/
...
/*<             hqh(N,1) = hq(IST,M,Imax-1) >*/
	    hqh[n + 4] = hq[istv2_1.ist + (m + (*imax - 1) * 50) * 9];
/*<             hqh(N,2) = hq(IST,M,Imax) >*/
	    hqh[n + 8] = hq[istv2_1.ist + (m + *imax * 50) * 9];
/*<             hqh(N,3) = hqx(IST,M,6*N-5) >*/
	    hqh[n + 12] = hqx[istv2_1.ist + (m + (n * 6 - 5) * 50) * 9];
/*<  50         hqh(N,4) = hqx(IST,M,6*N-4) >*/
/* L50: */
	    hqh[n + 16] = hqx[istv2_1.ist + (m + (n * 6 - 4) * 50) * 9];
	}
/*<       elseif (K.EQ.2) then >*/
    } else if (k == 2) {
/*<          do 60 M=J-1,J+2 >*/
...
/*<             hqh(N,1) = hq(IST,M,Imax) >*/
	    hqh[n + 4] = hq[istv2_1.ist + (m + *imax * 50) * 9];
/*<             do 60 I=2,4 >*/
	    for (i__ = 2; i__ <= 4; ++i__) {
/*<  60            hqh(N,I) = hqx(IST,M,6*N+I-7) >*/
/* L60: */
		hqh[n + (i__ << 2)] = hqx[istv2_1.ist + (m + (n * 6 + i__ - 7)
			 * 50) * 9];
	    }
	}
...
/* L70: */
		hqh[n + (i__ << 2)] = hqx[istv2_1.ist + (m + (n * 6 + i__ - 6)
			 * 50) * 9];
	    }
	}
...
/* L80: */
		hqh[n + (i__ << 2)] = hqx[istv2_1.ist + (m + (n * 6 + i__ - 5)
			 * 50) * 9];
	    }
	}
/*<          if (hqh(1,3).LT.1.d-10.OR.hqh(2,4).LT.1.d-10) bord = 0 >*/
...
/* L90: */
		hqh[n + (i__ << 2)] = hqx[istv2_1.ist + (m + (n * 6 + i__ - 4)
			 * 50) * 9];
	    }
	}
/*<          if (hqh(1,3).LT.1.d-10.OR.hqh(2,4).LT.1.d-10) bord = 0 >*/
...
/* L100: */
		hqh[n + (i__ << 2)] = hqx[istv2_1.ist + (m + (n * 6 + i__ - 3)
			 * 50) * 9];
	    }
	}
/*<       endif >*/
...
/* ******************************************************************** */
/*<       SUBROUTINE FITV2(bord,xx1,xx2,x1,x2,yg,result) >*/
/* Subroutine */ int fitv2_(integer *bord, doublereal *xx1, doublereal *xx2, 
	doublereal *x1, doublereal *x2, doublereal *yg, doublereal *result)
{
    static doublereal y[4], y1[4], y2[4], y12[4], x1l, x2l, x1u, x2u;
    extern doublereal d1fv2_(doublereal *, doublereal *, doublereal *, 
	    doublereal *), d2fv2_(doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *);
    extern /* Subroutine */ int iterv2_(doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *, doublereal *, doublereal *, doublereal *, 
	    doublereal *);
...
/*<       if (bord.EQ.1) then >*/
    if (*bord == 1) {
/*<          y1(1) = d1fv2(x1(1),x1(3),yg(1,2),yg(3,2)) >*/
	y1[0] = d1fv2_(&x1[1], &x1[3], &yg[9], &yg[11]);
/*<          y1(2) = d1fv2(x1(2),x1(4),yg(2,2),yg(4,2)) >*/
	y1[1] = d1fv2_(&x1[2], &x1[4], &yg[10], &yg[12]);
/*<          y1(3) = d1fv2(x1(2),x1(4),yg(2,3),yg(4,3)) >*/
	y1[2] = d1fv2_(&x1[2], &x1[4], &yg[14], &yg[16]);
/*<          y1(4) = d1fv2(x1(1),x1(3),yg(1,3),yg(3,3)) >*/
	y1[3] = d1fv2_(&x1[1], &x1[3], &yg[13], &yg[15]);
/*<          y2(1) = d1fv2(x2(1),x2(3),yg(2,1),yg(2,3)) >*/
	y2[0] = d1fv2_(&x2[1], &x2[3], &yg[6], &yg[14]);
/*<          y2(2) = d1fv2(x2(1),x2(3),yg(3,1),yg(3,3)) >*/
	y2[1] = d1fv2_(&x2[1], &x2[3], &yg[7], &yg[15]);
/*<          y2(3) = d1fv2(x2(2),x2(4),yg(3,2),yg(3,4)) >*/
	y2[2] = d1fv2_(&x2[2], &x2[4], &yg[11], &yg[19]);
/*<          y2(4) = d1fv2(x2(2),x2(4),yg(2,2),yg(2,4)) >*/
	y2[3] = d1fv2_(&x2[2], &x2[4], &yg[10], &yg[18]);
/*<        >*/
	y12[0] = d2fv2_(&x1[1], &x1[3], &x2[1], &x2[3], &yg[5], &yg[13], &yg[
		7], &yg[15]);
/*<        >*/
	y12[1] = d2fv2_(&x1[2], &x1[4], &x2[1], &x2[3], &yg[6], &yg[14], &yg[
		8], &yg[16]);
/*<        >*/
	y12[2] = d2fv2_(&x1[2], &x1[4], &x2[2], &x2[4], &yg[10], &yg[18], &yg[
		12], &yg[20]);
/*<        >*/
	y12[3] = d2fv2_(&x1[1], &x1[3], &x2[2], &x2[4], &yg[9], &yg[17], &yg[
		11], &yg[19]);
/*<       else >*/
    } else {
/*<          y1(1) = d1fv2(x1(2),x1(3),yg(2,2),yg(3,2)) >*/
	y1[0] = d1fv2_(&x1[2], &x1[3], &yg[10], &yg[11]);
/*<          y1(2) = y1(1) >*/
	y1[1] = y1[0];
/*<          y1(3) = d1fv2(x1(2),x1(3),yg(2,3),yg(3,3)) >*/
	y1[2] = d1fv2_(&x1[2], &x1[3], &yg[14], &yg[15]);
/*<          y1(4) = y1(3) >*/
	y1[3] = y1[2];
/*<          y2(1) = d1fv2(x2(2),x2(3),yg(2,2),yg(2,3)) >*/
	y2[0] = d1fv2_(&x2[2], &x2[3], &yg[10], &yg[14]);
/*<          y2(2) = d1fv2(x2(2),x2(3),yg(3,2),yg(3,3)) >*/
	y2[1] = d1fv2_(&x2[2], &x2[3], &yg[11], &yg[15]);
/*<          y2(3) = y2(2) >*/
	y2[2] = y2[1];
/*<          y2(4) = y2(1) >*/
	y2[3] = y2[0];
/*<        >*/
	y12[0] = d2fv2_(&x1[2], &x1[3], &x2[2], &x2[3], &yg[10], &yg[14], &yg[
		11], &yg[15]);
/*<          y12(2) = y12(1) >*/
	y12[1] = y12[0];
/*<          y12(3) = y12(1) >*/
...
/*<       call iterv2(y,y1,y2,y12,x1l,x1u,x2l,x2u,xx1,xx2,result) >*/
    iterv2_(y, y1, y2, y12, &x1l, &x1u, &x2l, &x2u, xx1, xx2, result);
/*<       RETURN >*/
    return 0;
/*<       END >*/
} /* fitv2_ */
...
/* ******************************************************************** */
/*<       SUBROUTINE iterv2(y,y1,y2,y12,x1l,x1u,x2l,x2u,x1,x2,res) >*/
/* Subroutine */ int iterv2_(doublereal *y, doublereal *y1, doublereal *y2, 
...
/* ******************************************************************** */
/*<       DOUBLE PRECISION FUNCTION d1fv2(x1,x2,f1,f2) >*/
doublereal d1fv2_(doublereal *x1, doublereal *x2, doublereal *f1, doublereal *
	f2)
{
    /* System generated locals */
    doublereal ret_val;
...
/* ******************************************************************** */
/*<       DOUBLE PRECISION FUNCTION d2fv2(x1,x2,y1,y2,f11,f12,f21,f22) >*/
doublereal d2fv2_(doublereal *x1, doublereal *x2, doublereal *y1, doublereal *
...
/*<       IMPLICIT DOUBLE PRECISION (a-z) >*/
/*<       d2fv2 = (f22 - f21 - f12 + f11)/((x2-x1)*(y2-y1)) >*/
