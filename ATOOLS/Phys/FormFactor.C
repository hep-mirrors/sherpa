...
FormFactor::FormFactor() {
  ...
  m_debug = s["Debug"].SetDefault(false).Get<bool>();
  m_xmin = s["XMIN"].SetDefault(0.1).Get<double>();
  m_xmax = s["XMAX"].SetDefault(10).Get<double>();
  m_cphi = 0.00045;
  ...
  // if(m_debug){
  //   std::string filename =
  //     std::string("FormFactor_")
  //     + to_string(m_form_mode)
  //     + ".dat";
...
void FormFactor::RegisterDefaultsKaon() {
  ...
  m_crhoppp =
      s["c_rhoppp"].SetDefault(1 - m_crho - m_crhop - m_crhopp).Get<double>();
}
...
Complex FormFactor::BW_GS(const double &q2, const Flavour &fl) {
  ...
  if (IsBad(num) || IsBad(den)) {
    msg_Error() << "NaN in " << METHOD << std::endl
                ...
                << "Final State Width = " << m_flv.Width(true) << std::endl
                << "den = " << num << std::endl
                << "mf = " << mf << std::endl
                << "wf = " << wf << std::endl
                << "d(mf) = " << d(mf) << std::endl
  ...
  msg_Debugging() << " Evaluating Form Factor at scale Q^2 = " << q2
                  << std::endl
                  << "Resonance Flavour = " << fl << std::endl
                  << "mf = " << mf << std::endl
                  << "wf = " << wf << std::endl
                  << "d(mf) = " << d(mf) << std::endl
                  << "f(q2,mf,wf) = " << f(q2, mf, wf) << std::endl
...
Complex FormFactor::KPlus(const double &q2) {
  Complex Form =
      0.5 * (m_crho * BW_GS(q2, m_rho) + m_crhop * BW_GS(q2, m_rhop) +
             m_crhopp * BW_GS(q2, m_rhopp) + m_crhoppp * BW_GS(q2, m_rhoppp));
  Form += 1. / 6. *
...
double FormFactor::Eval(const double &q2) {
  ...
  if (!m_initialize) {
    ...
    m_rhopi = Particle_Info(kf_rho_1450, m_rhop_mass, 0.0, m_rhop_g, 3, 0, 0, 0,
                            0, 0, 1, "rhopp", "rhopp", "rho", "rho");

    m_rhoppi = Particle_Info(kf_rho_1700, m_rhopp_mass, 0.0, m_rhopp_g, 3, 0, 0,
                             0, 0, 0, 1, "rho pp", "rho", "rho", "rho");

    m_rhopppi = Particle_Info(kf_rho_2150, m_rhoppp_mass, 0.0, m_rhoppp_g, 3, 0,
                              0, 0, 0, 0, 1, "rho ppp", "rho", "rho", "rho");

    m_omegai = Particle_Info(kf_omega_782, m_omega_mass, 0.0, m_omega_g, 3, 0,
                             0, 0, 0, 0, 1, "omega", "omega", "omega", "omega");
    m_omegapi =
        Particle_Info(kf_omega_782, m_omegap_mass, 0.0, m_omegap_g, 3, 0, 0, 0,
                      0, 0, 1, "omegap", "omega", "omega", "omega");
    m_omegappi =
        Particle_Info(kf_omega_782, m_omegapp_mass, 0.0, m_omegap_g, 3, 0, 0, 0,
                      0, 0, 1, "omegapp", "omega", "omega", "omega");

    m_phii = Particle_Info(kf_phi_1020, m_phi_mass, 0.0, m_phi_g, 3, 0, 0, 0, 0,
                           0, 1, "phi", "phi", "phi", "phi");
    m_phipi = Particle_Info(kf_phi_1020, m_phip_mass, 0.0, m_phip_g, 3, 0, 0, 0,
                            0, 0, 1, "phi", "phi", "phi", "phi");

    m_rho = Flavour(m_rhoi);
    m_rhop = Flavour(m_rhopi);
    m_rhopp = Flavour(m_rhoppi);
    m_rhoppp = Flavour(m_rhopppi);

    m_omega = Flavour(m_omegai);
    m_omegap = Flavour(m_omegapi);
    m_omegapp = Flavour(m_omegappi);
