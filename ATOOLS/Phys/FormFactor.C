...
void FormFactor::RegisterDefaultsKaon(){
  ...
  Complex FormFactor::f(const double &q2, const double &m, const double &g){
    Complex t1 = sqr(ppi(q2))*(h(q2)-h(m*m))+sqr(ppi(m*m))*(m*m-q2)*dh(m*m);
  ...
  Complex FormFactor::KPlus(const double &q2){
    Complex Form = 0.5*(m_crho*BW_GS(q2, m_rho) + m_crhop*BW_GS(q2, m_rhop) + m_crhopp*BW_GS(q2, m_rhopp));
    Form += 1./6.*(m_comega*BW_GS(q2, m_omega) + m_comegap*BW_GS(q2, m_omegap) + m_comegapp*BW_GS(q2, m_omegapp));
  ...
  double FormFactor::Eval(const double &q2){
    ...
    Particle_Info rhopi(kf_rho_1450, m_rhop_mass, 0.0,  m_rhop_g, 3,0,0,0,0,1,1,"rhopp","rhopp", "rho", "rho");
    
    Particle_Info rhoppi(kf_rho_1700, m_rhopp_mass, 0.0, m_rhopp_g, 3,0,0,0,0,1,1,"rho ppp","rho", "rho", "rho");
    
    Particle_Info rhopppi(kf_rho_2150, m_rhoppp_mass,  0.0, m_rhoppp_g, 3,0,0,0,0,1,1,"rho ppp","rho", "rho", "rho");
    
    Particle_Info omegai(kf_omega_782, m_omega_mass,0.0, m_omega_g, 3,0,0,0,0,1,1,"omega","omega", "omega", "omega");
    Particle_Info omegapi(kf_omega_782, m_omegap_mass,0.0, m_omegap_g, 3,0,0,0,0,1,1,"omegap","omega", "omega", "omega");
    Particle_Info omegappi(kf_omega_782, m_omegapp_mass,0.0, m_omegap_g, 3,0,0,0,0,1,1,"omegapp","omega", "omega", "omega");
    
    Particle_Info phii(kf_phi_1020, m_phi_mass,  0.0, m_phi_g,3,0,0,0,0,1,1,"phi","phi", "phi","phi" );
    Particle_Info phipi(kf_phi_1020, m_phip_mass, 0.0,m_phip_g,3,0,0,0,0,1,1,"phi","phi", "phi","phi" );
...
std::ostream &ATOOLS::operator<<(std::ostream &str,const finalstate::code &fs)
{
  if      (fs==finalstate::pion)    return str<<"Pi+Pi-";
  else if (fs==finalstate::kplus)    return str<<"K+K-";
  else if (fs==finalstate::off)    return str<<"Off";
  return str<<"unknown";
}
...
std::istream &ATOOLS::operator>>(std::istream &str, finalstate::code &mode)
{
  ...
  str>>tag;
  // mode=wgt::off;
  if      (tag.find("Pion")!=std::string::npos)    mode=finalstate::pion;
  else if (tag.find("Kplus")!=std::string::npos)   mode=finalstate::kplus;
  else if (tag.find("Off")!=std::string::npos)   mode=finalstate::off;
  else if (tag.find("None")!=std::string::npos)   mode=finalstate::off;
  else THROW(fatal_error, "Unknown Form_Factor: Mode ");
  return str;
}