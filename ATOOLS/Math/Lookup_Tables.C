...
size_t axis::bin(const double & x) const {
  ...
  if (dabs((x-m_xmax)/(x+m_xmax))<1.e-8*m_xstep) return m_nbins-1;
...
const double OneDim_Table::Inverse(const double & value) const {
  ...
  for (size_t i=0;i<m_x.m_nbins-1;i++) {
    val1 = m_values[i];
    val2 = m_values[i+1];
    if (val1>val2) THROW(fatal_error,"not a suitable method for non-cumulative table");
    if (value>=val1 && value<=val2) { bin = i; break; }
  }
  if ((val2-val1)/(val2+val1)<1.e-12) x = (m_x.x(bin)+m_x.x(bin+1))/2.;
  else x = (m_x.x(bin)*(val2-value)+m_x.x(bin+1)*(value-val1))/(val2-val1);
  return x;
}
...
const double OneDim_Table::Integral() const {
  double integral = 0;
  for (size_t i=0;i<m_x.m_nbins-1;i++) {
    integral += (m_x.x(i+1)-m_x.x(i))*(m_values[i]+m_values[i+1])/2.;
  }
  return integral;
}

OneDim_Table * OneDim_Table::Cumulative(const double & expo,double & integral) const {
  OneDim_Table * cumulative = new OneDim_Table(m_x);
  for (size_t i=1;i<m_x.m_nbins;i++) {
    double estimate = 0;
    if (m_x.m_mode==axis_mode::linear)
      estimate = (m_x.x(i)-m_x.x(i-1))*(m_values[i]+m_values[i-1])/2.;
    if (m_x.m_mode==axis_mode::log)
      estimate = (m_x.x(i)-m_x.x(i-1))*sqrt(m_values[i]*m_values[i-1]);
...
double TwoDim_Table::operator()(const double & x,const double & y) const {
  ...
  if (xbin==m_x.m_nbins-1 && ybin==m_y.m_nbins-1) return m_values[xbin][ybin];
  if (xbin==m_x.m_nbins-1) {
    double y1 = m_y.x(ybin), y2 = m_y.x(ybin+1);
    return ( m_values[xbin][ybin]   * (y2-y) +
	     m_values[xbin][ybin+1] * (y-y1) ) / (y2-y1);
  }
...
OneDim_Table * TwoDim_Table::Cumulative(const double & expo,const size_t & naxis,
  ...
  if (naxis==0) {
    ...
    for (size_t i=1;i<m_x.m_nbins;i++) {
      double estimate  = (m_x.x(i)-m_x.x(i-1))*(m_values[i][bin]+m_values[i-1][bin])/2.;
    ...
    for (size_t i=1;i<m_y.m_nbins;i++) {
      double estimate  = (m_y.x(i)-m_y.x(i-1))*(m_values[bin][i]+m_values[bin][i-1])/2.;
