...
Complex DiLog(const Complex& x) {
  ...
  if (imag(x) == 0.) {
    ...
    if (abs(x) < radius) {
      ...
      Complex sum(next);
      unsigned int i=1;
      // to do: smart up precision test to include less terms when possible
      while /*((real(next)/real(sum) > precision) ||
	      (imag(next)/imag(sum) > precision))*/(i < 100) {
	++i;
	prod *= x;
...
long double cyl_bessel_0 (long double x) {
  ...
  if (x <= 1) {
    ...
    r1 = ( evaluate_polynomial(sizeof(P1)/sizeof(P1[0]),P1,y) /
	   evaluate_polynomial(sizeof(Q1)/sizeof(Q1[0]),Q1,y) );
    r2 = ( evaluate_polynomial(sizeof(P2)/sizeof(P2[0]),P2,y) /
	   evaluate_polynomial(sizeof(Q2)/sizeof(Q2[0]),Q2,y) );
    factor = log(x);
    value = r1 - factor * r2;
  }
  else {
    double y = 1/x;
    r = ( evaluate_polynomial(sizeof(P3)/sizeof(P3[0]),P3,y) /
	  evaluate_polynomial(sizeof(Q3)/sizeof(Q3[0]),Q3,y) );
    factor = exp(-x)/sqrt(x);
    value = factor * r;
  }
...
double bessel_i1(double x) {
  ...
  if (xabs<3.75) {
    ...
    res = xabs * evaluate_polynomial(sizeof(P)/sizeof(P[0]),P,sqr(x/3.75));
  }
  else {
    static double P[] = {
      0.39894228,
    ...
    res = ( exp(xabs)/sqrt(xabs) *
	    evaluate_polynomial(sizeof(P)/sizeof(P[0]),P,sqr(3.75/xabs)) );
  }
...
double bessel_k0(double x) {
  if (x <= 2.0) {
    ...
    return ( -log(x/2.0)*ATOOLS::bessel_i0(x) +
	     evaluate_polynomial(sizeof(P)/sizeof(P[0]),P,x*x/4.) );
  }
  static double P[] = {
    1.25331414,
    -0.7832358e-1,
...
double bessel_k1(double x) {
  if (x <= 2.0) {
    ...
    return ( log(x/2.0)*ATOOLS::bessel_i1(x) +
	     1./x * evaluate_polynomial(sizeof(P)/sizeof(P[0]),P,x*x/4.) );
  }
  static double P[] = {
    1.25331414,
    0.23498619,
