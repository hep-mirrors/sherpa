...
namespace ATOOLS {
  ...
  TCMatrix<Scalar> operator*(const std::complex<Scalar> scal, const TCMatrix<Scalar>& in) {
    TCMatrix<Scalar> out(in.Rank());
  ...
  template<class Scalar>
  TCMatrix<Scalar> operator*(const TCMatrix<Scalar>& a,const TCMatrix<Scalar>& b) {
    if (a.Rank()!=b.Rank()) {
      msg_Error()<<"Error in "<<METHOD<<":"<<std::endl
		 <<"  Tried to multiply two matrices of different rank."<<std::endl
		 <<"  Return 0 and hope for the best."<<std::endl;
      return TCMatrix<Scalar>(0);
    }
    TCMatrix<Scalar> out(a.Rank());
...
template<class Scalar>
TCMatrix<Scalar>::TCMatrix(int _rank, bool unit) : m_rank(_rank) {
  p_m = new SComplex*[m_rank];
  for (int i=0;i<m_rank;++i) p_m[i] = new SComplex[m_rank];
  for (int i=0;i<m_rank;++i) {
    for (int j=i;j<m_rank;++j){
      p_m[i][j] = p_m[j][i] = SComplex(0.,0.);
      if (unit && i==j) p_m[i][j] = SComplex(1.0,0.);
    }
  }
}

template<class Scalar>
TCMatrix<Scalar>::TCMatrix(int _rank, SComplex value) : m_rank(_rank) {
  p_m = new SComplex*[m_rank];
  for (int i=0;i<m_rank;++i) p_m[i] = new SComplex[m_rank];
  for (int i=0;i<m_rank;++i) {
...
template<class Scalar>
TCMatrix<Scalar>::TCMatrix(SComplex** m, int rank) : p_m(m), m_rank(rank) {}

template<class Scalar>
TCMatrix<Scalar>::TCMatrix(const TCMatrix<Scalar>& _in) {
  m_rank = _in.m_rank;
  p_m = new SComplex*[m_rank];
  for (int i=0;i<m_rank;++i) p_m[i] = new SComplex[m_rank];
  for (int i=0;i<m_rank;++i) {
...
template<class Scalar>
TCMatrix<Scalar>::TCMatrix(const CMatrix& _in) {
  m_rank = _in.Rank();
  p_m = new SComplex*[m_rank];
  for (int i=0;i<m_rank;++i) p_m[i] = new SComplex[m_rank];
  for (int i=0;i<m_rank;++i) {
    for (int j=0;j<m_rank;++j) p_m[i][j] = SComplex(_in[i][j]);
    Complex a;
    Complex b(a);
  }
}
template<class Scalar>
TCMatrix<Scalar>::~TCMatrix() {
  for(short int i=0; i<m_rank; i++){
    delete[] p_m[i];
    p_m[i] = NULL;
  }
  delete[] p_m;
...
template<class Scalar>
TCMatrix<Scalar> TCMatrix<Scalar>::Conjugate(){
  TCMatrix conju = TCMatrix<Scalar>(m_rank);
  for(int i=0; i<m_rank; i++) {
...
template<class Scalar>
Vec4C TCMatrix<Scalar>::operator* (const Vec4C& cvec) {
  return Vec4C(
    std::complex<double>(p_m[0][0])*cvec[0] - std::complex<double>(p_m[0][1])*cvec[1] - std::complex<double>(p_m[0][2])*cvec[2] - std::complex<double>(p_m[0][3])*cvec[3],
    std::complex<double>(p_m[1][0])*cvec[0] - std::complex<double>(p_m[1][1])*cvec[1] - std::complex<double>(p_m[1][2])*cvec[2] - std::complex<double>(p_m[1][3])*cvec[3],
    std::complex<double>(p_m[2][0])*cvec[0] - std::complex<double>(p_m[2][1])*cvec[1] - std::complex<double>(p_m[2][2])*cvec[2] - std::complex<double>(p_m[2][3])*cvec[3],
    std::complex<double>(p_m[3][0])*cvec[0] - std::complex<double>(p_m[3][1])*cvec[1] - std::complex<double>(p_m[3][2])*cvec[2] - std::complex<double>(p_m[3][3])*cvec[3]);
}

// Add two CMatrices
template<class Scalar>
TCMatrix<Scalar> TCMatrix<Scalar>::operator+(const TCMatrix &cmatrix) const {
  if (m_rank!=cmatrix.Rank()) THROW(fatal_error, "Rank of matrices is different, can not be totaled!")
  SComplex** new_cmatrix = new SComplex*[m_rank];
  for (int i=0;i<m_rank;++i) new_cmatrix[i] = new SComplex[m_rank];
  for (int i(0); i<m_rank; ++i){
    for (int j(0); j<m_rank; ++j){
      new_cmatrix[i][j] = (*this)[i][j] + cmatrix[i][j];
    }
  }
  return TCMatrix<Scalar>(new_cmatrix, m_rank);
}
...
template<class Scalar>
TCMatrix<Scalar> TCMatrix<Scalar>::operator*(const std::complex<Scalar> scal) const {
  TCMatrix new_cmatrix(m_rank);
  for (int i(0); i<m_rank; ++i){
    for (int j(0); j<m_rank; ++j){
      new_cmatrix[i][j] = (*this)[i][j] * scal;
...
CMatrix::CMatrix(int _rank, Complex value){
  m_rank = _rank;
  p_m = new Complex*[m_rank];
  for (int i=0;i<m_rank;++i) p_m[i] = new Complex[m_rank];
  for (int i=0;i<m_rank;++i) {
...
CMatrix::CMatrix(Complex** m, int rank){
  p_m = m;
  m_rank = rank;
}
...
namespace ATOOLS {
  template class DDCMatrix;
  template DDCMatrix operator*(const std::complex<double> scal, const DDCMatrix& in);
  template DDCMatrix operator*(const DDCMatrix& a,const DDCMatrix& b);
  template class QDCMatrix;
  template QDCMatrix operator*(const std::complex<long double> scal, const QDCMatrix& in);
  template QDCMatrix operator*(const QDCMatrix& a,const QDCMatrix& b);
}
