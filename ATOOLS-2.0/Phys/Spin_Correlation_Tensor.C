...
namespace ATOOLS{
  ...
  Spin_Correlation_Tensor::Spin_Correlation_Tensor(std::vector<int>* particles, 
    ...
    p_next(NULL)
  {
    if (pPos<particles->size()) {
      ...
      size_t add = 1<<pPos;
      (*p_next)[0] = new Spin_Correlation_Tensor(particles,Amplitudes,pPos+1,aPos1, aPos2);
      (*p_next)[1] = new Spin_Correlation_Tensor(particles,Amplitudes,pPos+1,aPos1, aPos2+add);
      (*p_next)[2] = new Spin_Correlation_Tensor(particles,Amplitudes,pPos+1,aPos1+add, aPos2);
      (*p_next)[3] = new Spin_Correlation_Tensor(particles,Amplitudes,pPos+1,aPos1+add,aPos2+add);
    } else {
      // Set the values MM*
      m_particle = -1;
