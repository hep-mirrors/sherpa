...
Jet_Observable_Base::Jet_Observable_Base(unsigned int mode,double xmin,double xmax,int nbins,
					 unsigned int minn,unsigned int maxn, const std::string & lname) :
  Primitive_Observable_Base(00,xmin,xmax,nbins,NULL), m_mode(mode), m_minn(minn), m_maxn(maxn)
{
  m_listname=lname;
...
Jet_Eta_Distribution::Jet_Eta_Distribution(unsigned int mode,double xmin,double xmax,int nbins,
					   unsigned int minn,unsigned int maxn, const std::string & lname) :
  Jet_Observable_Base(mode,xmin,xmax,nbins,minn,maxn,lname) 
{
  m_name+="eta_";
}
...
{
  return new Jet_Eta_Distribution(m_mode,m_xmin,m_xmax,m_nbins,m_minn,m_maxn,m_listname);
}

Jet_PT_Distribution::Jet_PT_Distribution(unsigned int mode,double xmin,double xmax,int nbins,
					   unsigned int minn,unsigned int maxn, const std::string & lname) :
  Jet_Observable_Base(mode,xmin,xmax,nbins,minn,maxn,lname) 
{
  m_name+="pt_";
...
{
  return new Jet_PT_Distribution(m_mode,m_xmin,m_xmax,m_nbins,m_minn,m_maxn,m_listname);
}
