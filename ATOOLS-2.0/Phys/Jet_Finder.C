  ...
  if (m_combs.empty()) {
	...
	if (ID(m_combs[i])!=0) {
	  ...
	  for (size_t j(0);j<m_combs[i].size();++j)
	    if (m_combs[i][j]!=0) msg_Out()<<" "<<ID(j)<<"["<<m_flavs[j]<<","
					   <<m_flavs[j].Strong()<<"]";
	  msg_Out()<<" }\n";
	}
      }
...
double Jet_Finder::YminKt(Vec4D * p,int & j1,int & k1,int cl)
{
  PROFILE_HERE;
  double ymin = 2.;
  double pt2jk,pt2j,pt2k;
...
bool Jet_Finder::TwoJets(const Vec4D & p1,const bool fix) 
{
  ...
  if (m_type>=2) {
    ...
    msg_Out()<<"WARNING in Jet_Finder::TwoJets(Vec4D &) "<<std::endl
...
{ 
  if (dr<=1.e-6) {
    msg_Error()<<"ERROR in Jet_Finder::SetDeltaR("<<dr<<") : "<<std::endl
