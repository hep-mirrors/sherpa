...
void Debug::InitializeHist() {
	if (m_isr_debug && m_fsrmode != 2) {
		...
		m_histograms_ISR["weight"]  = new Histogram(0, 0, 20, 100);
...
void Debug::FillHist(const Vec4D_Vector &plab, YFS::ISR *p_isr, YFS::FSR *p_fsr) {
	if (m_fsr_debug && m_fsrmode != 0) {
		...
		if (p_fsr->m_fsrWeight != 0 && p_fsr->m_cut == 1) {
			if (p_fsr->m_sQ <= 0) PRINT_VAR(p_fsr->m_sQ);
			m_histograms_FSR["massSQ"]->Insert(sqrt(p_fsr->m_sQ));
			m_histograms_FSR["QQk"]->Insert(sqrt(p_fsr->m_Q * p_fsr->m_photonSum));
			m_histograms_FSR["FSRForm"]->Insert(p_fsr->m_fsrform);
			m_histograms_FSR["Eprime"]->Insert(sqrt(p_fsr->m_sprim));
			m_histograms_FSR["NRemoved"]->Insert(p_fsr->m_NRemoved);
			m_histograms_FSR["x_k"]->Insert(photonSumFSR[1]);
			m_histograms_FSR["y_k"]->Insert(photonSumFSR[2]);
			m_histograms_FSR["jacobian"]->Insert(p_fsr->m_wt2);
			m_histograms_FSR["A4"]->Insert(10 * p_fsr->m_A4);
			m_histograms_FSR["A"]->Insert(10 * p_fsr->m_A);
			m_histograms_FSR["r1r2"]->Insert(sqrt(p_fsr->m_r1 * p_fsr->m_r2));
			m_histograms_FSR["NPhotons"]->Insert(FSRPhotons.size());
			m_histograms_FSR["NPhotons_before_Removal"]->Insert(p_fsr->m_n);
			m_histograms_FSR["FSRWeight"]->Insert(p_fsr->m_fsrWeight / p_fsr->m_fsrform);
			// m_histograms_FSR["E1"]->Insert(p_fsr->m_dipole[0].E());
			// PRINT_VAR(p_dipoles->Old()[0].m_newmomenta[0].E());
			// PRINT_VAR(p_dipoles->Old()[0].m_newmomenta[1].E());
			m_histograms_FSR["E1"]->Insert(p_fsr->m_dipole[0].E());
			m_histograms_FSR["E1"]->Insert(p_fsr->m_dipole[0].E());
			m_histograms_FSR["E2"]->Insert(p_fsr->m_dipole[1].E());
			m_histograms_FSR["R_E1"]->Insert(p_fsr->m_r1.E());
			m_histograms_FSR["R_E2"]->Insert(p_fsr->m_r2.E());
			m_histograms_FSR["x_r1"]->Insert(p_fsr->m_r1[1]);
			m_histograms_FSR["x_r2"]->Insert(p_fsr->m_r2[1]);
			m_histograms_FSR["x_q1"]->Insert(plab[2][1]);
			m_histograms_FSR["x_q2"]->Insert(plab[3][1]);
			m_histograms_FSR["y_q1"]->Insert(plab[2][2]);
			m_histograms_FSR["y_q2"]->Insert(plab[3][2]);
			m_histograms_FSR["z_q1"]->Insert(plab[2][3]);
			m_histograms_FSR["z_q2"]->Insert(plab[3][3]);
			m_histograms_FSR["delta1"]->Insert(p_fsr->m_delta1 * 1e7);
			m_histograms_FSR["BVR_A"]->Insert(p_fsr->m_bvrA * 1e3);
			m_histograms_FSR["pT"]->Insert(pt);
			m_histograms_FSR["q1q2"]->Insert(sqrt(p_fsr->m_q1q2));
			m_histograms_FSR["svar"]->Insert(sqrt(p_fsr->m_sp));
			m_histograms_FSR["q1_E"]->Insert(plab[2].E());
			m_histograms_FSR["q2_E"]->Insert(plab[3].E());
			m_histograms_FSR["Q0"]->Insert(Q0);
			...
			for (auto c : cos)   m_histograms_FSR["cos(theta)"]->Insert(c);
			for (auto s : sin)   m_histograms_FSR["sin(theta)"]->Insert(s);
			for (auto p : phi)   m_histograms_FSR["phi"]->Insert(p);
			for (auto p : kvec0)   m_histograms_FSR["kvec0"]->Insert(p * p_fsr->m_xfact);
			for (auto d : dist1) m_histograms_FSR["f"]->Insert(d / 1e4);
			for (auto fbar : dist2) m_histograms_FSR["fbar"]->Insert(fbar / 1e4);
			for (auto d1 : del1) m_histograms_FSR["del1"]->Insert(d1);
			for (auto d2 : del2) m_histograms_FSR["del2"]->Insert(d2);
			m_histograms_FSR["Btil"]->Insert(p_fsr->m_btil);
			m_histograms_FSR["beta00"]->Insert(log(m_born));
			m_histograms_FSR["BtilQCrude"]->Insert(p_fsr->m_BtiQcru);
			m_histograms_FSR["BtilStar"]->Insert(p_fsr->m_btilStar);
			m_histograms_FSR["BtilXCrude"]->Insert(p_fsr->m_BtiXcru);
			m_histograms_FSR["VoluMC"]->Insert(p_fsr->m_volumc);
			m_histograms_FSR["DelVol"]->Insert(p_fsr->m_delvol);
			m_histograms_FSR["DelYFS"]->Insert(p_fsr->m_DelYFS);
			m_histograms_FSR["EminQ"]->Insert(log(p_fsr->m_EminQ * 1e6));
			m_histograms_FSR["m_r1"]->Insert(p_fsr->m_r1.Mass() * 10);
			m_histograms_FSR["m_r2"]->Insert(p_fsr->m_r2.Mass() * 10);
			m_histograms_FSR["YFS_IR"]->Insert(p_fsr->m_YFS_IR);
			m_histograms_FSR["VolMc"]->Insert(exp(p_fsr->m_volmc));
			m_histograms_FSR["wtmass"]->Insert(p_fsr->m_massW);
			m_histograms_FSR["hiddenWeight"]->Insert(p_fsr->m_hideW);
			m_histograms_FSR["massWeight"]->Insert(p_fsr->m_massW);
		}
		m_histograms_FSR["Cutflow"]->Insert(p_fsr->m_cut);
		m_histograms_FSR["g"]->Insert(p_fsr->m_g);
		m_histograms_FSR["gp"]->Insert(p_fsr->m_gp);
		m_histograms_FSR["beta"]->Insert(p_fsr->m_beta1);
		// m_histograms_FSR["qboost"]->Insert(m_Q.Mass());
		// m_histograms_FSR["pxboost"]->Insert(m_Q.Mass());
		if (FSRPhotons.size() != 0) {
			m_histograms_FSR["K0"]->Insert(photonSumFSR.E());
			m_histograms_FSR["Photon_pT"]->Insert(p_fsr->m_photonSumPreBoost.PPerp());
			m_histograms_FSR["yy"]->Insert(p_fsr->m_yy);
			m_histograms_FSR["xfact"]->Insert(1. / p_fsr->m_xfact);
		}
		// }
		// else m_histograms_FSR["Cutflow"]->Insert(0);
	}
	if (m_isr_debug && m_fsrmode != 2) {
		Vec4D_Vector ISRPhotons = p_isr->GetPhotons();
		m_histograms_ISR["v"]->Insert(p_isr->m_v);
		m_histograms_ISR["K0"]->Insert(p_isr->m_photonSum[0]);
		m_histograms_ISR["NGamma"]->Insert(ISRPhotons.size());
		m_histograms_ISR["nbar"]->Insert(p_isr->m_nbar);
		m_histograms_ISR["weight"]->Insert(p_isr->m_weight);
		m_histograms_ISR["massweight"]->Insert(p_isr->m_massW);
		m_histograms_ISR["jacweight"]->Insert(p_isr->m_jacW);
		if (p_isr->m_cut == 0) m_histograms_ISR["Cutflow"]->Insert(0);
		else m_histograms_ISR["Cutflow"]->Insert(1);
		...
		for (auto c : cos)   m_histograms_ISR["cos(theta)"]->Insert(c);
		for (auto j : jac)   m_histograms_ISR["jacobian"]->Insert(j);
		for (auto s : scale) m_histograms_ISR["lam"]->Insert(s);
		for (auto A : AA) m_histograms_ISR["A"]->Insert(A);
		for (auto k : K2) m_histograms_ISR["K2"]->Insert(k);
		for (auto p : ptk) m_histograms_ISR["ptk"]->Insert(p);
	}
}
