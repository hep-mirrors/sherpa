...
#include"TPSEMAR.h"
#include"TFOAM_INTEGRAND.h"  // abstract class
#include"TFOAM.h"
...
// and allocated dynamicaly array of components
  ...
  if (n>0){
    ...
    if(m_chat) {
      if(m_Coords == NULL)
	{ cout<<"TFVECT constructor failed to allocate"<<endl; exit(0);}
    }
...
// "copy constructor"
  ...
  if(m_chat) {
    if(m_Coords == NULL){ cout<<"TFVECT constructor failed to allocate m_Coords"<<endl; exit(0); }
  }
...
////////////////////////////////////////////////////////////////////////////////
void TFCELL::MakeVolume(void){
//    Calculates Cartesian volume of the cell
  int i,j,k;
  double volu=1.0;
  if(m_nDim>0){         // simplical subspace
    TFMATRIX m(m_nDim);
    TFVECT *v, *v0;
    v0 = (*this)[0];
    for(i=1; i<m_nDim+1; i++){ // vertices
      v = (*this)[i];
      for(j=0; j<m_nDim; j++) m(i-1,j)=(*v)[j]-(*v0)[j]; //coordinates;
    }
    volu *= m.Determinant();
...
// Constructor
  ...
  sprintf(m_Date,"%s","  Release date:  2002.02.13    "); // Release date
  m_Version  = 2.05;                                      // Release version
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void TFOAM::Initialize(TPSEMAR *PseRan, TFOAM_INTEGRAND *fun ){
...
//
  ...
  case 0:
    ...
    m_vMax= (long)(pow((double)2, (double)m_nDim))+nDivi;
    ...
    NoVert = (long)pow((double)2,(double)m_nDim);
    iVe=0;
    partition.Reset();
    do {
...
// Initialize "root part" of the FOAM of cells
  ...
  if(m_nDim==0 && m_kDim>0){
    ...
    case 0:
      ...
      npow = (long)pow((double)m_nDim,(double)m_nDim);
      Perm  = new int[m_nDim];
...
//   already defined prior to calling this routine.
  ...
  TFCELL  *Parent;
  TFMATRIX Yrel(m_nDim), Xrel(m_nDim); // m_nDim=0 is handled internaly
  ...
  for(iev=0;iev<m_nSampl;iev++){
	...
	for(iv=0; iv<m_nDim+1; iv++){
	  if(iv!=jv){  // vertex jv will be replaced with the random vertex
	    for(j=0; j<m_nDim; j++)
	      Yrel(k,j) = (*(*Cell)[iv])[j] - xRand[m_N0Si+j];
	    k++;
	  }
	}
...
// Provides random vector Alpha  0< Alpha(i) < 1
  ...
  m_PseRan->MakeVec(m_kDim,m_Rvec);   // kDim random numbers needed
...
// For higher dimension random-walk method is used instead of ordering.
  ...
  if (m_nDim>nDimMax){                   // faster random-walk algorithm for high dimensions
    m_PseRan->MakeVec(m_nDim+1,m_Rvec);  // nDim+1 random numbers needed
    sum = 0.0;
    for(i=0; i<=m_nDim; i++){
      sum += -log(m_Rvec[i]);
    ...
    m_PseRan->MakeVec(m_nDim,m_Rvec);    // nDim random numbers needed
