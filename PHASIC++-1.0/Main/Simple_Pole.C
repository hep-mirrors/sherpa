...
void Simple_Pole_Uniform::CalculateLimits(ATOOLS::Info_Key &spkey,ATOOLS::Info_Key &ykey) 
{
...
void Simple_Pole_Forward::CalculateLimits(ATOOLS::Info_Key &spkey,ATOOLS::Info_Key &ykey) 
{
  for (size_t i=0;i<3;++i) m_spkey[i]=spkey[i];
  if (!m_zchannel) m_spkey[1]=spkey[3];
}
...
void Simple_Pole_Backward::CalculateLimits(ATOOLS::Info_Key &spkey,ATOOLS::Info_Key &ykey) 
{
  for (size_t i=0;i<3;++i) m_spkey[i]=spkey[i];
  if (!m_zchannel) m_spkey[1]=spkey[3];
...
void Simple_Pole_Central::CalculateLimits(ATOOLS::Info_Key &spkey,ATOOLS::Info_Key &ykey) 
{
  for (size_t i=0;i<3;++i) m_spkey[i]=spkey[i];
  if (!m_zchannel) m_spkey[1]=spkey[3];
}
