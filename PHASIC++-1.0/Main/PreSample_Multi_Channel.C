...
PreSample_Multi_Channel::PreSample_Multi_Channel(std::string name,
						 Color_Integrator *const colint) :
  Multi_Channel(name), p_colint(colint)
{ 
  m_subchannels.clear();
  m_alpha.clear();
  m_palpha.clear();
...
void PreSample_Multi_Channel::DropAllChannels()
{
...
void PreSample_Multi_Channel::GeneratePoint(ATOOLS::Vec4D *p,ATOOLS::Cut_Data *cuts)
{ 
...
void PreSample_Multi_Channel::GenerateWeight(ATOOLS::Vec4D *p,ATOOLS::Cut_Data *cuts)
{
