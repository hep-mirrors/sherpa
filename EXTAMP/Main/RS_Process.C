...
namespace EXTAMP {
  ...
  RS_Process::RS_Process(const PHASIC::Process_Info& pi) : Process(pi) {
    ...
    for(std::vector<PHASIC::CS_Dipole*>::const_iterator it=m_dipoles.begin();
	it!=m_dipoles.end(); ++it)
      msg_Debugging() << (*it)->Info() << "\n";

    /* Get parameters for subevent smearing */
    ATOOLS::Settings& s = ATOOLS::Settings::GetMainSettings();
    m_alpha_0     = s["NLO_SMEAR_THRESHOLD"].Get<double>();
    m_smear_power = s["NLO_SMEAR_POWER"].Get<double>();
	...
	for(std::vector<size_t>::const_iterator k=PartonIndices().begin(); k!=PartonIndices().end(); ++k)
	  {
	    ...
	    PHASIC::Dipole_Info di(m_flavs, *i, *j, *k, subtype, alphamin, alphamax);
	    ret.push_back(External_ME_Interface::GetExternalDipole(di));
	  }
    return ret;
  }
					     ...
					     const Dipole_Vector& dipoles) const
  {
	...
	/* Now tell the dipole where to get its (pre-calculated)
	   running couplings */
	dipoles[i]->SetCouplings(nrqcd,nrqed);
      }
  }
