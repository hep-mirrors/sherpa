...
namespace PHASIC {
  class C4_14 : public Single_Channel {
    Info_Key m_kI_2_34,m_kI_3_4,m_kI_5_234,m_kZS_82;
    Vegas* p_vegas;
  public:
    C4_14(int,int,Flavour*,Integration_Info * const);
    ~C4_14();
...
extern "C" Single_Channel * Getter_C4_14(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C4_14(nin,nout,fl,info);
}

void C4_14::GeneratePoint(Vec4D * p,Cut_Data * cuts,double * _ran)
{
...
void C4_14::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p2345=p[0]+p[1];
  double s2345_max = p2345.Abs2();
...
C4_14::C4_14(int nin,int nout,Flavour* fl,Integration_Info * const info)
       : Single_Channel(nin,nout,fl)
{
  name = std::string("C4_14");
  rannum = 8;
...
void C4_14::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 82.419;
  width = 0.;
...
void C4_14::AddPoint(double Value)
{
  Single_Channel::AddPoint(Value);
  p_vegas->AddPoint(Value,rans);
}
std::string C4_14::ChID()
{
  return std::string("CGPR$I_2_34$I_3_4$I_5_234$MP24_234$MP24_34$ZS_82$");
}
