...
namespace PHASIC {
  class C4_15 : public Single_Channel {
    Info_Key m_kI_25_34,m_kI_2_5,m_kI_3_4,m_kZS_120;
    Vegas* p_vegas;
  public:
    C4_15(int,int,Flavour*,Integration_Info * const);
    ~C4_15();
...
extern "C" Single_Channel * Getter_C4_15(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C4_15(nin,nout,fl,info);
}

void C4_15::GeneratePoint(Vec4D * p,Cut_Data * cuts,double * _ran)
{
  ...
  CE.Isotropic2Momenta(p2345,s25,s34,p25,p34,ran[2],ran[3]);
  CE.Isotropic2Momenta(p25,s2,s5,p[2],p[5],ran[4],ran[5]);
  CE.Isotropic2Momenta(p34,s3,s4,p[3],p[4],ran[6],ran[7]);
}

void C4_15::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p2345=p[0]+p[1];
  double s2345_max = p2345.Abs2();
  ...
  if (m_kI_25_34.Weight()==ATOOLS::UNDEFINED_WEIGHT)
    m_kI_25_34<<CE.Isotropic2Weight(p25,p34,m_kI_25_34[0],m_kI_25_34[1]);
  wt *= m_kI_25_34.Weight();
...
C4_15::C4_15(int nin,int nout,Flavour* fl,Integration_Info * const info)
       : Single_Channel(nin,nout,fl)
{
  name = std::string("C4_15");
  rannum = 8;
  rans  = new double[rannum];
  m_kI_25_34.Assign(std::string("I_25_34"),2,0,info);
...
void C4_15::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 120.6285;
  width = 0.;
...
void C4_15::AddPoint(double Value)
{
  Single_Channel::AddPoint(Value);
  p_vegas->AddPoint(Value,rans);
}
std::string C4_15::ChID()
{
  return std::string("CG2$I_25_34$I_2_5$I_3_4$MP24_34$MTH_25$ZS_120$");
}
