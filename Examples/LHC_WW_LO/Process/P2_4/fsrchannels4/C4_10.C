...
namespace PHASIC {
  class C4_10 : public Single_Channel {
    double m_amct,m_alpha,m_ctmax,m_ctmin;
    Info_Key m_kI_2_4,m_kI_3_24,m_kTC_0__1__5_234,m_kZS_120;
    Vegas* p_vegas;
  public:
    C4_10(int,int,Flavour*,Integration_Info * const);
    ~C4_10();
...
extern "C" Single_Channel * Getter_C4_10(int nin,int nout,Flavour* fl,Integration_Info * const info) {
  return new C4_10(nin,nout,fl,info);
}

void C4_10::GeneratePoint(Vec4D * p,Cut_Data * cuts,double * _ran)
{
...
void C4_10::GenerateWeight(Vec4D* p,Cut_Data * cuts)
{
  double wt = 1.;
  Vec4D p2345=p[0]+p[1];
  double s2345_max = p2345.Abs2();
...
C4_10::C4_10(int nin,int nout,Flavour* fl,Integration_Info * const info)
       : Single_Channel(nin,nout,fl)
{
  name = std::string("C4_10");
  rannum = 8;
...
void C4_10::ISRInfo(int & type,double & mass,double & width)
{
  type  = 2;
  mass  = 120.6285;
  width = 0.;
...
void C4_10::AddPoint(double Value)
{
  Single_Channel::AddPoint(Value);
  p_vegas->AddPoint(Value,rans);
}
std::string C4_10::ChID()
{
  return std::string("CG2$I_2_4$I_3_24$MP24_234$MTH_24$TC_0__1__5_234$ZS_120$");
}
