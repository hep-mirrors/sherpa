...
namespace Rivet {
  ...
  class MC_Zmumu : public Analysis {
  public:

    /// Constructor
    DEFAULT_RIVET_ANALYSIS_CTOR(MC_Zmumu);
    ...
    void init() {
      ...
      Bare_Leptons.acceptIdPair(13);
      Photons.acceptId(22);
      Neutrinos.acceptIdPair(14);
      ...
      _h_sum_E_gamma = bookHisto1D("Sum_E_Gamma_bare_m", 100, 0., 200.);
      _h_sum_pT_gamma = bookHisto1D("Sum_pT_Gamma_bare_m", 100, 0., 200.);
      _h_sum_E_gamma_rest_frame = bookHisto1D("Sum_E_Gamma_rest_frame_bare_m", 100, 0., 100.);
      _h_sum_pT_gamma_rest_frame = bookHisto1D("Sum_pT_Gamma_rest_frame_bare_m", 100, 0., 100.);
      _h_sum_E_gamma_coarse_rest_frame = bookHisto1D("Sum_E_Gamma_coarse_rest_frame_bare_m", 25, 0., 100.);
      _h_sum_pT_gamma_coarse_rest_frame = bookHisto1D("Sum_pT_Gamma_coarse_rest_frame_bare_m", 25, 0., 100.);
      ...
      _h_theta_phot = bookHisto1D("Theta_photon_bare_m", 50, 0., pi);
      _h_theta_closest_phot = bookHisto1D("Theta_photon_closest_bare_m", 50, 0., 10.);
      _h_theta_hardest_phot = bookHisto1D("Theta_hardest_photon_bare_m", 50, 0., pi);

      // invariant mass lepton antilepton
      _h_mll = bookHisto1D("mmm_bare", 100, 0., 150.);
      // invariant mass lepton antilepton photon - index 0 - hardest photon
      // index 1 = closest photon
      _h_mllg[0] = bookHisto1D("mmmg_hardest_g_bare", 100, 0., 150.);
      _h_mllg[1] = bookHisto1D("mmmg_closest_g_bare", 100, 0., 150.);
      // lepton pT and mT distributions (charged and all leptons)
      _h_pt_l = bookHisto1D("pT_m_charged_bare", 100, 0., 75.);
      _h_pt_ln = bookHisto1D("pT_m_all_bare", 100, 0., 75.);
      _h_mt_l = bookHisto1D("mT_m_charged_bare", 100, 0., 150.);
      _h_mt_ln = bookHisto1D("mT_m_all_bare", 100, 0., 150.);
      // mT lepton/antilepton system
      _h_mt_system_l = bookHisto1D("mT_m_system_bare", 100, 0., 150.);
