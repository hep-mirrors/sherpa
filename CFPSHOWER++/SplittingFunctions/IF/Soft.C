...
IF_Soft::IF_Soft(const Kernel_Info & info)  :
  SF_Base(info),
  m_is_g2gg(info.GetSplit().IsVector()), 
  m_swapped(info.TagSequence()[0]!=0),
  m_kin(info.KinType())
{
  m_name = std::string("IF: Soft");
}

double IF_Soft::operator()(const Splitting & split) {
...
double IF_Soft::Integral(const Splitting & split) const {
  double Kmax = ((m_CMW==1.) ?
...
double IF_Soft::OverEstimate(const Splitting & split) const {
  double Kmax = ((m_CMW==1.) ?
		 (1.+split.GetKernel()->GetGauge()->KMax(split)) : 1.);
...
void IF_Soft::GeneratePoint(Splitting & split) const {
  split.SetZ(1. - (1.-split.Zmin()) *
	     pow((1.-split.Zmax())/(1.-split.Zmin()), ran->Get()));
  split.SetPhi(2.*M_PI*ran->Get());
}

double IF_Soft::A1(const Splitting & split) const {
...
DECLARE_GETTER(IF_Soft,"IF_Soft",SF_Base,Kernel_Info);

SF_Base * ATOOLS::Getter<SF_Base,Kernel_Info,IF_Soft>::
operator()(const Parameter_Type & info) const
{
  if (info.Type()==kernel_type::IF &&
	...
	(info.GetSplit().IsVector() &&
	 ...
	 int(info.SFType() & 2)>0 ) )
    return new IF_Soft(info);
  }
  return NULL;
}

void ATOOLS::Getter<SF_Base,Kernel_Info,IF_Soft>::
PrintInfo(std::ostream &str,const size_t width) const
{
  str<<"Soft splitting function (IF)";
}
