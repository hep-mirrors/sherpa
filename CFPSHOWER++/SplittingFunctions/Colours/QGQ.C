...
namespace CFPSHOWER {
  class QGQ : public Gauge_Base {
    int m_mode;
  public:
    QGQ(const Kernel_Info & info) : Gauge_Base(info) {
      m_charge = m_CF;
      SetName("3-8-3");
    }
...
bool QGQ::SetColours(Splitting & split) {
  m_colors.clear();
  ...
  //for (size_t i=0;i<2;i++) msg_Out()<<" --> "<<i<<": "<<m_colors[i]<<"\n";
  return (m_colors.size()==2); 
}

DECLARE_GETTER(QGQ,"QGQ",Gauge_Base,Kernel_Info);

Gauge_Base * ATOOLS::Getter<Gauge_Base,Kernel_Info,QGQ>::
...
void ATOOLS::Getter<Gauge_Base,Kernel_Info,QGQ>::
PrintInfo(std::ostream &str,const size_t width) const
{
  str<<"gauge part for 3-8-3 in colour space";
}
