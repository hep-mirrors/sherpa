...
namespace CFPSHOWER {
  ...
  private:
    ...
    ATOOLS::Vec4D m_splitmom, m_spectmom, m_newmom, m_kt; 
...
void FF_PanGlobal_Kinematics::ConstructK(Splitting & split) {
  ...
  m_kt = sqrt(split.T()) * (nt * cos(split.Phi()) + lt * sin(split.Phi()));
  m_newmom = (1.-m_zi)*m_splitmom + (1.-m_zj)*m_spectmom + m_kt;
...
void FF_PanGlobal_Kinematics::MakeSystem(Splitting & split,const Configuration & config) {
  ...
  for (Parton_List::const_iterator pit=config.begin();pit!=config.end();pit++) {
    ...
    //if (dabs(mom[0]-m_u*(*pit)->Mom()[0])>1.e-6) {
    //  msg_Out()<<"Gotcha! "<<(m_u*mom)<<" vs. "<<(*pit)->Mom()<<" from u = "<<m_u<<"\n"
    //	       <<"   offshell momentum?  "<<(*pit)->Mom().Abs2()
    //	       <<" = "<<sqr((*pit)->Mom()[0])<<" - "<<sqr((*pit)->Mom().PSpat())<<"\n";
    //}
    systot   += mom;
  }
  //msg_Out()<<"Check this "<<m_splitmom<<" + "<<m_spectmom<<"\n"
  //	   <<"    = "<<(m_splitmom+m_spectmom)<<" ";
  m_splitmom    *= m_zi * m_u;
  ...
  //msg_Out()<<"vs. "<<((m_splitmom + m_spectmom + m_newmom)/m_u-m_kt)<<"\n"
  //	   <<"    "<<(systot/m_u-m_kt)<<"\n";
