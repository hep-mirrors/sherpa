#include "CFPSHOWER++/Main/CFP_Cluster_Definitions.H"

//#include "CFPSHOWER++/Shower/Shower.H"
//#include "PHASIC++/Channels/CFPS_Kinematics.H"
...
/*
CFP_Cluster_Definitions::
CFP_Cluster_Definitions(Shower *const shower,const int kmode,
			const int pdfcheck,const int kfmode):
  p_shower(shower), m_kmode(kmode), m_pdfcheck(pdfcheck), m_kfmode(kfmode) {}

Cluster_Param CFP_Cluster_Definitions::Cluster(const Cluster_Config &ca)
{
  DEBUG_FUNC(ca);
  CFP_Parameters cs(KT2(ca.p_ampl,
...
Splitting_Function_Base *CFP_Cluster_Definitions::GetSF
(const ATOOLS::Cluster_Leg *i,const ATOOLS::Cluster_Leg *j,
 const ATOOLS::Cluster_Leg *k,const ATOOLS::Flavour &mo,CFP_Parameters &cs) const
{
...
CFP_Parameters CFP_Cluster_Definitions::KT2
 ...
 const int kmode,const int force)
{
  ...
  Kin_Args lt;
  CFP_Parameters cs(sqrt(std::numeric_limits<double>::max()),
		   1.0,1.0,0.0,0.0,0.0,
  ...
  if ((i->Id()&3)==0) {
	...
	cs=CFP_Parameters(kt2,lt.m_z,lt.m_y,lt.m_phi,1.0,Q2,0,kin,kmode&1);
	...
	cs=CFP_Parameters(kt2,lt.m_z,lt.m_y,lt.m_phi,1.0-lt.m_y,Q2,2,kin,kmode&1);
	cs.m_pkt=-(cs.m_pk=lt.m_pk);
	cs.m_pijt=lt.m_pi;
      }
    }
	...
	cs=CFP_Parameters(kt2,lt.m_z,lt.m_y,lt.m_phi,lt.m_z,Q2,1,lt.m_mode,kmode&1);
	cs.m_pkt=cs.m_pk=lt.m_pk;
	cs.m_pijt=-lt.m_pi;
	cs.m_lt=lt.m_lam;
      }
	...
	cs=CFP_Parameters(kt2,lt.m_z,lt.m_y,lt.m_phi,lt.m_z,Q2,3,kin,kmode&1);
	cs.m_pkt=-(cs.m_pk=lt.m_pk);
	cs.m_pijt=-lt.m_pi;
	cs.m_lt=lt.m_lam;
      }
...
double CFP_Cluster_Definitions::GetX(const Cluster_Leg* l,
...
Flavour CFP_Cluster_Definitions::ProperFlav(const Flavour &fl,const int anti) const
{
  Flavour pfl(fl);
  switch (pfl.Kfcode()) {
...
void CFP_Cluster_Definitions::KernelWeight
(const ATOOLS::Cluster_Leg *i,const ATOOLS::Cluster_Leg *j,
 const ATOOLS::Cluster_Leg *k,const ATOOLS::Flavour &mo,
 CFP_Parameters &cs,const int kmode) const
{
...
ATOOLS::Vec4D_Vector  CFP_Cluster_Definitions::Combine
...
namespace CFPSHOWER {
  ...
  std::ostream &operator<<(std::ostream &str,const CFP_Parameters &cfp)
  {
    return str<<"CFP{kt="<<sqrt(cfp.m_kt2)<<",z="<<cfp.m_z<<",phi="<<cfp.m_phi
	      <<",mode="<<cfp.m_mode<<",kin="<<cfp.m_kin
	      <<",kmode="<<cfp.m_kmode<<"}";
  }
