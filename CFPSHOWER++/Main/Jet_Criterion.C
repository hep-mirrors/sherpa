...
namespace CFPSHOWER {
  ...
  private:
    struct Q2_Value {
      ...
      inline Q2_Value(const double &q2,const ATOOLS::Flavour &fl,
		      int i, int j, int k):
  ...
  public:
    Jet_Criterion(const PDF::JetCriterion_Key &args) :
      p_clus(args.p_shower->GetClusterDefinitions()) {}
    
    double Qij2(const ATOOLS::Vec4D &pi,const ATOOLS::Vec4D &pj,
		const ATOOLS::Vec4D &pk,
		const ATOOLS::Flavour &fi,const ATOOLS::Flavour &fj) const;
    double Value(ATOOLS::Cluster_Amplitude *ampl,int mode);
  };
}
  
namespace CFPSHOWER {
  double Jet_Criterion::Qij2(const Vec4D &pi,const Vec4D &pj,const Vec4D &pk,
  ...
  double Jet_Criterion::Value(Cluster_Amplitude *ampl,int mode)
  {
    ...
    while (q2list.size()) { 
      ...
      PDF::Cluster_Param cp=
	p_clus->Cluster(PDF::Cluster_Config(ampl,imin,jmin,kmin,
					    mofl,ampl->MS(),NULL,1));
      if (cp.m_pijt==Vec4D())
	cp=p_clus->Cluster(PDF::Cluster_Config(ampl,jmin,imin,kmin,
					       mofl,ampl->MS(),NULL,1));
...
DECLARE_GETTER(CFPSHOWER::Jet_Criterion,"CFP",Jet_Criterion,JetCriterion_Key);

Jet_Criterion *Getter<Jet_Criterion,JetCriterion_Key,CFPSHOWER::Jet_Criterion>::
operator()(const JetCriterion_Key &args) const
{
  return new CFPSHOWER::Jet_Criterion(args);
}

void Getter<Jet_Criterion,JetCriterion_Key,CFPSHOWER::Jet_Criterion>::
PrintInfo(std::ostream &str,const size_t width) const
{ 
  str<<"The CFPSHOWER jet criterion";
}
