...
double SF_FI12::Jacobean(const Splitting & split) const {
  ...
  double pdf_old = split.GetKernel()->GetXPDF(      xB,split.t(0),flav,beam);
  double pdf_new = split.GetKernel()->GetXPDF(xB/ymass,split.t(0),flav,beam);
...
bool SF_FI12::Construct(Splitting & split,const int & mode) {
  double y = 1. - split.t(0)/(split.Q2red()*(1.-split.z(0)));
  Kin_Args kin_args(1.-y,split.z(0),split.phi(0),1|8);
