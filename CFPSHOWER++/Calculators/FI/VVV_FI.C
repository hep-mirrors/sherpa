...
VVV_FI::VVV_FI(const Kernel_Info & info) :
  SF_FI(info) {
  SetName("V->VV");
}
...
double VVV_FI::B2(const double & z,const double & kappa2) const {
  double b2 = 0.0;
  return b2;
}
...
operator()(const Parameter_Type & info) const
{
  ...
  if (info.Type()==kernel_type::FI &&
      (info.GetFlavs()[0].IsVector() &&
       ...
       info.GetFlavs()[2].IsVector())) return new VVV_FI(info);
  return NULL;
}
