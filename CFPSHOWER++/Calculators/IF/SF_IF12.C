...
double SF_IF12::Jacobean(const Splitting & split) const {
  ...
  double pdf_new = split.GetKernel()->GetXPDF(eta_new,split.t(),m_flavs[0],beam);
...
double SF_IF12::PDFEstimate(const Splitting & split) const
{
  ...
  double pdf_new = split.GetKernel()->GetXPDF(eta,scale2,m_flavs[0],beam);
  ...
  if (m_flavs[0].Mass(true)<1.0 && m_split.Mass(true)>=1.0) {
    ...
    double pdf_new1 = split.GetKernel()->GetXPDF(0.2,tcut,m_flavs[0],beam);
  ...
  msg_Out()<<METHOD<<"("<<m_flavs[0]<<" <- "<<m_flavs[1]<<" + "<<m_flavs[2]<<") = "
	   <<dabs(pdf_new/pdf_old)<<".\n";
  return dabs(pdf_new/pdf_old);
}


bool SF_IF12::InitKinematics(Splitting & split) const {
  split.Set_y(split.t() / (split.Q2() * (1.-split.z(0))));
  split.Set_x(split.z(0));
  return true;
}

int SF_IF12::Construct(Splitting & split) const {
