...
#include "MALARIC/Main/Gamma.H"
#include "MALARIC/Tools/Amplitude.H"
#include "MALARIC/Shower/Cluster_Definitions.H"
		  ...
		  PDF::ISR_Handler *const isr)
{
  ...
  auto pss = ms["SHOWER"], nlopss = ms["MC@NLO"];
  DEBUG_FUNC(this);
  p_model=model;
  ...
  m_maxem=nlopss["MAXEM"].Get<size_t>();
  m_reweight=nlopss["REWEIGHT_EM"].Get<unsigned int>();
	...
	for (int type(0);type<2;++type) {
	  if (types&(1<<type))
	    for (int mode(0);mode<2;++mode) {
	      if (v->in[2-mode].IsVector() && v->in[2-mode].Mass(true)==0.)
		AddKernel(new Kernel(this,Kernel_Key(v,mode,0,type,"Soft")));
...
int Shower::Evolve(Amplitude &a, unsigned int &nem)
{
  ...
  for (m_s=Splitting(GeneratePoint(a,t,nem));
    ...
    int stat(m_s.p_sk->Construct(m_s,1));
...
(const Amplitude &a,const double &t,const unsigned int &nem)
{
  ...
  std::vector<int> rcl = p_recoil->RecoilTags(ampl,0,0);
  ...
  for (Amplitude::const_reverse_iterator
    ...
    if(p_recoil->m_needrecmodes) {
      rcl = p_recoil->RecoilTags(ampl,1<<split,0,&recmodes);
    }
...
Splitting Shower::GeneratePoint
(Parton &p,const double &t,const int &cm,
 const std::vector<int> &rcl,const unsigned int &nem)
{
 ...
 const ATOOLS::Flavour &fl,const int b) const
{
  ...
  if (x<p_pdf[b]->XMin() ||
    ...
    return 0.0;
  if (m_fsf*Q2<p_pdf[b]->Q2Min())
    p_pdf[b]->Calculate(x,p_pdf[b]->Q2Min());
  else p_pdf[b]->Calculate(x,m_fsf*Q2);
  return p_pdf[b]->GetXPDF(fl.Bar());
}

double Shower::MuF2(const Splitting &s) const
{
  if (m_muf==1) return s.m_t;
  return -2.0*s.m_pi*s.m_pj;
}

const Kernel_Vector &Shower::GetKernels(const Flavour &fl) const
{
  SKernel_Map::const_iterator skit(m_sks.find(fl));
  if (skit!=m_sks.end()) return skit->second;
  static Kernel_Vector dummy;
  return dummy;
}
