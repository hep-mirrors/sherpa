...
namespace MALARIC {
  ...
  class FVF_FS: public Lorentz_FS_Split {
  public:

    inline FVF_FS(const Kernel_Key &key):
      Lorentz_FS_Split(key) {}
  ...
  public:
    ...
    double Value(const Splitting &s) const
    {
      if (p_sk->PS()->MassScheme() &&
	  s.m_t<2.*sqr(m_fl[1].Mass(true))) return 0.;
...
operator()(const Parameter_Type &args) const
{
  ...
  if (args.p_v->in[0].IntSpin()==1 &&
    ...
    if (args.m_mode) return new FVF_FS(args);
    return new FFV_FS(args);
  }
  if (args.p_v->in[0].IntSpin()==1 &&
      args.p_v->in[1].IntSpin()==2 &&
      args.p_v->in[2].IntSpin()==1) {
    if (args.m_mode) return new FFV_FS(args);
    return new FVF_FS(args);
  }
