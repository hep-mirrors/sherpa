...
Gamma::Gamma(MCatNLO *const alaric,Shower *const shower):
  p_alaric(alaric), p_shower(shower)
{
}
...
Weight_Map Gamma::CalculateWeight(Cluster_Amplitude *const ampl)
{
  ...
  msg_Debugging()<<*ampl<<"\n";
#endif
  Cluster_Amplitude *bampl(p_alaric->GetBornAmplitude());
...
bool Gamma::Reject()
{
  ...
  Cluster_Amplitude *rampl=p_alaric->GetRealEmissionAmplitude(1);
