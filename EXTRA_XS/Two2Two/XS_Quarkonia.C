#include "XS_Quarkonia.H"
#include "ATOOLS/Math/Random.H"
#include "ATOOLS/Org/Run_Parameter.H"
#include "ATOOLS/Phys/Flow.H"
#include "ATOOLS/Phys/LDME.H"
#include "ATOOLS/Org/Message.H"
#include "MODEL/Main/Running_AlphaS.H"
#include "MODEL/Main/Running_AlphaQED.H"
#include "MODEL/Main/Model_Base.H"
#include "PHASIC++/Process/External_ME_Args.H"
#include "PHASIC++/Process/Process_Base.H"
#include "MODEL/UFO/UFO_Model.H"
#include "EXTRA_XS/Main/ME2_Base.H"

#define PropID(i,j) ((1<<i)|(1<<j))

using namespace EXTRAXS;
using namespace MODEL;
using namespace ATOOLS;
using namespace PHASIC;

QuarkoniumState::QuarkoniumState(const Flavour_Vector& fl)
{
R02 = R12 = 0;
bool octet = false;
for (short int i=2;i<4;i++) {
  if (i>1 && fl[i].IsQuarkonia()) index = i;
if(fl[index].IsOctetMeson()) octet = true;
}
  kfc = fl[index].Kfcode();
  if(octet){
    ldme = GetLDME(kfc);
    mass = Flavour((kfc / 100) % 10).Mass(true) +
    Flavour((kfc / 10) % 10).Mass(true);
  }
  else{
    ldme = GetLDME(kfc);
    R02 = ldme *2.*M_PI/(3.*3.);
    R12 = R02;
    mass = fl[index].Mass();
  }
  mass2 = sqr(mass);
};

namespace Quarkonium{
    const Flavour* FindState(const Flavour_Vector& fl)
    {
      for (size_t i=2; i<4; ++i)
        if (fl[i].IsQuarkonia()) return &fl[i];
      return nullptr;
    }

    namespace ColourFlowSetter{
    void qg_qSinglet(size_t m_q, size_t m_a, size_t m_g, size_t m_S, std::vector<std::vector<int>>& m_colours)
    {
        m_colours[m_g][m_a]   = m_colours[5-m_S][m_a] = Flow::Counter();
        m_colours[m_g][1-m_a] = m_colours[m_q][m_a]   = Flow::Counter();
    }
    void qqbar_gSinglet(size_t m_S, size_t m_a, std::vector<std::vector<int>>& m_colours)
    {
        m_colours[1-m_a][0] = m_colours[5-m_S][0] = Flow::Counter();
        m_colours[m_a][1]   = m_colours[5-m_S][1] = Flow::Counter();
    }
    void gg_gSinglet(size_t m_S, std::vector<std::vector<int>>& m_colours)
    {
        size_t bit = ran->Get()<0.5 ? 0 : 1;
        m_colours[0][bit] = m_colours[1][1-bit]     = Flow::Counter();
        m_colours[0][1-bit] = m_colours[5-m_S][1-bit] = Flow::Counter();
        m_colours[1][bit] = m_colours[5-m_S][bit]   = Flow::Counter();
    } 

    void qg_qOctet(size_t m_a, size_t m_g, size_t m_S, std::vector<std::vector<int>>& m_colours){
      if ((m_g+2) == m_S){
        size_t cross = ran->Get()<0.5 ? 0 : 1;
        if (cross==0){
          m_colours[m_g][m_a] = m_colours[m_S][m_a] = Flow::Counter();
          m_colours[1-m_g][m_a]   = m_colours[m_g][1-m_a] = Flow::Counter();
          m_colours[5-m_S][m_a]   = m_colours[m_S][1-m_a] = Flow::Counter();
      
        }
        if (cross==1){
          m_colours[m_g][1-m_a] = m_colours[m_S][1-m_a] = Flow::Counter();
          m_colours[1-m_g][m_a]   = m_colours[m_S][m_a] = Flow::Counter();
          m_colours[5-m_S][m_a]   = m_colours[m_g][m_a] = Flow::Counter();
        }
      }
      if ((m_g+2) != m_S){
        m_colours[m_g][1-m_a] = m_colours[m_S][1-m_a] = Flow::Counter();
        m_colours[m_g][m_a] = m_colours[5-m_S][m_a] = Flow::Counter();
        m_colours[1-m_g][m_a] = m_colours[m_S][m_a] = Flow::Counter();
      }
    }

    void qqbar_gOctet(size_t m_S, size_t m_a, std::vector<std::vector<int>>& m_colours){
        m_colours[1-m_a][0] = m_colours[3-m_a][0] = Flow::Counter();
        m_colours[m_a][1]   = m_colours[2+m_a][1] = Flow::Counter();
        m_colours[2][m_a]   = m_colours[3][1-m_a] = Flow::Counter();
    }

    void gg_gOctet(size_t m_S, std::vector<std::vector<int>>& m_colours){
        size_t bit = ran->Get()<0.5 ? 0 : 1;
        size_t cross = ran->Get()<0.5 ? 0 : 1;
        if (cross==0) {
          m_colours[0][bit] = m_colours[m_S][bit] = Flow::Counter();
          m_colours[0][1-bit] = m_colours[1][bit] = Flow::Counter();
          m_colours[1][1-bit] = m_colours[5-m_S][1-bit] = Flow::Counter();
          m_colours[m_S][1-bit] = m_colours[5-m_S][bit] = Flow::Counter();
        }
        if (cross==1) {
          m_colours[0][bit] = m_colours[m_S][bit] = Flow::Counter();
          m_colours[0][1-bit] = m_colours[5-m_S][1-bit] = Flow::Counter();
          m_colours[1][bit] = m_colours[5-m_S][bit] = Flow::Counter();
          m_colours[1][1-bit] = m_colours[m_S][1-bit] = Flow::Counter();
        }
    }
    }


    namespace IsCandidate
    {
      bool qg_qQuarkonium(const Flavour_Vector& fl, std::initializer_list<kf_code> allowed_fls)
      {
        if ((((fl[0].IsQuark() && fl[1].IsGluon() && fl[2]==fl[0]) ||
            (fl[0].IsGluon() && fl[1].IsQuark() && fl[2]==fl[1])) && fl[3].IsQuarkonia()) ||
            (((fl[0].IsQuark() && fl[1].IsGluon() && fl[3]==fl[0]) ||
            (fl[0].IsGluon() && fl[1].IsQuark() && fl[3]==fl[1])) && fl[2].IsQuarkonia()))
        {
            kf_code kfc = Quarkonium::FindState(fl)->Kfcode();
            for(auto allowed_kfc : allowed_fls){
              if(kfc == allowed_kfc) {
                if(!IsZero(GetLDME(kfc))) return true;
                WarnZeroLDME(kfc);
              }}}
        return false;
      }

      bool qqbar_gQuarkonium(const Flavour_Vector& fl, std::initializer_list<kf_code> allowed_fls)
      {
        if ((fl[0].IsQuark() && fl[1].IsQuark() && fl[1]==fl[0].Bar() && fl[2].IsGluon() && fl[3].IsMeson() ) ||
            (fl[0].IsQuark() && fl[1].IsQuark() && fl[1]==fl[0].Bar() && fl[3].IsGluon() && fl[2].IsMeson() )) 
        {
            kf_code kfc = Quarkonium::FindState(fl)->Kfcode();
            for(auto allowed_kfc : allowed_fls){
              if(kfc == allowed_kfc) {
                if(!IsZero(GetLDME(kfc))) return true;
                WarnZeroLDME(kfc);
              }}}
        return false;
      }

      bool gg_gQuarkonium(const Flavour_Vector& fl, std::initializer_list<kf_code> allowed_fls)
      {
          if ((fl[0].IsGluon() && fl[1].IsGluon() && fl[2].IsGluon() && fl[3].IsMeson() ) ||
             (fl[0].IsGluon() && fl[1].IsGluon() && fl[3].IsGluon() && fl[2].IsMeson() )) 
          {
            kf_code kfc = Quarkonium::FindState(fl)->Kfcode();
            for(auto allowed_kfc : allowed_fls){
              if(kfc == allowed_kfc) {
                if(!IsZero(GetLDME(kfc))) return true;
                WarnZeroLDME(kfc);
          }}}
          return false;
      }

    }

    namespace Calc
    {
      // SINGLET MATRIX ELEMENT CALCULATORS
      double qg_q1S0(const double & s, const double & t, const double & u, const double & mass2){
          return 2./36./sqrt(mass2)*(sqr(t-mass2)-2.*s*u)/(-t*sqr(t-mass2));
      }
      double qqbar_g1S0(const double & s, const double & t, const double & u, const double & mass2){
          return 4./27./sqrt(mass2)*(sqr(s-mass2)-2.*t*u)/(s*sqr(s-mass2));
      }
      double gg_g1S0(const double & s, const double & t, const double & u, const double & mass2){
          double sM = s-mass2, sM2 = sqr(s-mass2);
          double tM = t-mass2, tM2 = sqr(t-mass2);
          double uM = u-mass2, uM2 = sqr(u-mass2);
          double stu  = s*t*u, sum2 = sqr(s)+sqr(t)+sqr(u), mass4 = sqr(mass2);
          double mix2 = sqr(s*t)+sqr(s*u)+sqr(t*u);           // dim = GeV^8
          double fac  = 1./(stu*sM2*tM2*uM2);                 // dim = 1/GeV^18
          double sbit = ( pow(s,4)*sM2 * (sM2+2.*mass4) -     // dim = GeV^16 
              stu * (4./3.  * sum2 * sM*tM*uM +   // dim = GeV^6 * GeV^10
              16./3. * mass2 * mix2 +    // dim = GeV^6 * GeV^10
              28./3. * mass4 * stu) );     // dim = GeV^6 * GeV^10
          double tbit = ( pow(t,4)*sM2 * (tM2+2.*mass4) -
              stu * (4./3.  * sum2 * sM*tM*uM +
              16./3. * mass2 * mix2 +
              28./3. * mass4 * stu) );
          double ubit = ( pow(u,4)*uM2 * (uM2+2.*mass4) -
              stu * (4./3.  * sum2 * sM*tM*uM +
              16./3. * mass2 * mix2 +
              28./3. * mass4 * stu) );
          return 1./8./sqrt(mass2)*fac*(sbit+tbit+ubit);
      }
      double gg_g3S1(const double & s, const double & t, const double & u, const double & mass2){
          double M2 = mass2, sM2 = sqr(s-M2), tM2 = sqr(t-M2),  uM2 = sqr(u-M2);
          double all = sqr(s)/(uM2*tM2)+sqr(t)/(uM2*sM2)+sqr(u)/(sM2*tM2);
          return (5./36.)*sqrt(mass2)*all;
      }

      double qg_q3P0(const double & s, const double & t, const double & u, const double & mass2){
          return 2./9./pow(sqrt(mass2),3)*(sqr(t-3*mass2)*(sqr(s)+sqr(u)))/(-t*pow(t-mass2,4));
      }

      double qqbar_g3P0(const double & s, const double & t, const double & u, const double & mass2){
          return -16./27./pow(sqrt(mass2),3)*(sqr(s-3*mass2)*(sqr(t)+sqr(u)))/(-s*pow(s-mass2,4));
      }

      double gg_g3P0(const double & s, const double & t, const double & u, const double & mass2){
          double sM2 = sqr(s-mass2);
          double tM2 = sqr(t-mass2);
          double uM2 = sqr(u-mass2);

          double Q = s*t*u;
          double P = s*t+t*u+u*s;
          double M2 = s+t+u; double M4 = sqr(M2); double M8 = sqr(M4);
          double mul = 1/(Q*pow(Q-M2*P,4));
          double all1 = 9.*M4*pow(P,4)*(M8-2.*M4*P+sqr(P))-6.*M2*pow(P,3)*Q*(2.*M8-5.*M4*P+sqr(P));
          double all2 = -sqr(P)*sqr(Q)*(M8+2.*M4*P-sqr(P))+2.*M2*P*pow(Q,3)*(M4-P)+6.*M4*pow(Q,4);
          double all = (all1+all2)*mul;
          return all/pow(sqrt(mass2),3);
      }

      double qg_q3P1(const double & s, const double & t, const double & u, const double & mass2){
          return 4./3./pow(sqrt(mass2),3)*((-t)*(sqr(s)+sqr(u))-(4*mass2*s*u))/(pow(t-mass2,4));
      }
      
      double qqbar_g3P1(const double & s, const double & t, const double & u, const double & mass2){
          return -8./3.*4./3./pow(sqrt(mass2),3)*((-s)*(sqr(t)+sqr(u))-(4*mass2*t*u))/(pow(s-mass2,4));
      }

      double gg_g3P1(const double & s, const double & t, const double & u, const double & mass2){
          double sM2 = sqr(s-mass2);
          double tM2 = sqr(t-mass2);
          double uM2 = sqr(u-mass2);

          double Q = s*t*u;
          double P = s*t+t*u+u*s; double P2 = sqr(P);
          double M2 = mass2;      double M4 = sqr(M2);
          double nom = P2*(M2*P2*(M4-4.*P)+2.*Q*(-sqr(M4)+5.*M4*P+P2)-15.*M2*sqr(Q));
          double dnom = pow(Q-M2*P,4);
          return 3/pow(sqrt(mass2),3)*nom/dnom;
      }

      double qg_q3P2(const double & s, const double & t, const double & u, const double & mass2){
          return 4./9./pow(sqrt(mass2),3)*(sqr(t-mass2)*(sqr(t)+6*sqr(mass2))-2*s*u*(sqr(t)-6*mass2*(t-mass2)))/(-t*pow(t-mass2,4));
      }
      
      double qqbar_g3P2(const double & s, const double & t, const double & u, const double & mass2){
          return -8./3.*4./9./pow(sqrt(mass2),3)*(sqr(s-mass2)*(sqr(s)+6*sqr(mass2))-2*t*u*(sqr(s)-6*mass2*(s-mass2)))/(-s*pow(s-mass2,4));
      }
      
      double gg_g3P2(const double & s, const double & t, const double & u, const double & mass2){
          double sM2 = sqr(s-mass2);
          double tM2 = sqr(t-mass2);
          double uM2 = sqr(u-mass2);

          double Q = s*t*u;
          double P = s*t+t*u+u*s; double P2 = sqr(P);
          double M2 = mass2;      double M4 = sqr(M2); double M8 = sqr(M4);
          double nom = 12.*M4*sqr(P2)*(M8-2*M4*P+P2)-3.*M2*pow(P,3)*Q*(8.*M8-M4*P+4.*P2)+2.*P2*sqr(Q)*(-7.*M8+43.*M4*P+P2);
          double nom2 = M2*P*pow(Q,3)*(16.*M4-61*P)+12.*M4*pow(Q,4);
          double dnom = Q*pow(Q-M2*P,4);
          double all = (nom+nom2)/dnom;
          return all/pow(sqrt(mass2),3);
      }
        // OCTETS      
      double qg_q1S0_oct(const double & s, const double & t, const double & u, const double & mass2){
          return  -5./72./sqrt(mass2)*(sqr(s)+sqr(u))/(t*sqr(t-mass2));
      }
      double qqbar_g1S0_oct(const double & s, const double & t, const double & u, const double & mass2){
          return  5./27./sqrt(mass2)*(sqr(t)+sqr(u))/(s*sqr(s-mass2));
      }
      double gg_g1S0_oct(const double & s, const double & t, const double & u, const double & mass2){
          double sM = s-mass2, sM2 = sqr(s-mass2);
          double tM = t-mass2, tM2 = sqr(t-mass2);
          double uM = u-mass2, uM2 = sqr(u-mass2);
          double M2 = (sqr(s)*sM2+s*t*u*(mass2-2.*s)+sqr(t*u))*(sqr(sqr(s)-mass2*s+sqr(mass2))-t*u*(2*sqr(t)+3*t*u+2*sqr(u)))/(s*t*u*sM2*tM2*uM2);
          return 5./(16.*sqrt(mass2))*M2;
      }
      double qg_q3S1_oct(const double & s, const double & t, const double & u, const double & mass2){
          double s2 = sqr(s);
          double u2 = sqr(u);
          double M2 = mass2, sM = sqr(s-M2), tM = sqr(t-M2), uM = sqr(u-M2);
          double heq0 = 2.*M2*t;
          double heq1 = ((s2+u2+2.*M2*t)*sM-2.*M2*s*t*u)/(s*u); 
          double dnom = sqr(sM*tM);
          double nom = (4.*(s2+u2)-s*u);
          double all = nom*(heq0+heq1)/dnom;
          return -1./(108.*pow(sqrt(mass2),3))*all;
      }
      double qqbar_g3S1_oct(const double & s, const double & t, const double & u, const double & mass2){
          double t2 = sqr(t);
          double u2 = sqr(u);

          double alphaS = 0.41;
          double M2 = mass2;
          double heq0 = 4.*M2*s;
          double heq1 = (sqr(s)+sqr(M2))*(t2+u2)/(t*u);
          double nom = 4.*(t2+u2)-t*u;
          double dnom = pow(s-M2,4);
          double all = nom*(heq0+heq1)/dnom;
          return 2./(81.*pow(sqrt(mass2),3))*all;
      }
      double gg_g3S1_oct(const double & s, const double & t, const double & u, const double & mass2){
          double sM2 = sqr(s-mass2);
          double tM2 = sqr(t-mass2);
          double uM2 = sqr(u-mass2);

          double heq0 = 2.*s*mass2*(sqr(t)+sqr(u))*t*u;
          double heq1 = sqr(s)*(sqr(sM2)+pow(t,4)+pow(u,4)+2.*sqr(mass2)*sqr(t*u/s));
          double nom = 27.*(s*t+t*u+u*s)-19.*sqr(mass2);
          double dnom = sM2*sM2*tM2*uM2;
          double all = (heq0+heq1)*nom/dnom;
          return -1./(144.*pow(sqrt(mass2),3))*all;
      }
      double qg_q3P0_oct(const double & s, const double & t, const double & u, const double & mass2){
          return -5./(54.*pow(sqrt(mass2),3))*(sqr(t-3.*mass2)*(sqr(s)+sqr(u)))/(t*pow(t-mass2,4));
      }
      double qqbar_g3P0_oct(const double & s, const double & t, const double & u, const double & mass2){
          return 20./(81.*pow(sqrt(mass2),3))*(sqr(s-3.*mass2)*(sqr(t)+sqr(u)))/(s*pow(s-mass2,4));
      }
      double gg_g3P0_oct(const double & s, const double & t, const double & u, const double & mass2){
          double sM2 = sqr(s-mass2), s2 = sqr(s), s4 = sqr(s2), s6 = pow(s,6), s8 = sqr(s4);
          double tM2 = sqr(t-mass2);
          double uM2 = sqr(u-mass2);

          double z = sqrt(t*u); double z2 = sqr(z); double z4 = sqr(z2); double z6 = pow(z,6); double z8 = sqr(z4);
          double M = sqrt(mass2);
          double line1 = s2*z4*pow(s2-z2,4)+sqr(M)*s*z2*sqr(s2+z2)*(3.*s2-2.*z2)*(2.*s4-6.*s2*z2+3*z4);
          double line2 = pow(M,4)*(9.*pow(s,12)-84.*pow(s,10)*z2+265.*s8*z4-382.*s6*z6+276.*s4*z8-88.*s2*pow(z,10)+9.*pow(z,12));
          double line3 = -pow(M,6)*s*(54.*pow(s,10)-357.*s8*z2+844.*s6*z4-898.*s4*z6+439*s2*z8-81*pow(z,10));
          double line4 = pow(M,8)*(153.*pow(s,10)-798.*s8*z2+1415.*s6*z4-1041.*s4*z6+301*s2*z8-18*pow(z,10));
          double line5 = -pow(M,10)*s*(270.*s8-1089.*s6*z2+1365.*s4*z4-616.*s2*z6+87.*z8);
          double line6 = pow(M,12)*(324.*s8-951.*s6*z2+769.*s4*z4-189.*s2*z6+9.*z8);
          double line7 = -9.*pow(M,14)*s*(6.*s2-z2)*(5.*s4-9.*s2*z2+3.*z4);
          double line8 = 3.*pow(M,16)*s2*(51*s4-59*s2*z2+12*z4)-27.*pow(M,18)*pow(s,3)*(2.*s2-z2)+9.*pow(M,20)*s4;
          double dnom = s*z2*pow(s-sqr(M),4)*pow(s*sqr(M)+z2,4);
          double all = line1+line2+line3+line4+line5+line6+line7+line8;
          return 5./12./pow(sqrt(mass2),3)*all/dnom;
      }
      double qg_q3P1_oct(const double & s, const double & t, const double & u, const double & mass2){
          double sM = s-mass2;
        
          double heq0 = (t*(sqr(s*(sM))+sqr(u*(s+mass2))));
          double heq1 = 4.*mass2*(s*u*(sqr(t)+t*u+sqr(u)));
          double dnom = (pow(t-mass2,4)*sqr(sM));
          double all = (heq0+heq1)/dnom;
          return -5./(27.*pow(sqrt(mass2),3))*all;
      }
      double qqbar_g3P1_oct(const double & s, const double & t, const double & u, const double & mass2){
          double heq0 = s*(sqr(t)+sqr(u));
          double heq1 = 4.*mass2*t*u;
          double dnom = pow(s-mass2,4);
          double all = (heq0+heq1)/dnom;
          return 40./(81.*pow(sqrt(mass2),3))*all;
      }
      double gg_g3P1_oct(const double & s, const double & t, const double & u, const double & mass2){
          double sM2 = sqr(s-mass2); double s2 = sqr(s), s4 = sqr(s2), s6 = pow(s,6), s8 = sqr(s4);
          double tM2 = sqr(t-mass2);
          double uM2 = sqr(u-mass2);

          double M = sqrt(mass2), M2 = mass2;

          double z = sqrt(t*u), z2 = sqr(z), z4 = sqr(z2), z6 = pow(z,6), z8 = sqr(z4);
          double heq0 = s*z2*(sqr(s2-z2)-2.*M2*s*z2-sqr(M2)*(s2+2.*z2)+pow(M,8))*(sqr(s2-z2)-M2*s*(2.*s2-z2)+sqr(M2)*s2);
          double heq1l1 = M2*(2.*sqr(s2-z2)*(s6-4.*s4*z2+s2*z4-z6)-M2*s*(2.*s2-z2)*(5.*s6-17.*s4*z2+9.*s2*z4-z6)+sqr(M2)*(21.*s8-49.*s6*z2+21.*s4*z4-4.*s2*z6+z8));
          double heq1l2 = M2*(-pow(M,6)*s*(24.*s6-30.*s4*z2+6.*s2*z4-z6)+pow(M,8)*s2*(16.*s4-9.*s2*z2+2*z4)-pow(M,10)*pow(s,3)*(6.*s2-z2)+pow(M,12)*s4);
          double heq = heq0 + heq1l1 + heq1l2;
          double dnom = pow(s-M2,4)*pow(s*M2+z2,4);
          double all = heq/dnom;
          return 5./(6.*pow(sqrt(mass2),3))*all;
      }
      double qg_q3P2_oct(const double & s, const double & t, const double & u, const double & mass2){
          double s2 = sqr(s); double sM4 = pow(s-mass2,4);
          double m_mass4 = sqr(mass2);
          double heq0 = t*(s2+sqr(u)+12.*mass2*s*sqr(u)*(s2+mass2*s+m_mass4)/sM4);
          double heq1 = 12.*mass2*s*u*(sqr(s-mass2)*(s2+m_mass4)-sqr(s+mass2)*t*u)/sM4;
          double heq2 = (6.*m_mass4*(s2+sqr(u)+2.*s2*t*u*((s-mass2)*(2.*t+u)-sqr(u))/sM4))/t;
          double dnom = pow(t-mass2,4);
          double all = (heq0+heq1+heq2)/dnom;
          return -1./(27.*pow(sqrt(mass2),3))*all;
      }
      double qqbar_g3P2_oct(const double & s, const double & t, const double & u, const double & mass2){
          double heq0 = s*(sqr(t)+sqr(u));
          double heq1 = 36.*mass2*t*u;
          double heq2 = 18.*sqr(mass2)*(sqr(t)+sqr(u))/s;
          double dnom = pow(s-mass2,4);
          double all = (heq0+heq1+heq2)/dnom;
          return 8./(81.*pow(sqrt(mass2),3))*all;
      }
      double gg_g3P2_oct(const double & s, const double & t, const double & u, const double & mass2){
          double sM2 = sqr(s-mass2), s2 = sqr(s), s4 = sqr(s2), s6 = pow(s,6), s8 = sqr(s4), s10 = pow(s,10), s12 = pow(s,12);
          double tM2 = sqr(t-mass2);
          double uM2 = sqr(u-mass2);

          double M = sqrt(mass2), M2 = mass2;

          double z = sqrt(t*u), z2 = sqr(z), z4 = sqr(z2), z6 = pow(z,6), z8 = sqr(z4), z10 = pow(z,10), z12 = pow(z,12);

          double heq0 = s*z2*(s2*pow(s2-z2,4)-M2*s*z2*sqr(s2-z2)*(11.*s2+2.*z2)
                        +sqr(M2)*(s8-12.*s6*z2+41.*s4*z4-20.*s2*z6+z8)
                        -pow(M,6)*(s8-12.*s6*z2+41.*s4*z4-20.*s2*z6+z8)
                        +pow(M,8)*(29.*s6-114.*s4*z2+108.*s2*z4-10.*z6)
                        -pow(M,10)*s*(65.*s4-104.*s2*z2-33.*z4)+ pow(M,12)*(54.*s4-20.*s2*z2+7.*z4)
                        -pow(M,14)*s*(23.*s2+5.*z2)+7.*pow(M,16)*s2);
          double heq1 = 3.*M2*(2.*s2*sqr(s2-z2)*(s6-4.*s4*z2+s2*z4-z6)
                          - M2*s*(10.*s10-37.*s8*z2+19.*s6*z4+11.*s4*z6-s2*z8-4*z10)
                          + sqr(M2)*(25.*s10-61.*s8*z2+27.*s6*z4-34.*s4*z6+23.*s2*z8-2.*z10)
                          - pow(M,6)*s*(42.*s8-77.*s6*z2+41.*s4*z4-22.*s2*z6+17.*s8)
                          + pow(M,8)*(53.*s8-88*s6*z2+69.*s4*z4-68.*s2*z6+3*z8)
                          - pow(M,10)*s*(54.*s6 - 85.*s4*z2 + 60.*s2*z4 - 9.*z6) + pow(M,12)*s2*(43.*s4-47.*s2*z2+20*z4)
                          - pow(M,14)*pow(s,3)*(22.*s2-9.*z2)+5.*pow(M,16)*s4);
          double heq2 = 3.*sqr(M2)*(2.*s2*(s12-8.*s10*z2+22.*s8*z4-24.*s6*z6+10.*s4*z8-3.*s2*z10+z12)
                              - M2*s*(16.*s12-102.*s10*z2+210.*s8*z4-153.*s6*z6+36.*s4*z8-6.*s2*z10+4.*z12)
                              + sqr(M2)*(60.*s12-306.*s10*z2+482*s8*z4-271.*s6*z6+77.*s4*z8-18.*s2*z10+2.*z12)
                              - pow(M,6)*s*(140.*s10-573*s8*z2+710.*s6*z4-344.*s4*z6+91.*s2*z8-18.*z10)
                              + pow(M,8)*(226.*s10-741.*s8*z2+737.*s6*z4-310.*s4*z6+77.*s2*z8-4.*z10)
                              - pow(M,10)*s*(264.*s8-686.*s6*z2+541.*s4*z4-177.*s2*z6+25.*z8)
                              + pow(M,12)*(226.*s8-452*s6*z2+261*s4*z4-55.*s2*z6+2*z8)
                              - pow(M,14)*(140.*s6-201.*s4*z2+71.*s2*z4-6.*z6)
                              + pow(M,16)*s2*(60.*s4-53*s2*z2+8.*z4)-2.*pow(M,18)*pow(s,3)*(8.*s2-3.*z2)+2.*pow(M,20)*s4)/(s*z2);
          double dnom = pow(s-M2,6)*pow(s*M2+z2,4);
          double all = (heq0+heq1+heq2)/dnom;
          return all/(6.*pow(sqrt(mass2),3));
      }
    }
    

}


//////////////////////////////////////////////////////////////////////////
//
// Singlet production of eta states (1S0 = 441, 551)
//
//////////////////////////////////////////////////////////////////////////
DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q1S0,"1XS_qg_q1S0")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q1S0>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_eta_c_1S, kf_eta_b}))
    {
      return new XS_qg_q1S0(args);
    }
  }
  return NULL;
}

XS_qg_q1S0::XS_qg_q1S0(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*Q.R02;
}

double XS_qg_q1S0::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q1S0(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_qg_q1S0::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qSinglet(m_q, m_a, m_g, Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g1S0,"1XS_qqbar_g1S0")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g1S0>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_eta_c_1S, kf_eta_b}))
    {
      return new XS_qqbar_g1S0(args);
    }
  }
  return NULL;
}

XS_qqbar_g1S0::XS_qqbar_g1S0(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*Q.R02;
}

double XS_qqbar_g1S0::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g1S0(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_qqbar_g1S0::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gSinglet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g1S0,"1XS_gg_g1S0")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g1S0>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_eta_c_1S, kf_eta_b}))
    {
      return new XS_gg_g1S0(args);
    }
   }
  return NULL;
}

XS_gg_g1S0::XS_gg_g1S0(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*Q.R02;
}

double XS_gg_g1S0::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g1S0(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_gg_g1S0::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gSinglet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Singlet production of 3S1 states (3S1 = 443, 553)
//
//////////////////////////////////////////////////////////////////////////


DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3S1,"1XS_gg_g3S1")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3S1>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_J_psi_1S, kf_psi_2S, kf_Upsilon_1S, kf_Upsilon_2S, kf_Upsilon_3S}))
    {
      return new XS_gg_g3S1(args);
    }
  }
  return NULL;
}

XS_gg_g3S1::XS_gg_g3S1(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*Q.R02;
}

double XS_gg_g3S1::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g3S1(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*all*m_pref;
}

bool XS_gg_g3S1::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gSinglet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Singlet production of 3P0 states (3P0 = 10441, 10551)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q3P0,"1XS_qg_q3P0")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q3P0>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_chi_c0_1P, kf_chi_b0_1P, kf_chi_b0_2P}))
    {
      return new XS_qg_q3P0(args);
    }
  }
  return NULL;
}

XS_qg_q3P0::XS_qg_q3P0(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*Q.R12;
}

double XS_qg_q3P0::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q3P0(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_qg_q3P0::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qSinglet(m_q, m_a, m_g, Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g3P0,"1XS_qqbar_g3P0")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g3P0>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_chi_c0_1P, kf_chi_b0_1P, kf_chi_b0_2P}))
    {
      return new XS_qqbar_g3P0(args);
    }
  }
  return NULL;
}

XS_qqbar_g3P0::XS_qqbar_g3P0(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*Q.R12;
}

double XS_qqbar_g3P0::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g3P0(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_qqbar_g3P0::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gSinglet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3P0,"1XS_gg_g3P0")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3P0>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_chi_c0_1P, kf_chi_b0_1P, kf_chi_b0_2P}))
    {
      return new XS_gg_g3P0(args);
    }
  }
  return NULL;
}

XS_gg_g3P0::XS_gg_g3P0(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4.*sqr(4.*M_PI)* pow(m_alphaS,3)*Q.R12;
}

double XS_gg_g3P0::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g3P0(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_gg_g3P0::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gSinglet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Singlet production of 3P1 states (3P1 = 20443, 20553)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q3P1,"1XS_qg_q3P1")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q3P1>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_chi_c1_1P, kf_chi_b1_1P, kf_chi_b1_2P}))
    {
      return new XS_qg_q3P1(args);
    }
  }
  return NULL;
}

XS_qg_q3P1::XS_qg_q3P1(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }

  //we use the wave function for the calculations and not the LDME. The division by 3 appears because of the conversion
  double m_R12 = Q.R12/3.;
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*m_R12*pow(m_alphaS,3);
}

double XS_qg_q3P1::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q3P1(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_qg_q3P1::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qSinglet(m_q, m_a, m_g, Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g3P1,"1XS_qqbar_g3P1")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
            PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g3P1>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_chi_c1_1P, kf_chi_b1_1P, kf_chi_b1_2P}))
    {
      return new XS_qqbar_g3P1(args);
    }
  }
  return NULL;
}

XS_qqbar_g3P1::XS_qqbar_g3P1(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  //we use the wave function for the calculations and not the LDME. The division by 3 appears because of the conversion
  double m_R12 = Q.R12/3.;
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*m_R12;
}

double XS_qqbar_g3P1::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g3P1(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_qqbar_g3P1::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gSinglet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3P1,"1XS_gg_g3P1")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3P1>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_chi_c1_1P, kf_chi_b1_1P, kf_chi_b1_2P}))
    {
      return new XS_gg_g3P1(args);
    }
  }
  return NULL;
}

XS_gg_g3P1::XS_gg_g3P1(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  //we use the wave function for the calculations and not the LDME. The division by 3 appears because of the conversion
  double m_R12 = Q.R12/3.;
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4.*sqr(4.*M_PI)*m_R12;
}

double XS_gg_g3P1::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g3P1(s,t,u,Q.mass2);
  return pow(m_alphaS,3)*CouplingFactor(3,0)*m_pref*all;
}

bool XS_gg_g3P1::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gSinglet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Singlet production of 3P2 states (3P2 = 445, 555)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q3P2,"1XS_qg_q3P2")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q3P2>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_chi_c2_1P, kf_chi_b2_1P, kf_chi_b2_2P}))
    {
      return new XS_qg_q3P2(args);
    }
  }
  return NULL;
}

XS_qg_q3P2::XS_qg_q3P2(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  //we use the wave function for the calculations and not the LDME. The division by 5 appears because of the conversion
  double m_R12 = Q.R12/5.;
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*m_R12*pow(m_alphaS,3);
}

double XS_qg_q3P2::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q3P2(s,t,u,Q.mass2);
  return CouplingFactor(3,0)*m_pref*all;
}

bool XS_qg_q3P2::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qSinglet(m_q, m_a, m_g, Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g3P2,"1XS_qqbar_g3P2")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g3P2>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_chi_c2_1P, kf_chi_b2_1P, kf_chi_b2_2P}))
    {
      return new XS_qqbar_g3P2(args);
    }
  }
  return NULL;
}

XS_qqbar_g3P2::XS_qqbar_g3P2(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  //we use the wave function for the calculations and not the LDME. The division by 5 appears because of the conversion
  double m_R12 = Q.R12/5.;
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4*sqr(4.*M_PI)*pow(m_alphaS,3)*m_R12;
}

double XS_qqbar_g3P2::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g3P2(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qqbar_g3P2::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gSinglet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3P2,"1XS_gg_g3P2")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3P2>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_chi_c2_1P, kf_chi_b2_1P, kf_chi_b2_2P}))
    {
      return new XS_gg_g3P2(args);
    }
  }
  return NULL;
}

XS_gg_g3P2::XS_gg_g3P2(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  //we use the wave function for the calculations and not the LDME. The division by 5 appears because of the conversion
  double m_R12 = Q.R12/5.;
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = 4.*sqr(4.*M_PI)*m_R12*pow(m_alphaS,3);
}

double XS_gg_g3P2::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g3P2(s,t,u,Q.mass2); 
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_gg_g3P2::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gSinglet(Q.index, m_colours);
  return true;
}

//For S
//LDME = 9/(2*M_PI)*M_R02

//For P
//LDME = 3/(4*M_PI)*(2J+1)*m_R12 [1503.08439]
//LDME = 9/(2*M_PI)*(2J+1)*m_R12 [1203.0329] <-- this is what we are using now.

//values from [9503356]


/////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////
//
//                                OCTETS
//
/////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////

//LDMEs from: https://arxiv.org/pdf/1906.10049, https://arxiv.org/html/2501.15575v1
//LDME rules: https://arxiv.org/pdf/1411.5287


//////////////////////////////////////////////////////////////////////////
//
// Octet production of 1S0 (1S0 = 9900441, 9900551)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q1S0_oct,"1XS_qg_q1S0_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q1S0_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_1S0_c}))
    {
      return new XS_qg_q1S0_oct(args);
    }
  }
  return NULL;
}

XS_qg_q1S0_oct::XS_qg_q1S0_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
  
}

double XS_qg_q1S0_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q1S0_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qg_q1S0_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qOctet(m_a, m_g, Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g1S0_oct,"1XS_qqbar_g1S0_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g1S0_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_1S0_c}))
    {
      return new XS_qqbar_g1S0_oct(args);
    }
  }
  return NULL;
}

XS_qqbar_g1S0_oct::XS_qqbar_g1S0_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
}

double XS_qqbar_g1S0_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g1S0_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qqbar_g1S0_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gOctet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g1S0_oct,"1XS_gg_g1S0_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g1S0_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_1S0_c}))
    {
      return new XS_gg_g1S0_oct(args);
    }
  }
  return NULL;
}

XS_gg_g1S0_oct::XS_gg_g1S0_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = Q.ldme*pow(4.*M_PI*m_alphaS,3);
  
}

double XS_gg_g1S0_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g1S0_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_gg_g1S0_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gOctet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Octet production of 3S1 states (3S1 = 99000443, 99000553, ...)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q3S1_oct,"1XS_qg_q3S1_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q3S1_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_3S1_c}))
    {
      return new XS_qg_q3S1_oct(args);
    }
  }
  return NULL;
}

XS_qg_q3S1_oct::XS_qg_q3S1_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*CouplingFactor(3,0)*Q.ldme;
  
}

double XS_qg_q3S1_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q3S1_oct(s,t,u,Q.mass2);
  return m_pref*all;
}

bool XS_qg_q3S1_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qOctet(m_a,m_g,Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g3S1_oct,"1XS_qqbar_g3S1_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g3S1_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_3S1_c}))
    {
      return new XS_qqbar_g3S1_oct(args);
    }
  }
  return NULL;
}

XS_qqbar_g3S1_oct::XS_qqbar_g3S1_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
}

double XS_qqbar_g3S1_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g3S1_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qqbar_g3S1_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gOctet(Q.index, m_a, m_colours);
  return true;
}


DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3S1_oct,"1XS_gg_g3S1_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3S1_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_3S1_c}))
    {
      return new XS_gg_g3S1_oct(args);
    }
  }
  return NULL;
}

XS_gg_g3S1_oct::XS_gg_g3S1_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
}

double XS_gg_g3S1_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g3S1_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_gg_g3S1_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gOctet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Octet production of 3P0 states (3P0 = 9910441, 9910551)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q3P0_oct,"1XS_qg_q3P0_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q3P0_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_3P0_c}))
    {
      return new XS_qg_q3P0_oct(args);
    }
  }
  return NULL;
}

XS_qg_q3P0_oct::XS_qg_q3P0_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
  
}

double XS_qg_q3P0_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q3P0_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qg_q3P0_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qOctet(m_a,m_g,Q.index, m_colours);
  return true;
}


DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g3P0_oct,"1XS_qqbar_g3P0_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g3P0_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_3P0_c}))
    {
      return new XS_qqbar_g3P0_oct(args);
    }
  }
  return NULL;
}

XS_qqbar_g3P0_oct::XS_qqbar_g3P0_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
  
}

double XS_qqbar_g3P0_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g3P0_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qqbar_g3P0_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gOctet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3P0_oct,"1XS_gg_g3P0_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3P0_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_3P0_c}))
    {
      return new XS_gg_g3P0_oct(args);
    }
  }
  return NULL;
}

XS_gg_g3P0_oct::XS_gg_g3P0_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*CouplingFactor(3,0)*Q.ldme;
}

double XS_gg_g3P0_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g3P0_oct(s,t,u,Q.mass2);
  return m_pref*all;
}

bool XS_gg_g3P0_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gOctet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Octet production of 3P1 states (3P1 = 9920443, 9920553)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q3P1_oct,"1XS_qg_q3P1_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q3P1_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_3P1_c}))
    {
      return new XS_qg_q3P1_oct(args);
    }
  }
  return NULL;
}

XS_qg_q3P1_oct::XS_qg_q3P1_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  } 
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
}

double XS_qg_q3P1_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q3P1_oct(s,t,u,Q.mass2);

  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qg_q3P1_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qOctet(m_a,m_g,Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g3P1_oct,"1XS_qqbar_g3P1_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g3P1_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
   if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_3P1_c}))
    {
      return new XS_qqbar_g3P1_oct(args);
    }
  }
  return NULL;
}

XS_qqbar_g3P1_oct::XS_qqbar_g3P1_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
  
}

double XS_qqbar_g3P1_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qqbar_g3P1_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qqbar_g3P1_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gOctet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3P1_oct,"1XS_gg_g3P1_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3P1_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
   if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_3P1_c}))
    {
      return new XS_gg_g3P1_oct(args);
    }
  }
  return NULL;
}

XS_gg_g3P1_oct::XS_gg_g3P1_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
}

double XS_gg_g3P1_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::gg_g3P1_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_gg_g3P1_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gOctet(Q.index, m_colours);
  return true;
}

//////////////////////////////////////////////////////////////////////////
//
// Octet production of 3P2 states (3P2 = 9900445, 9900555)
//
//////////////////////////////////////////////////////////////////////////

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qg_q3P2_oct,"1XS_qg_q3P2_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qg_q3P2_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qg_qQuarkonium(fl, {kf_3P2_c}))
    {
      return new XS_qg_q3P2_oct(args);
    }
  }
  return NULL;
}

XS_qg_q3P2_oct::XS_qg_q3P2_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsGluon()) m_g = i;
    if (i<2 && fl[i].IsQuark()) m_q = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_a      = fl[m_q].IsAnti() ? 1 : 0; 
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
  

}

double XS_qg_q3P2_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  double all = Quarkonium::Calc::qg_q3P2_oct(s,t,u,Q.mass2);

  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qg_q3P2_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qg_qOctet(m_a,m_g,Q.index, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_qqbar_g3P2_oct,"1XS_qqbar_g3P2_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_qqbar_g3P2_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;
  
  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::qqbar_gQuarkonium(fl, {kf_3P2_c}))
    {
      return new XS_qqbar_g3P2_oct(args);
    }
  }
  return NULL;
}

XS_qqbar_g3P2_oct::XS_qqbar_g3P2_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    if (i<2 && fl[i].IsAnti())  m_a = i;
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref   = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
  
}

double XS_qqbar_g3P2_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  
  double all = Quarkonium::Calc::qqbar_g3P2_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_qqbar_g3P2_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::qqbar_gOctet(Q.index, m_a, m_colours);
  return true;
}

DECLARE_TREEME2_GETTER(EXTRAXS::XS_gg_g3P2_oct,"1XS_gg_g3P2_oct")
Tree_ME2_Base *ATOOLS::Getter<PHASIC::Tree_ME2_Base,
			      PHASIC::External_ME_Args,EXTRAXS::XS_gg_g3P2_oct>::
operator()(const External_ME_Args &args) const
{
  if (dynamic_cast<UFO::UFO_Model*>(MODEL::s_model)) return NULL;

  const Flavour_Vector fl = args.Flavours();
  if (fl.size()==4 && args.m_orders[0]==3 && args.m_orders[1]==0) {
    if(Quarkonium::IsCandidate::gg_gQuarkonium(fl, {kf_3P2_c}))
    {
      return new XS_gg_g3P2_oct(args);
    }
  }
  return NULL;
}

XS_gg_g3P2_oct::XS_gg_g3P2_oct(const External_ME_Args& args):
  ME2_Base(args), Q(args.Flavours())
{
  m_oqcd = 3; m_oew = 0;
  const ATOOLS::Flavour_Vector& fl = args.Flavours();
  for (short int i=0;i<4;i++) {
    m_colours[i][0] = m_colours[i][1] = 0;
  }
  m_alphaS = MODEL::s_model->ScalarConstant("alpha_S");
  m_pref = pow(4.*M_PI*m_alphaS,3)*Q.ldme;
  
}

double XS_gg_g3P2_oct::operator()(const Vec4D_Vector& mom) 
{
  double s  = (mom[0]+mom[1]).Abs2();
  double t  = (mom[0]-mom[2]).Abs2();
  double u  = (mom[0]-mom[3]).Abs2();
  
  double all = Quarkonium::Calc::gg_g3P2_oct(s,t,u,Q.mass2);
  return m_pref*all*CouplingFactor(3,0);
}

bool XS_gg_g3P2_oct::SetColours(const Vec4D_Vector& mom) 
{
  Quarkonium::ColourFlowSetter::gg_gOctet(Q.index, m_colours);
  return true;
}
