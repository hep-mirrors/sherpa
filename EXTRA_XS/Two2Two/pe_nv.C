...
namespace EXTRAXS
{
    ...
    public:
        ...
        double m_alpha, m_alphas, m_s, m_massW;
        std::unique_ptr<FormFactor_EMnucleon> p_formfactor;
        incomingnucleon::code m_nucleon; // neutron or proton
    };
        ...
        : ME2_Base(args), m_nucleon(nucleon)
    {
        ...
        m_sintt = 0; // No s/t/u channel restriction for W exchange
    ...
    double pe_nv::operator()(const ATOOLS::Vec4D_Vector &momenta)
    {
        ...
        double Q2 = -t; // Q2 > 0 for spacelike W exchange
...
operator()(const External_ME_Args &args) const
{
    ...
    // check if "elastic" CC scattering
    if (fl.size() != 4)
        return NULL;
    ...
    if (fl[0] == Flavour(kf_p_plus) && fl[2] == Flavour(kf_n) && fl[1].IsChargedLepton() && fl[3].IsNeutrino())
    {
        return new pe_nv(args, incomingnucleon::proton);
    }
    else if (fl[0] == Flavour(kf_n) && fl[2] == Flavour(kf_p_plus) && fl[1].IsNeutrino() && fl[3].IsChargedLepton())
    {
        return new pe_nv(args, incomingnucleon::neutron);
    }
    else
    {
