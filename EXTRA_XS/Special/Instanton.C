...
namespace EXTRAXS {
  ...
  bool Data_Table::ReadTable()
  {
    ...
    if (allxs.empty()) {
      msg_Info() << "Warning in " << METHOD << ":\n"
                 << "   did not find matrix of cross sections under tag "
                    "INSTANTON_XSECS\n"
  ...
  void Data_Table::ConstructDefaultTable()
  {
    /*
      ...
      , Cross-section (pb)} where the cross-section allows for N_f=4 plus N_f=5
      if it’s above the threshold (so always 8 to 10 fermions in addition to n_g
      gluons in the final state).

      You can see that the largest cross-section is 14.5 milli barn at 10 GeV
      (with alpha_s = 0.415 still kind of reasonable) and dropping to 4 pico
      barns at 40 GeV
  ...
  bool Data_Table::Interpolate(const double& E)
  {
    ...
    for (dit = m_data.begin(); dit != m_data.end(); dit++)
      if (dit->first > E) break;
    m_upper = dit->second;
    dit--;
    m_lower = dit->second;
  ...
  void Data_Table::Test(const double& E)
  {
    Output();
    Interpolate(E);
    msg_Out() << "For E = " << E << " GeV: sigma' = " << Sigmahat()
              << " 1/GeV^2 = " << Sigmahat() * rpa->Picobarn() << " pb, "
              << "1/rho = " << Rho() << " GeV, "
              << "<Ngluons> = " << Ngluons() << ".\n";
  }

  struct instantonScale {
    enum code { rho = 1, Ehat = 2, Ehat_by_sqrtN = 3 };
  };

  // Channel types for instanton initial state
  enum class InstantonChannel {
    gg = 0,    // gluon-gluon
    qg = 1,    // quark-gluon (or antiquark-gluon)
    qqbar = 2, // quark-antiquark (same flavour)
    qq = 3     // two quarks (different flavours, same chirality)
  };
  ...
  public:
    ...
    // it.
    int OrderQCD(const int&) const override
    {
      return NonfactorizingCoupling::WithCustomVariationWeight;
    };

    double CustomRelativeVariationWeightForRenormalizationScaleFactor(
        double scalefactor) const override;
  };
} // namespace EXTRAXS

XS_instanton::XS_instanton(const External_ME_Args& args)
    : ME2_Base(args), m_channel(InstantonChannel::gg), m_norm(0.),
      m_S(sqr(rpa->gen.Ecms())), m_Ehat(0), m_bthreshold(100.),
      m_cthreshold(20.), m_Ngluons_factor(1.), m_sigmahat_factor(1.),
      m_scalechoice(instantonScale::rho), m_Ecms(0), m_threshold(0),
      m_mean_Ngluons(0), m_nquarks(0), m_ngluons(0)
{
  m_sintt = 0;
  DEBUG_INFO("now entered EXTRAXS::XS_instanton ...");
  // Determine initial-state channel from flavour vector
  {
    const Flavour_Vector& fl = args.Flavours();
    bool f0gluon = (fl[0].Strong() && fl[0].IntCharge() == 0);
    bool f1gluon = (fl[1].Strong() && fl[1].IntCharge() == 0);
    bool f0quark = (abs(fl[0].IntCharge()) > 0);
    bool f1quark = (abs(fl[1].IntCharge()) > 0);
    if (f0gluon && f1gluon) {
      m_channel = InstantonChannel::gg;
    } else if ((f0quark && f1gluon) || (f0gluon && f1quark)) {
      m_channel = InstantonChannel::qg;
    } else if (f0quark && f1quark) {
      // qq̄ if one is the antiparticle of the other; qq otherwise
      if (fl[0] == fl[1].Bar())
        m_channel = InstantonChannel::qqbar;
      else
        m_channel = InstantonChannel::qq;
    }
    // else remains gg as default (should not occur given getter vetting)
  }
  // Colour/spin averaging factor m_norm per initial-state channel.
  // Per-leg factors derived from arXiv:1911.09726v3 eqs.(3.11)-(3.15):
  //   gluon leg: 1/(3 SU(2) adj. colours) * 1/(2 helicities) = 1/6
  //   quark leg: 1/(2 SU(2) fund. colours) * 1/(1 coupled chirality) = 1/2
  // (quark factor by analogy; papers confirm quark channels "evaluated
  // analogously")
  switch (m_channel) {
  case InstantonChannel::gg:
    m_norm = 1. / 36.;
    break; // (1/6)^2
  case InstantonChannel::qg:
    m_norm = 1. / 12.;
    break; // (1/2) * (1/6)
  case InstantonChannel::qqbar:
    m_norm = 1. / 4.;
    break; // (1/2)^2
  case InstantonChannel::qq:
    m_norm = 1. / 4.;
    break; // (1/2)^2
  default:
    THROW(fatal_error,
          "XS_instanton: unhandled InstantonChannel in m_norm switch");
  }
  Settings& s = Settings::GetMainSettings();
  ...
  if (m_Ehatmin < Ehatmin || m_Ehatmax > Ehatmax) {
    ...
    if (m_Ehatmin < Ehatmin) {
      msg_Error()
          << "   demand minimal instanton mass below smallest energy in data:\n"
          << "   " << m_Ehatmin << " < " << Ehatmin
          << " -- this could be a problem due to critical extrapolation.\n";
    }
    if (m_Ehatmax > Ehatmax) {
      msg_Error()
          << "   demand maximal instanton mass above largest energy in data:\n"
          << "   " << m_Ehatmax << " > " << Ehatmax
          << " -- this should not become a problem.\n";
    }
  }
  m_Ngluons_factor =
      s["INSTANTON_NGLUONS_MODIFIER"].SetDefault(1.0).Get<double>();
  m_tthreshold =
      (s["INSTANTON_T_PRODUCTION_THRESHOLD"].SetDefault(1000.0).Get<double>());
  m_bthreshold =
      (s["INSTANTON_B_PRODUCTION_THRESHOLD"].SetDefault(100.0).Get<double>());
  m_cthreshold =
      (s["INSTANTON_C_PRODUCTION_THRESHOLD"].SetDefault(20.0).Get<double>());
  m_includeQ = s["INSTANTON_INCLUDE_QUARKS"].SetDefault(5).Get<int>();
  m_sigmahat_factor =
      s["INSTANTON_SIGMAHAT_MODIFIER"].SetDefault(1.0).Get<double>();
  ...
  if (choice == "shat")
    m_scalechoice = instantonScale::Ehat;
  else if (choice == "shat/N")
    m_scalechoice = instantonScale::Ehat_by_sqrtN;
  m_scale_factor = s["INSTANTON_SCALE_FACTOR"].SetDefault(1.).Get<double>();


  p_alphaS =
      dynamic_cast<Running_AlphaS*>(s_model->GetScalarFunction("alpha_S"));
...
double XS_instanton::operator()(const Vec4D_Vector& momenta)
{
  ...
  m_Ehat = sqrt(shat);
  if (m_Ehat < m_Ehatmin || m_Ehat > m_Ehatmax || !m_data.Interpolate(m_Ehat))
    return 0.;
...
{
  return m_scale_factor * (m_scalechoice == instantonScale::Ehat ? m_Ehat
                           : (m_scalechoice == instantonScale::Ehat_by_sqrtN)
                               ? m_Ehat / sqrt(m_data.Ngluons())
                               : m_data.Rho());
}
...
bool XS_instanton::FillFinalState(const std::vector<Vec4D>& mom)
{
  ...
  while ((trials--) > 0) {
    ...
    if (DefineFlavours() && DistributeMomenta() && MakeColours()) {
      ...
      if (dabs((mom[2] - sum).Abs2()) < 1.e-6 &&
          dabs(mom[2][0] - sum[0]) < 1.e-6) {
        return true;
      }
    }
  }
...
double XS_instanton::CustomRelativeVariationWeightForRenormalizationScaleFactor(
    double fac) const
{
  // This is based on the dominant scale dependence of the instanton cross
  // section, cf. Eq. (2.30) of 1911.09726
  if (dabs(fac - 1.) < 1.e-3) return 1.;
  double scale2 = sqr(FixScale());
  return (exp(-4. * M_PI *
              (1. / (*p_alphaS)(fac * scale2) - 1. / (*p_alphaS)(scale2))) *
          pow(sqrt(fac) / m_alphaS_factor,
              2. * 4. * p_alphaS->Beta0(fac * scale2)));
}

bool XS_instanton::DefineFlavours()
{
  // Build m_flavours: IS partons first (indices 0,1), then FS quarks and
  // gluons.
  //
  // The chirality guard
  //   addParticle = (flav.Bar() != m_flavs[0] && flav.Bar() != m_flavs[1])
  //   addAnti     = (flav       != m_flavs[0] && flav       != m_flavs[1])
  // implements the correct per-channel flavour content:
  //   gg/qqbar  -- all N_f pairs (qqbar IS quarks cancel, so no mandatory
  //   minimum) qg        -- IS quark chirality-flipped into FS; all lighter
  //   flavours mandatory qq/qQbar  -- both IS quarks chirality-flipped; all
  //   flavours lighter than the
  //                heavier IS quark are mandatory
  //
  // Quarks up to the PDG code of the heaviest IS quark are MANDATORY and are
  // always added (no production-threshold gate); the point is rejected if they
  // cannot fit in m_Ehat.  Heavier quarks are OPTIONAL and gated by
  // cthreshold/bthreshold.
  m_flavours.clear();
  double totmass = 0.;
  ...
  if (m_Ehat < m_flavs[0].Mass(true) + m_flavs[1].Mass(true)) return false;

  // Determine the heaviest IS quark PDG code.  Quarks with code <= this value
  // are mandatory FS content (bypassing production thresholds); heavier quarks
  // are optional.  For gg and qqbar, max_IS_quark_kf == 0 (no IS quarks).
  size_t max_IS_quark_kf = 0;
  for (size_t i = 0; i < 2; i++) {
    if (m_flavs[i].IsQuark()) {
      size_t kf = (size_t)m_flavs[i].Kfcode();
      if (kf > max_IS_quark_kf) max_IS_quark_kf = kf;
    }
  }
  ...
  for (size_t i = 1; i < 6; i++) {
    ...
    // Chirality guard: determines what this flavour contributes to the FS.
    bool addParticle = (flav.Bar() != m_flavs[0] && flav.Bar() != m_flavs[1]);
    bool addAnti = (flav != m_flavs[0] && flav != m_flavs[1]);
    double addedMass =
        (addParticle ? flav.Mass(true) : 0.) + (addAnti ? flav.Mass(true) : 0.);
    if (addedMass == 0.) continue; // both suppressed (same-flavour qqbar IS)

    bool isMandatory = (i <= max_IS_quark_kf);
    if (isMandatory) {
      // Lighter-than-IS quarks (and the IS quark itself, chirality-flipped)
      // must always be present.  Reject this phase-space point if they cannot
      // fit.
      if (totmass + addedMass > m_Ehat) return false;
    } else {
      // Optional heavier quarks: gate by production thresholds and mass budget.
      if (flav == Flavour(kf_c) && m_Ehat < m_cthreshold) continue;
      if (flav == Flavour(kf_b) && m_Ehat < m_bthreshold) continue;
      if (flav == Flavour(kf_t) && m_Ehat < m_tthreshold) continue;
      if (totmass + addedMass > m_Ehat)
        break;
    }
    if (addParticle) {
  ...
  } while (m_ngluons >= 30 - m_nquarks); // use 30 as safe-guard for multiplicity
  Flavour flav = Flavour(kf_gluon);
  for (size_t i = 0; i < m_ngluons; i++)
    m_flavours.push_back(flav);
  return true;
}
...
bool XS_instanton::MakeColours()
{
  ...
  do {
    ...
    for (size_t i = 0; i < 2; i++) {
      cols[i].erase(find(cols[i].begin(), cols[i].end(), parts[i]));
    }
  } while (cols[0].size() > 1);
...
void XS_instanton::Test()
{
  for (size_t i = 0; i < 5; i++) {
    ...
    m_mean_Ngluons = m_data.Ngluons();
    long int maxtrials = 1000000, totn = 0;
...
Tree_ME2_Base*
ATOOLS::Getter<PHASIC::Tree_ME2_Base, PHASIC::External_ME_Args,
               EXTRAXS::XS_instanton>::operator()(const External_ME_Args& args)
    const
{
  ...
  if (fl[2] != Flavour(kf_instanton)) return NULL;
