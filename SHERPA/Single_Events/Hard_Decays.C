  ...
  for (size_t blit(0); blit < bloblist->size(); ++blit) {
    ...
    if (blob->Has(blob_status::needs_harddecays) && p_dechandler) {
        ...
        if (p_dechandler->SpinCorr()) {
          ...
          if (signal) {
            ...
            if (data) {
              ...
              // save production amplitude tensor before it is changed by the
            ...
            if (Pol_CrossSec) {
              if (!Spin_Coor) {
                THROW(fatal_error, "Calculation of polarized cross sections "
                                   "only possible together with "
                                   "spin correlations")
              }
              METOOLS::Polarized_CrossSections_Handler
                  *pol_crosssection_handler =
                      p_dechandler->GetPolarizationHandler();
              if (prod_amps) {
                std::vector<METOOLS::PolWeights_Map *> polweights =
                    pol_crosssection_handler->Treat(
                        signal, prod_amps, p_dechandler->GetDecayMatrices());
                std::vector<std::string> refsystem =
                    pol_crosssection_handler->GetRefSystems();
                Weights_Map wgtmap = bloblist->WeightsMap();
                for (size_t p(0); p < polweights.size(); ++p) {
                  std::string name(refsystem[p]);
                  if (!(refsystem[p] == "Lab" || refsystem[p] == "RestFrames" ||
                        refsystem[p] == "COM" || refsystem[p] == "PPFr")) {
                ...
                signal->AddData("WeightsMap",
                                new Blob_Data<Weights_Map>(wgtmap));
                delete prod_amps;
              } else {
                THROW(fatal_error, "No Amplitude2_Tensor for calculation of "
                                   "polarized cross sections found")
              }
            }
          }
        } else
      ...
      if (!bloblist->FourMomentumConservation()) {
        msg_Tracking() << METHOD
                       << " found four momentum conservation error.\n";
