...
Scale_Variations::Scale_Variations() :
  ...
  m_gluon(Flavour(kf_gluon)), m_quark(Flavour(kf_quark)),
  m_photon(Flavour(kf_photon)),
                                         ...
                                         PHASIC::Process_Base * proc)
{
  ...
  m_params.relqcdcpl=mewgt->m_relqcdcpl;
                                 ...
                                 PHASIC::Process_Base * proc)
{
  ...
  if (proc->GetSubevtList()) {
    ...
    for (size_t i(0);i<proc->GetSubevtList()->size();++i) {
      sv->SetValue(i,Calculate(mewgttype::none,0,
                               m_params.rswgts[i],0.,
                                   ...
                                   MODEL::One_Running_AlphaS * as)
{
  ...
  if (nometstype==mewgttype::none) { // B,R,S
    ...
    double asfborn((type&mewgttype::VI||type&mewgttype::KP)?
                   pow(asnew/asold,oqcd-relqcdcpl):asf);
    ...
    std::vector<double> w(16,0.);
    for (int i(0);i<8;++i) w[i]=kpwgts[i]+kpwgts[i+8]*lf;
    for (int i(0);i<8;++i) w[i+8]=kpwgts[i+16]+kpwgts[i+24]*lf;
    double faqqcd(0.0), faqxqcd(0.0), fagqcd(0.0), fagxqcd(0.0);
    double faqqed(0.0), faqxqed(0.0), fagqed(0.0), fagxqed(0.0);
    double fbqqcd(0.0), fbqxqcd(0.0), fbgqcd(0.0), fbgxqcd(0.0);
    double fbqqed(0.0), fbqxqed(0.0), fbgqed(0.0), fbgxqed(0.0);
    if (m_kpnegativepdf || (fa>0. && fb>0.)) {
      ...
      if (w[0]!=0. || w[1]!=0. || w[2]!=0. || w[3]!=0.) {
        ...
        if (flav1.IsQuark()) {
          faqqcd=fa;
          fagqcd=pdf1->GetXPDF(m_gluon)/x1;
          pdf1->Calculate(x1/x1p,muF12new);
          faqxqcd=pdf1->GetXPDF(fl1)/x1;
          fagxqcd=pdf1->GetXPDF(m_gluon)/x1;
        }
        else if (flav1.IsGluon()) {
          fagqcd=fa;
          for (size_t i=0;i<m_quark.Size();++i)
            faqqcd+=pdf1->GetXPDF(m_quark[i])/x1;
          pdf1->Calculate(x1/x1p,muF12new);
          fagxqcd=pdf1->GetXPDF(fl1)/x1;
          for (size_t i=0;i<m_quark.Size();++i)
            faqxqcd+=pdf1->GetXPDF(m_quark[i])/x1;
        }
      ...
      if (w[4]!=0. || w[5]!=0. || w[6]!=0. || w[7]!=0.) {
        ...
        if (flav2.IsQuark()) {
          fbqqcd=fb;
          fbgqcd=pdf2->GetXPDF(m_gluon)/x2;
          pdf2->Calculate(x2/x2p,muF22new);
          fbqxqcd=pdf2->GetXPDF(fl2)/x2;
          fbgxqcd=pdf2->GetXPDF(m_gluon)/x2;
        }
        else if (flav2.IsGluon()) {
          fbgqcd=fb;
          for (size_t i=0;i<m_quark.Size();++i)
            fbqqcd+=pdf2->GetXPDF(m_quark[i])/x2;
          pdf2->Calculate(x2/x2p,muF22new);
          fbgxqcd=pdf2->GetXPDF(fl2)/x2;
          for (size_t i=0;i<m_quark.Size();++i)
            fbqxqcd+=pdf2->GetXPDF(m_quark[i])/x2;
        }
        else THROW(not_implemented,
                   std::string("Change of scales not implemented for ")
                   +ToString(fl2));
      ...
      // QED KP-Term Beam A
      if (w[8]!=0. || w[9]!=0. || w[10]!=0. || w[11]!=0.) {
        Flavour flav1(abs(fl1),fl1<0);
        if (flav1.IsQuark()) {
          faqqed=fa;
          fagqed=pdf1->GetXPDF(m_photon)/x1;
          pdf1->Calculate(x1/x1p,muF12new);
          faqxqed=pdf1->GetXPDF(fl1)/x1;
          fagxqed=pdf1->GetXPDF(m_photon)/x1;
        }
        else if (flav1.IsPhoton()) {
          fagqed=fa;
          for (size_t i=0;i<m_quark.Size();++i)
            faqqed+=pdf1->GetXPDF(m_quark[i])/x1;
          pdf1->Calculate(x1/x1p,muF12new);
          fagxqed=pdf1->GetXPDF(fl1)/x1;
          for (size_t i=0;i<m_quark.Size();++i)
            faqxqed+=pdf1->GetXPDF(m_quark[i])/x1;
        }
        else THROW(not_implemented,
                   std::string("Change of scales not implemented for ")
                   +ToString(fl1));
      ...
      // QED KP-Term Beam B
      if (w[12]!=0. || w[13]!=0. || w[14]!=0. || w[15]!=0.) {
        Flavour flav2(abs(fl2),fl2<0);
        if (flav2.IsQuark()) {
          fbqqed=fb;
          fbgqed=pdf2->GetXPDF(m_photon)/x2;
          pdf2->Calculate(x2/x2p,muF22new);
          fbqxqed=pdf2->GetXPDF(fl2)/x2;
          fbgxqed=pdf2->GetXPDF(m_photon)/x2;
        }
        else if (flav2.IsPhoton()) {
          fbgqed=fb;
          for (size_t i=0;i<m_quark.Size();++i)
            fbqqed+=pdf2->GetXPDF(m_quark[i])/x2;
          pdf2->Calculate(x2/x2p,muF22new);
          fbgxqed=pdf2->GetXPDF(fl2)/x2;
          for (size_t i=0;i<m_quark.Size();++i)
            fbqxqed+=pdf2->GetXPDF(m_quark[i])/x2;
        }
        else THROW(not_implemented,
                   std::string("Change of scales not implemented for ")
                   +ToString(fl2));
    ...
    double KPQCDnew(0.),KPQEDnew(0.);
    KPQCDnew+=(faqqcd*w[0] +faqxqcd*w[1] +fagqcd*w[2] +fagxqcd*w[3] )*fb;
    KPQCDnew+=(fbqqcd*w[4] +fbqxqcd*w[5] +fbgqcd*w[6] +fbgxqcd*w[7] )*fa;
    KPQEDnew+=(faqqed*w[8] +faqxqed*w[9] +fagqed*w[10]+fagxqed*w[11])*fb;
    KPQEDnew+=(fbqqed*w[12]+fbqxqed*w[13]+fbgqed*w[14]+fbgxqed*w[15])*fa;
    KPQCDnew*=asf*pdffac;
    KPQEDnew*=asf*pdffac;
    double KPnew(KPQCDnew+KPQEDnew);
