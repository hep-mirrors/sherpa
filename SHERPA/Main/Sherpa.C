...
bool Sherpa::SummarizeRun()
{
  if (p_eventhandler) {
    ...
    for (auto const& [key, val] : chosen_alpha_map) {
      ...
      if (!swmode) curr_xsec = dabs(xsec_map[sub_name])/chosen_efficiency_map[sub_name]/pow(chosen_alpha_map[sub_name],0.5);
    ...
    if (!swmode || m_min_tot_unc_per_day || generation_mode==0) chosen_effevperev = pow(sum_p_eff/sum_p_unw,2)*pow(sum_p_eff_sign/sum_p_eff,2);
	...
	if (!swmode) curr_xsec = dabs(xsec_map[sub_name])/efficiency_manual_map[sub_name][i]/pow(alpha_manual_map[sub_name][i],0.5);
	...
	sum_t_ov_after += (overhead_after+sudakov_efficiency[sub_name]*timing_statistics_det_sim)*efficiency_manual_map[sub_name][i]*curr_xsec;
      ...
      if (!swmode || m_min_tot_unc_per_day) mean_manual_alpha[i] = pow(sum_p_eff/sum_p_unw,2)*pow(sum_p_eff_sign/sum_p_eff,2);
	...
	for(int i=0; i < epsilon_values.size(); i++){	
	  ...
	  if (!swmode) curr_xsec = dabs(xsec_map[sub_name])/efficiency_manual_fscan_map[sub_name][fi][i]/pow(alpha_manual_fscan_map[sub_name][fi][i],0.5);
	...
	sum_p_eff_sign += opt_p_eff_sign;
      ...
      if (!swmode || m_min_tot_unc_per_day) mean_manual_alpha_fscan[fi] = pow(sum_p_eff/sum_p_unw,2)*pow(sum_p_eff_sign/sum_p_eff,2);
