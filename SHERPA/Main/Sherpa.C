...
bool Sherpa::SummarizeRun()
{
  if (p_eventhandler) {
    ...
    double optimal_manual_sum_xsec = 0;
	...
	sum_t_trial += (tges+efficiency_manual_map[sub_name][i]*overhead_after)*curr_xsec;
	...
	sum_alpha += alpha_manual_map[sub_name][i]*efficiency_manual_map[sub_name][i]*sudakov_efficiency[sub_name]*curr_xsec; //=sum_p_eff - everything consistent :)
      ...
      mean_manual_events_up[i] = 60*60*24/(sum_t_trial/(sum_p_unw_up_corr+pow(sum_p_unw_up,0.5))); //*sum_xsec/sum_xsec
	...
	optimal_manual_sum_xsec=sum_xsec;
      }
    }

    //time per subprocess for optimised solution
    /*
    msg_Info()<<std::endl<<"time per subprocess for optimised solution (emax="<<epsilon_values[optimal_manual_i]<<"):"<<std::endl;
    for (auto const& [key, val] : alpha_manual_map) {
      std::string sub_name = key;
      //msg_Info() << sub_name << std::endl;
      double curr_xsec = xsec_map[sub_name]/efficiency_manual_map[sub_name][optimal_manual_i]/alpha_manual_map[sub_name][optimal_manual_i]; //need to weight with sampling probability in manual approach. Why not sudakov? - bacause happens afterwards - but still more events needed for optimal eff events? no
      double tges  = (time_map["sum_total_"+sub_name])/number_map["n_total_"+sub_name]; //in s
      if (number_map["n_total_"+sub_name]==0) {
	tges = 0; //critical, because underestimate - need to make sure that enough events generated...unc estimate hard, because 0 is 0
      }
	...
	overhead_after = 0;
      ...
      double temp_t_trial = (tges+efficiency_manual_map[sub_name][optimal_manual_i]*overhead_after)*curr_xsec;
      msg_Info()<<" "<< std::right<<std::setw(2) << round(temp_t_trial/optimal_manual_sum_t_trial*100)<<"% in '"<<sub_name<<"'"<<std::endl;
    }*/
	...
	sum_alpha += alpha_manual_fraction_map[sub_name][i]*efficiency_manual_map[sub_name][i]*sudakov_efficiency[sub_name]*curr_xsec; //=sum_p_eff - everything consistent :)
      ...
      mean_manual_fraction_alpha[i] = sum_alpha/sum_p_unw;
      mean_manual_fraction_events[i] = 60*60*24/(sum_t_trial/sum_p_unw);
      mean_manual_fraction_eff_events[i] = 60*60*24/(sum_t_trial/sum_p_eff);
      //"=" to update optimal_manual_sum_eff, if i=0 is optimal
    ...
    std::cout << "+-----------------------------+---------------------------------+---------------------------------+" << std::endl;
    std::cout << "|                             |                                 |       Large weight region       |" << std::endl;
    std::cout << "|                             |           Average region        |        (fraction: "<< timing_statistics_large_weight_fraction <<")        |" << std::endl;
    std::cout << "| epsilon_max    events/day   | eff. events/day     sample size | eff. events/day     sample size |" << std::endl;
    std::cout << "+-----------------------------+---------------------------------+---------------------------------+" << std::endl;
    const auto default_precision{std::cout.precision()};
