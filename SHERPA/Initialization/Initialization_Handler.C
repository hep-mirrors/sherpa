...
InitISRHandler(const PDF::isr::id & pid,Settings& settings) {
  ...
  for (size_t beam = 0; beam < 2; beam++) {
    ...
    std::string set = ((sets.empty() ||
			sets[Min(beam, sets.size() - 1)] == "Default") ?
		       m_defsets[pid][beam] :
		       sets[Min(beam, sets.size() - 1)]);
    int version =     (versions.empty() ? 0 :
		       versions[Min(beam, versions.size() - 1)]);
    int order = -1, scheme = -1;
    // special treatment of electron PDF
    if ((pid==PDF::isr::hard_process || pid==PDF::isr::hard_subprocess) &&
        set=="PDFe") {
    ...
    Flavour flav;
    if (pid!=isr::bunch_rescatter) flav = m_bunch_particles[beam];
    else {
      ...
      m_bunchtags[isr::bunch_rescatter][beam] = bbrmode & (beam + 1) ? 0 : 1;
    }
    PDF_Arguments args = PDF_Arguments(flav,beam,set,version,order,scheme);
    if (pid!=isr::bunch_rescatter) {
...
bool Initialization_Handler::InitializeTheRemnants() {
  ...
  // Set up the correct combination of remnant handlers for the bunch
  // rescattering, depending on the mode
  if (m_isrhandlers.find(isr::bunch_rescatter)!=m_isrhandlers.end()) {
	...
	nullptr, nullptr};
      // For mixed beam-bunch rescattering, the remnant for the bunch has already
      // been created for the hard process; we need to get it from there and hand it over
      remnants[bbrmode - 1] =
	m_remnanthandlers[PDF::isr::hard_process]->GetRemnant(bbrmode - 1);
      m_remnanthandlers[isr::bunch_rescatter] =
  ...
  if (m_remnanthandlers.find(isr::bunch_rescatter) != m_remnanthandlers.end())
    msg_Info()<<"  Rescattering: "
	      <<(m_remnanthandlers[isr::bunch_rescatter]->GetRemnant(0)->
		 GetBeam()->Bunch(m_bunchtags[isr::bunch_rescatter][0]))<<" ("
	      <<m_remnanthandlers[isr::bunch_rescatter]->GetRemnant(0)->Type()<<") + "
	      <<(m_remnanthandlers[isr::bunch_rescatter]->GetRemnant(1)->
		 GetBeam()->Bunch(m_bunchtags[isr::bunch_rescatter][1]))<<" ("
...
bool Initialization_Handler::InitializeTheShowers()
{
  ...
  std::vector<isr::id> isrtypes = {isr::hard_process, isr::hard_subprocess};
  if (m_isrhandlers.find(isr::bunch_rescatter)!=m_isrhandlers.end())
...
bool Initialization_Handler::InitializeTheUnderlyingEvents()
{
  ...
  if (m_isrhandlers[isr::hard_process]->Mode() != PDF::isrmode::hadron_hadron)
    Settings::GetMainSettings()["MI_HANDLER"].OverrideScalar<std::string>(
            "None");
  std::vector<isr::id> isrtypes = {isr::hard_subprocess};
  if (m_isrhandlers.find(isr::bunch_rescatter) != m_isrhandlers.end())
    isrtypes.push_back(isr::bunch_rescatter);
  for (isr::id id : isrtypes) {
    as->SetActiveAs(isr::hard_subprocess);
    MI_Handler * mih = new MI_Handler(p_model,m_isrhandlers[id],
				      p_yfshandler, m_remnanthandlers[id]);
