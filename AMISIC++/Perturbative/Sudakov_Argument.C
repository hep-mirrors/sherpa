...
void Sudakov_Argument::FillTables() {
  ...
  for (m_sbin=0;m_sbin<sbinmax;m_sbin++) {
    ...
    m_norm = ( p_processes->GetXSecs()->XSndNorm() *
...
void Sudakov_Argument::OutputTables() {
  ...
  if (m_variable_s && m_variable_b)
    msg_Info()<<std::setw(10)<<"E_{c.m.} [GeV]"<<" | "
	      <<std::setw(10)<<"b [fm]"<<" | ";
  if (m_variable_s)
    msg_Info()<<std::setw(23)<<"E_{c.m.} [GeV]"<<" | ";
  if (m_variable_b)
    msg_Info()<<std::setw(10)<<"b [fm]"<<" | ";
  msg_Info()<<std::setw(10)<<"xs_hard/xs_ND"<<" | "
	    <<std::setw(10)<<"pt"<<" | "
	    <<std::setw(15)<<"f(pt^2)"<<" |  "
	    <<std::setw(15)<<"Int(pt^2)"<<" | "
	    <<std::setw(10)<<"Int(pt^2) * sigma_ND"<<"\n"
  ...
  for (size_t sbin=0;sbin<sbinmax;sbin++) {
	...
	msg_Info()<<std::setw(15)<<xsratio<<" | "
		  <<std::setw(14)<<sqrt(p_pt2bins->x(bin))<<" | ";
	if (m_variable_s && m_variable_b)
	  msg_Info()<<std::setw(10)<<p_function3->Value(sbin,bbin,bin)<<" | "
		    <<std::setw(10)<<p_integral3->Value(sbin,bbin,bin)<<" | "
		    <<std::setw(10)<<(p_integral3->Value(sbin,bbin,bin)*xsnd);
	if (m_variable_b)
    ...
    msg_Info()<<"Calculated look-up tables and values for the"
	      <<"Sudakov_Argument:\n"
	      <<"     (non-diffractive xsec = "<<xsnd<<" GeV^-2 = "
	      <<(xsnd*rpa->Picobarn()/1e9)<<" mb, "
	      <<"total xsec = "<<p_processes->GetXSecs()->XStot()<<" mb vs \n"
