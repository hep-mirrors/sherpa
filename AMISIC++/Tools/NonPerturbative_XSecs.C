...
NonPerturbative_XSecs::~NonPerturbative_XSecs() {
  if (m_fails>0)
    msg_Info()<<METHOD<<" with "<<m_fails<<" fails in "<<m_calls<<" calls.\n";
}

void NonPerturbative_XSecs::SetBeams(BEAM::Beam_Base * beam1,BEAM::Beam_Base * beam2) {
  ////////////////////////////////////////////////////////////////////////////////////
  // This essentially fixes whether we have to select a c.m. frame of the collision
  // (in the case of EPA, i.e. photon-photon or photon-nucleon collisions) or not
  // (for pure hadronic initial states, i.e. nucleon-nucleon collisions).
  ////////////////////////////////////////////////////////////////////////////////////
...
void NonPerturbative_XSecs::CalculateSDependentCrossSections() {
  ////////////////////////////////////////////////////////////////////////////////////
  // Pre-calculating the s-dependent cross sections by folding the EPA spectrum with
  // the non-perturbative cross sections enoced in the p_xsecs, the
  // Hadronic_XSec_Calculator class.  The initial-state integration is performed
  // within the Beam_Integrator class.
  ////////////////////////////////////////////////////////////////////////////////////
  double GeV2mb = rpa->Picobarn()/1.e9;
  if (m_variableS) {
    m_integrator.Init(p_xsecs,p_beams[0],p_beams[1]);
    m_xsec = m_integrator.TotalXSec();
    msg_Tracking()<<METHOD<<" yields xsec = "<<m_xsec<<" mb, "
		  <<"compare with "<<(p_xsecs->SimulatedXSec(100.)/GeV2mb)<<" mb "
		  <<"at 10 GeV without EPA spectrum.\n";
  }
  else {
    m_xsec = p_xsecs->SimulatedXSec(m_s)/GeV2mb;
    msg_Tracking()<<"   "<<string(99,'-')<<"\n"
		  <<"   | "<<METHOD<<": hadronic cross sections (all in mb)    |\n"
		  <<"   | sigma_tot   | sigma_act   | sigma_el    |"
		  <<"sigma_SDA    | sigma_SDB   | sigma_DD    | sigma_ND    |\n"
		  <<"   | "
		  <<setprecision(6)<<setw(11)<<(p_xsecs->XStot())<<" | "
		  <<setprecision(6)<<setw(11)<<(m_xsec*GeV2mb)<<" | "
...
bool NonPerturbative_XSecs::MakeScatter(Blob * blob) {
  //////////////////////////////////////////////////////////////////////////////////
  // Creating an initial state kinematics, i.e. its c.m. energy squared s and its
  // rapidity y - the latter also fixing the boost from the c.m. to the lab frame.
  // In the second step the mode (elastic/diffractive) is fixed and the final state
  // flavours are selected.
  // For initial photons these can be either of the four lowest vector mesons:
  // rho (770), omega (782), phi (1020), or J/Psi.  Then the final-state
  // kinematics is created according to the differential cross sections of the
  // Schuler-Sjostrand model
  // (relevant equations are documented in the respective methods).
  //////////////////////////////////////////////////////////////////////////////////
  m_calls++;
  if (m_variableS) {
    if (!m_integrator()) {
...
{
  ////////////////////////////////////////////////////////////////////////////////////
  // Differential elastic scatter dsigma_el/dt as given in (7).  We consciously ignore
  // the effect of Reggeon exchange that would be relevant for lower c.m. energies, and
  // operate solely in the high-energy limit.  The couplings of the pomeron to the
  // hadrons, beta, are encoded through intercept and slope, and realised as inline
  // funtions in the .H file.
...
bool NonPerturbative_XSecs::ElasticScatter(Blob * blob) {
  ...
  // again in the high-energy limit also used in DiffElXSec.  Once t is chosen, we
  // create outgoing momenta in FixOutMomenta() and fill the scatter blob.
					 ...
					 const double & M2,const size_t & pos) const
{
  ...
  // in the high-energy limit.   In the spirit of Schuler-Sjostrand we modify this
  // however with the F_SD function of (22), to arrive at the differential form
  // underpinning (24) and (25), but without the slope modifications of (20).
...
bool NonPerturbative_XSecs::SingleDiffractiveScatter(const size_t & pos,Blob * blob) {
  ////////////////////////////////////////////////////////////////////////////////////
  // MC realisation of selecting the 4-momentum transfer t and the diffractive mass M
  // in single diffractive scattering of particle at pos = {0,1}, again in the
  // high-energy limit.  This is the MC realisation of the DiffSDXSec above.
  // After t and M are fixed and the outgoing momenta are constructed in FixOutMomenta,
  // the diffracted hadron is split into constitutents with SplitDiffractiveState(pos)
  // and the scatter blob is filled.
  // In the Tests method we explicitly verify that the MC reproduces the differential
  ...
  do {
    ...
    do {
      ...
      value  = ( F_SD(M2,M2res) *
		 exp(2.*(p_xsecs->s_slopes[m_hadtags[1-pos]]+
			 m_alphaP_pomeron*log(m_s/M2))*t) /
		 exp(argt * t) );
    } while (value/maxval<ran->Get());
    m_outmasses2[2*pos]     = M2;
  } while (!FixOutMomenta(t));
  //msg_Out()<<METHOD<<"(pos = "<<pos<<"): M = "<<sqrt(M2)<<".\n";
...
{
  ///////////////////////////////////////////////////////////////////////////////////
  // Differential double diffractive scatter dsigma_el/dt dM_1^2 dM_2^2 as given
  // in (13), again in the high-energy limit.
  // In the spirit of Schuler-Sjostrand we modify this however with the F_DD function
  // of (22), to arrive at the differential form underpinning (24) and (25), but
  // again without the slope modifications of (23).
...
bool NonPerturbative_XSecs::DoubleDiffractiveScatter(ATOOLS::Blob * blob) {
  ////////////////////////////////////////////////////////////////////////////////////
  // MC realisation of selecting the 4-momentum transfer t and the diffractive masses
  // M_1 and M_2 in double diffractive scattering of particle, again in the high-energy
  // limit.  This is the MC realisation of the DiffDDXSec above.
  // After t, M_1 and M_2 are fixed and the outgoing momenta are constructed in
  // FixOutMomenta, the diffracted hadron is split into constitutents with
  // SplitDiffractiveState() and the scatter blob is filled.
  // In the Tests method we explicitly verify that the MC reproduces the differential
  ...
  InitBlob(blob, max(M2[0],M2[1]),max(M2[0],M2[1]));
  return true;
}
...
bool NonPerturbative_XSecs::SetRhoMasses2() {
  ////////////////////////////////////////////////////////////////////////////////////
  // If there are rho mesons in the final state and we apply any form of in-class
  // distribution of virtual rho masses in their decay into two pions, we fix the rho
  // mass here.
  ////////////////////////////////////////////////////////////////////////////////////
...
double NonPerturbative_XSecs::RhoMassModifier(const double & M2) {
  ...
  ////////////////////////////////////////////////////////////////////////////////////
  double A_nr = ( (m_twopions==two_pions::cont_only ||
		   m_twopions==two_pions::rho_omega_cont) ?
  ...
  if (m_twopions==two_pions::rho_omega || m_twopions==two_pions::rho_omega_cont) {
    ...
    double  GomegaM2    = m_Gomega*qomegaratio*m_momega/sqrt(M2);
    double Gomega2M2    = GomegaM2*GomegaM2;
    Complex BWomega     = ( m_momega*m_Gomega/(sqr(M2-m_momega2)+m_momega2*Gomega2M2) *
			    Complex(M2-m_momega2, -m_momega*GomegaM2) );
    A += ( m_f_omega * M2/m_momega2 *
	   Complex(cos(m_phi_omega),sin(m_phi_omega)) * BWomega );
  }
  ////////////////////////////////////////////////////////////////////////////////////
...
bool NonPerturbative_XSecs::SplitDiffractiveState(const size_t & pos) {
  ...
  // isotropically in its own rest frame.  This method also includes a boost back
  // of the constituent momenta into the lab-frame.
  ////////////////////////////////////////////////////////////////////////////////////
  Vec4D  diffmom  = m_outmom[2*pos];
  double M2       = diffmom.Abs2();
  //msg_Out()<<METHOD<<"(pos = "<<pos<<", mom = "<<diffmom<<", M = "<<sqrt(M2)<<"\n";
  ...
  if (diff==2212) {
	...
	M2>sqr(Flavour(kf_d).HadMass()+Flavour(kf_uu_1).HadMass())) {
      m_outflav[2*pos]=Flavour(kf_d); m_outflav[2*pos+1]=Flavour(kf_uu_1);
	     ...
	     M2>sqr(Flavour(kf_u).HadMass()+Flavour(kf_ud_1).HadMass())) {
...
InitBlob(Blob * blob,const double & muR2,const double & muQ2) {
  ...
  blob->AddData("Weight_Norm",new Blob_Data<double>(m_xsec*rpa->Picobarn()));
}

void NonPerturbative_XSecs::Tests() {
  InitHistos();
  double E = 7000;
  double p = sqrt(sqr(sqr(E)-m_masses2[0]-m_masses2[1])-
		  4.*m_masses2[0]*m_masses2[1])/(2.*E);
  m_s = sqr(E);
  (*p_xsecs)(m_s);
...
void NonPerturbative_XSecs::TestSingleD() {
  ...
  for (size_t i=0;i<m_Nevents/100;i++) {
    Blob * blob = new Blob();
    SingleDiffractiveScatter(0,blob);
