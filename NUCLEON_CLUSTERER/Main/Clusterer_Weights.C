...
Clusterer_Weights::Clusterer_Weights(Clusterer_Base * clusterer) :
  p_clusterer(clusterer)
{
  SetLists();
}

void Clusterer_Weights::SetLists() {
  for (size_t pos=0;pos<2;pos++) p_parts[pos] = p_clusterer->GetParts(pos);
}

void Clusterer_Weights::Reset() {
...
void Clusterer_Weights::FillTables() {
  Particle * trip, * anti;
  for (ParticleSet::iterator tit=p_parts[0]->begin();
    ...
    for (ParticleSet::iterator ait=p_parts[1]->begin();
	 ait!=p_parts[1]->end();ait++) {
      ...
      (*m_distances[trip])[anti] = p_clusterer->Distance(trip,anti);
    }
  }
  //OutputWeightTable();
}
void Clusterer_Weights::OutputWeightTable() {
