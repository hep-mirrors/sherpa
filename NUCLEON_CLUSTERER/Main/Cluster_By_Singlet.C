...
Cluster_By_Singlet::Cluster_By_Singlet() :
  Clusterer_Base(), m_weights(Clusterer_Weights(this)) {}

Cluster_By_Singlet::~Cluster_By_Singlet() {}

int Cluster_By_Singlet::operator()(Blob_List *const blobs) {
...
void Cluster_By_Singlet::Reset() {
  m_singlets.clear();
  m_weights.Reset();
  Clusterer_Base::Reset();
}

void Cluster_By_Singlet::SetParameters() {
  ...
  auto  s = Settings::GetMainSettings()["COLOUR_NUCLEON_CLUSTERER"];
...
void Cluster_By_Singlet::MakeSinglets() {
...
void Cluster_By_Singlet::FlattenSinglets() {
  m_particles.clear();
...
Particle * Cluster_By_Singlet::FindStart() {
  Particle * start(NULL);
...
Particle * Cluster_By_Singlet::FindNext(const size_t & col) {
  std::map<unsigned int, ATOOLS::Particle *>::iterator cpit = m_cols[1].find(col);
  if (cpit==m_cols[1].end())
    THROW(fatal_error,"Cluster_By_Singlet::FindNext did not find particle with the right colour.");
...
void Cluster_By_Singlet::ReshuffleSinglets() {
...
bool Cluster_By_Singlet::ReshuffleSinglet(Part_List * singlet) {
...
void Cluster_By_Singlet::ClusterSinglets() {
  ...
  do {
    ...
    while (sit2!=m_singlets.end() && !hit) {
      ...
      // if they should be Clustered cross-wise, i.e., if singlet 1 contains of
...
void Cluster_By_Singlet::SpliceSinglets(Part_List * sing1,Part_List * sing2,
...
void Cluster_By_Singlet::AftermathOfSlicing(Particle * part11,Particle * part12,
...
double Cluster_By_Singlet::Distance(ATOOLS::Particle * trip,ATOOLS::Particle * anti) {
  return (MomDistance(trip,anti) *
	  PosDistance(trip,anti) *
	  ColDistance(trip,anti));
}

double Cluster_By_Singlet::MomDistance(Particle * part1,Particle * part2) {
...
double Cluster_By_Singlet::PosDistance(Particle * part1,Particle * part2) {
  double xdist2 = dabs((part1->XProd().Perp()-part2->XProd().Perp()).Abs2());
  return xdist2<1.e-6? 1. : pow(xdist2/m_R02, m_etaR);
}

double Cluster_By_Singlet::ColDistance(Particle * part1,Particle * part2) {
