...
Cluster_Statistical::Cluster_Statistical() :
  Clusterer_Base(),
...
void Cluster_Statistical::SetParameters() {
...
void Cluster_Statistical::Reset() {
  m_collist.clear();
  Clusterer_Base::Reset();
}

void Cluster_Statistical::FixPMode(const string & pm) {
...
int Cluster_Statistical::operator()(Blob_List *const blobs) {
...
bool Cluster_Statistical::AttemptSwap(const unsigned int col[2]) {
...
void Cluster_Statistical::UpdateColours() {
  for (size_t i=0;i<2;i++) {
...
double Cluster_Statistical::Distance(Particle * trip,Particle * anti) {
  return MomDistance(trip,anti);
}

double Cluster_Statistical::MomDistance(Particle * trip,Particle * anti) {
...
double Cluster_Statistical::PosDistance(Particle * trip,Particle * anti) {
  double xdist2 = dabs((trip->XProd()-anti->XProd()).Abs2());
  return xdist2<m_R02 ? 1. : pow(xdist2/m_R02, m_etaR);
}

double Cluster_Statistical::ColDistance(Particle * trip,Particle * anti) {
  return trip->GetFlow(1)==anti->GetFlow(2) ? 1. : m_reshuffle;
}

double Cluster_Statistical::TotalLength() {
  double total = 1.;
