  ...
  if(pre_event){
    ...
    for (list<shared_ptr<Parton>>::iterator iter1=p_partons->begin(); iter1!=p_partons->end(); iter1++) {
      ...
      if(found_id_1 != 1 && flow_id_1 != 0){
        msg_Out() << "ERROR: flow_id = " << flow_id_1 << " has " << found_id_1 << " anti-partners" << endl;
        all_found = false;
      }
      if(found_id_2 != 1 && flow_id_2 != 0){
        msg_Out() << "ERROR: flow_id = " << flow_id_2 << " has " << found_id_2 << " anti-partners" << endl;
        all_found = false;
      }
      if(found_err_1 != 0 && flow_id_1 != 0){
        msg_Out() << "ERROR: flow_id = " << flow_id_1 << " has " << found_err_1 << " copies" << endl;
        all_found = false;
      }
      if(found_err_2 != 0 && flow_id_2 != 0){
        msg_Out() << "ERROR: flow_id = " << flow_id_2 << " has " << found_err_2 << " copies" << endl;
        all_found = false;
      }
    }

    if(all_found){
      msg_Out() << "All colour indices match" << endl;
    } else{
      msg_Out() << "Error in colour indices post run" << endl;
      //exit(1.);
    }
  }
}
