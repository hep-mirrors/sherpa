             ...
             m_test_double(HIPars.TestDouble()), m_test_bool(HIPars.TestBool())
{
  ...
  msg_Out() << "########## HIPars.TauRestart() = " << HIPars.TauRestart() << endl;

  //Handlers
  p_colour_handler = make_shared<Colour_Handler>(p_partons);
...
int Event_Generator::GenerateEvent(shared_ptr<list<shared_ptr<Parton>>> ptr_parton_list) {
...
void Event_Generator::FillTauTables(shared_ptr<list<shared_ptr<Parton>>> ptr_parton_list) {
  if(p_partons->size() == 0){
